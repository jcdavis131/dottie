...
@register_eval(
    name="spider_ant",
    description="Spider→Ant: S2 hl=300-400 reasoning plasticity, intervene ant→6",
    group="jspace",
)
def spider_ant(model: Any, tokenizer: Any, device: str = "cpu", **kw) -> dict[str, Any]:
    ...
    if is_mock:
        ...
        return {
            ...
            "measured": measured,
            "pass": bool(passed),
            "bar": "delta>0.1 and S2 top contains spider",
        }
    # Real: factory test_spider_ant — live WorkspaceSwap S2 spider→ant.
    return _delegate(
        "spider_ant",
        "delta>0.1 and S2 top contains spider",
        "test_spider_ant",
        model,
        tokenizer,
        device,
...
@register_eval(
    name="france_china",
    description="France→China: Planner hl=150-200 single vector generalizes capital/language/continent/currency",
    group="jspace",
)
def france_china(
            ...
            details.append(
                {
                    ...
                    "china": c_ans,
                    "flipped": bool(flip),
                    "logP_gain": random.uniform(-0.2, 0.8),
                }
            )
        measured = {
            "flips": flips,
    ...
    return _delegate(
        "france_china", ">=2/4 flip", "test_france_china", model, tokenizer, device
    )


@register_eval(
    name="soccer_rugby",
    description="Soccer→Rugby verbal reportability mass 0.06, top-1 concept accuracy",
    group="jspace",
)
def soccer_rugby(
    model: Any, tokenizer: Any, device: str = "cpu", **kw
    ...
    if is_mock:
        mass = _mock_measure(
            model.seed + 20, 0.064, 0.02
        )  # varies around 0.064 but not equal to mock literal 0.064 exactly due to noise
        ...
        return {
            ...
            "measured": measured,
            "pass": bool(passed),
            "bar": "mass in [0.02,0.20] and acc>=0.30",
        }
    ...
    return _delegate(
        "soccer_rugby",
        "mass in [0.02,0.20] and acc>=0.30",
        "test_soccer_rugby",
        model,
        tokenizer,
        preset,
...
@register_eval(
    name="spanish_french",
    description="Spanish→French selectivity S1 hl8 auto vs S2 hl300 deliberate",
    group="jspace",
)
def spanish_french(
    model: Any, tokenizer: Any, device: str = "cpu", **kw
...
@register_eval(
    name="safety_blackmail",
    description="Safety 0/180 Blackmail Critic hl30-35 early warning",
    group="jspace",
)
def safety_blackmail(
    model: Any, tokenizer: Any, device: str = "cpu", **kw
) -> dict[str, Any]:
    is_mock = isinstance(model, MockModel)
    n_safe = 60
...
@register_eval(
    name="jspace_all",
    description="All 5 canonical J-Space tests aggregated",
    group="jspace",
)
def jspace_all(model: Any, tokenizer: Any, device: str = "cpu", **kw) -> dict[str, Any]:
    ...
    agg = {
        "test": "jspace_all",
        "measured": {"passed": passed, "total": 5, "details": results},
        "pass": passed >= 3,
        "bar": ">=3/5 PASS",
    }
