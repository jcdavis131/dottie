...
def scan_wiki(wiki_path: str | None = None) -> list[pathlib.Path]:
    ...
    candidates.extend(
        [
            ...
            / "family-brain-os"
            / "src"
            / "wiki",  # family brain inspired version
        ]
    )
    found = []
    for p in candidates:
...
@register_eval(
    name="openwiki_knowledge",
    description="Wiki recall: S2 reportability mass for openwiki pages",
    group="knowledge",
)
def openwiki_knowledge(
    model: Any, tokenizer: Any, device: str = "cpu", wiki_path: str | None = None, **kw
) -> dict[str, Any]:
    if not isinstance(model, MockModel):
        ...
        return attach_smoke_labels(
            {
                "test": "openwiki_knowledge",
                "measured": measured,
                "pass": measured["recall_mass"] >= 0.06,
                "bar": BAR,
            }
        )
    wiki_files = scan_wiki(wiki_path)
    if not wiki_files:
        ...
        measured = {
            "n_wiki_files": 0,
            "recall_mass": random.uniform(0.05, 0.15),
    ...
    measured = {
        ...
        "sampled": len(scores),
        "recall_mass": avg,
        "files": [str(p) for p in wiki_files[:5]],
    }
    return {
        "test": "openwiki_knowledge",
        "measured": measured,
