...
@register_eval(
    name="probes",
    description="Arithmetic, modus_ponens, facts, code_out probes ≥200 items each",
    group="core",
)
def probes(
    model: Any, tokenizer: Any, device: str = "cpu", probe_n: int = 200, **kw
) -> dict[str, Any]:
    if not isinstance(model, MockModel):
        ...
        detail = mods["evals.probes"].score_probes(
            model, tokenizer, n_per_set=probe_n, device=device
        )
        scores = {name: v["accuracy"] for name, v in detail.items()}
        passed = (
            scores.get("arithmetic", 0.0) >= 0.60 and scores.get("facts", 0.0) >= 0.70
        )
    ...
    return {
        "test": "probes",
        "measured": {"acc": scores, "n_per_set": probe_n},
        "pass": bool(passed),
        "bar": BAR,
    }
