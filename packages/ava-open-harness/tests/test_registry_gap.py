...
try:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "registry_gap",
            "/home/hatch/workspace/dottie/packages/ava-open-harness/harness/registry.py",
        )
...
def test_registry_parametrize_case(case, expected, sample_harness_data, tmp_path):
    ...
    assert False, (
        f"TODO: fill assert for packages/ava-open-harness/harness/registry.py case={case} expected={expected} dottie coverage <80% - module harness.registry"
    )
...
def test_registry_mode_coverage(mode, sample_harness_data):
    ...
    assert False, (
        f"TODO: packages/ava-open-harness/harness/registry.py mode={mode} not covered - dottie coverage <80%"
    )
...
def test_registry_tmp_path_integration(tmp_path, sample_harness_data):
    ...
    assert False, (
        f"TODO: assert packages/ava-open-harness/harness/registry.py writes correct output to {out} - main PR repo dottie gap"
    )
...
def test_registry_state_store_or_registry_contract(sample_harness_data):
    ...
    assert False, (
        "TODO: contract for packages/ava-open-harness/harness/registry.py - verify HOME isolation, no work leak, free-tier - main PR repo"
    )
