...
| §08 (goal strings) | `router.py` `route_goal` | **the** Dottie router: scout (`scout route`, `harness route`, `harness run`) and jarvisd call it. MoMA-lite decides; learned backends are advisory unless gate_passed AND human-stamped, and only for an equal-or-cheaper tier. Default decisions equal the pre-unification heuristic (golden-tested). See `docs/ROUTER.md` |
| §08 backends | `backends.py`, `mlp_infer.py` | the MoMA-lite heuristic (its one implementation), the orchestrator MLP (numpy lazy), System One over `/decide` (stdlib urllib, never raises; `shape_concentration`, never `confidence`; `untrained` = no signal) |
| §16 traces | `traces.py` | one JSONL line per routed goal + its run outcome, `~/.dottie/traces` (`DOTTIE_TRACE_DIR`), `source: production|test`, goal hash + features (text only with `DOTTIE_TRACE_TEXT=1`); never written under pytest by default |
| §21/§24/§25 router loop | `router_training.py`, `router_artifacts.py`, `jev.py` | traces -> strict `jev-decision-schema-1.0.0` pack (observed-outcome labels, whole-goal holdout ≥ 20%, manifest sha256, `consent.champion=false`, test rows refused) -> eval vs heuristic (`evaluate_gates` + `promotion_decision` -> `gate_passed`) -> human stamp (`scout router promote --i-have-reviewed`). Nothing auto-promotes |
