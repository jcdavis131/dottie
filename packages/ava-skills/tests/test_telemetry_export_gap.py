...
try:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("telemetry_export_gap", "/home/hatch/workspace/dottie/packages/ava-skills/skills/telemetry_export.py")
...
def test_telemetry_export_parametrize_case(case, expected, sample_harness_data, tmp_path):
    ...
    assert False, f"TODO: fill assert for packages/ava-skills/skills/telemetry_export.py case={case} expected={expected} dottie coverage <80% - module skills.telemetry_export"
...
def test_telemetry_export_mode_coverage(mode, sample_harness_data):
    ...
    assert False, f"TODO: packages/ava-skills/skills/telemetry_export.py mode={mode} not covered - dottie coverage <80%"
...
def test_telemetry_export_tmp_path_integration(tmp_path, sample_harness_data):
    ...
    assert False, f"TODO: assert packages/ava-skills/skills/telemetry_export.py writes correct output to {out} - main PR repo dottie gap"
...
def test_telemetry_export_state_store_or_registry_contract(sample_harness_data):
    ...
    assert False, f"TODO: contract for packages/ava-skills/skills/telemetry_export.py - verify HOME isolation, no work leak, free-tier - main PR repo"
