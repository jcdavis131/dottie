...
def gen_truth_tables(n: int = 50) -> list[dict[str, Any]]:
    ...
    op_names = list(
        OPS
    )  # AND, OR, IMPLIES — cycling guarantees IMPLIES coverage for n>=3
    out: list[dict[str, Any]] = []
    for i in range(n):
        op = op_names[i % len(op_names)]
        rows = [
            {"P": a, "Q": b, "value": OPS[op](a, b)}
            for a in (True, False)
            for b in (True, False)
        ]
        out.append(
...
def run(
    ...
    if mode == "mock":
        ...
        measured = {
            "n_generated": len(tables) + len(sylls),
        ...
        return {
            ...
            "mode": "mock",
            "measured": measured,
            "pass": len(tables) + len(sylls) > 0,
            "bar": "generation non-empty",
        }
