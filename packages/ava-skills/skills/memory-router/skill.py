...
def _shardmemo_scope_before_routing(instruction: str) -> dict[str, Any]:
    ...
    tier_b_scope = (
        max(tier_b_scores, key=tier_b_scores.get) if tier_b_scores else "S2_slow_300"
    )
    ...
    tier_c_scope = (
        max(tier_c_scores, key=tier_c_scores.get) if tier_c_scores else "base"
    )
    ...
    return {
        "tier_a": {
            "triggered": tier_a_triggered,
            "scope": tier_a_scope,
            "keywords": TIER_A_SAFETY_KEYWORDS[:3],
        },
...
def run(
    ...
    **kw,
    ...
    recalled, recall_error = _recall_minted(
        query,
    ...
    return {
        ...
        "targets": targets,
        "pass": passed,
        "bar": "kl<1.0 + ShardMemo Tier A/B/C",
        "shardmemo": shardmemo,
    }
...
def _recall_minted(
    ...
    try:
        ...
        if name in sys.modules:
            ...
            sys.modules[name] = (
                mint  # required before exec: dataclasses resolve cls.__module__
            )
        ...
        return (
            [],
            None,
        )  # mint present but unimportable in this env: treated as not installed
