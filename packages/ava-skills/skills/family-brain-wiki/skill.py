...
def _scan_family_wiki() -> list[dict[str, Any]]:
    ...
    for name in ["your_files", "family-brain-os"]:
        ...
        if p.exists():
            candidates.extend(
                [{"source": str(f), "type": "md"} for f in p.rglob("*wiki*.md")][:5]
            )
    return candidates
...
def _load_export(export_path: str | pathlib.Path) -> list[dict[str, str]]:
    ...
    if not isinstance(raw, list):
        raise ValueError(
            f"export must contain a list of pages under {STORAGE_KEY!r}, got {type(raw).__name__}"
        )
    return [
        {"title": str(pg.get("title", "")), "body": str(pg.get("body", ""))}
        for pg in raw
...
def run(model: Any = None, tokenizer: Any = None, mode: str = "mock", **kw):
    ...
    if export_path:
        try:
            ...
            pages_source = "export"
            wiki_info.insert(
                0, {"source": str(export_path), "type": "localStorage-export"}
            )
    ...
    measured = {
        "n_wiki_sources": len(wiki_info),
        "n_pages_generated": len(pages),
