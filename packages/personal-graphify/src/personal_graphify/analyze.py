...
def god_nodes(G: nx.MultiDiGraph, top_n: int = 15) -> list[tuple[str, dict]]:
    ...
    deg = dict(G.degree())
    sorted_nodes = sorted(
        G.nodes(data=True), key=lambda x: deg.get(x[0], 0), reverse=True
    )
...
def naive_token_estimate(G: nx.MultiDiGraph, files_count: int = 0):
    ...
    return (
        G.number_of_nodes() * 50 + files_count * 200,
...
def token_stats(G: nx.MultiDiGraph, files_count: int) -> dict:
    ...
    scoped_payload = [
        {
            ...
            "degree": deg,
            "node": {
                k: v for k, v in G.nodes[n].items() if k in ("type", "file", "label")
            },
            "neighbors": list(list(G.successors(n))[:8]),
        }
        for n, deg in top
    ]
    ...
    return {
        "naive": est_naive_tokens,
        "query": est_query_tokens,
