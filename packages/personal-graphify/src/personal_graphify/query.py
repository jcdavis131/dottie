...
def _tokenize(s: str) -> list[str]:
    return [
        t.lower()
        for t in re.split(r"[^a-z0-9→_]+", s.lower())
        if len(t) >= 2 and t not in STOP_LABELS
    ]
...
def get_ollama_embeddings(
    texts: list[str], model: str = _EMBED_MODEL_DEFAULT
        ...
        if not cpath.exists():
            ...
            try:
                ...
                if Path(graph_path).exists() and Path(graph_path).is_file():
                    gj = json.loads(
                        Path(graph_path).read_text(encoding="utf-8")[:2000000]
                    )
...
def format_cost_dashboard(cost_path: Path) -> str:
    try:
        ...
        lines = [
            f"Cost dashboard — {len(qs)} queries logged (cost.json)",
            f"Total naive tokens: {total_naive} | scoped: {total_scoped} | saved: {total_saved} → {avg_reduction:.1f}x avg reduction",
            "",
        ]
        ...
        for q in qs[-15:][::-1]:
            ...
            lines.append(
                f"  - [{q.get('mode', 'lex')}] [{basis_tag}] {q.get('ts', '')[:16]} | {q.get('question', '')[:60]} → {q.get('scoped')} vs {q.get('naive')} = {q.get('reduction_x')}x saved {q.get('saved')}"
            )
        ...
        measured_saved = sum(
            q.get("saved", 0)
            for q in qs
            if str(q.get("basis", "")).startswith("measured")
        )
        unmeasured = len(qs) - sum(
            1 for q in qs if str(q.get("basis", "")).startswith("measured")
        )
        est_dollars = measured_saved * 0.005 / 1000
        lines.append("")
        lines.append(
            f"Est. API cost avoided (at $5/M): ~${est_dollars:.2f} based on {measured_saved} measured-basis tokens saved"
        )
        if unmeasured:
            lines.append(
                f"  ({unmeasured} queries had estimated/unknown token basis — excluded from $ figure)"
            )
        lines.append(
            f"Storage: free local Ollama — mode: {data.get('mode', 'ollama-first local')}"
        )
        return "\n".join(lines)
...
def search_nodes(
    ...
    query: str,
    limit: int = 30,
    boost_file_type: str = None,
    semantic: bool = False,
    embed_model: str = _EMBED_MODEL_DEFAULT,
    top_lexical_for_rerank: int = 60,
) -> list[dict]:
    qlower = query.lower()
    ...
    if semantic and lexical_pool:
        try:
            ...
            if query_emb_list and cand_emb_list and len(cand_emb_list) == len(texts):
                ...
                reranked = []
                for (score, deg, nid, data, matched), cand_emb in zip(
                    lexical_pool, cand_emb_list
                ):
...
def subgraph_for_query(
    ...
    matches = search_nodes(
        G, query, limit=8, semantic=semantic, embed_model=embed_model
    )
    if not matches:
...
def explain_node(
    ...
    neighbors_out.sort(
        key=lambda x: (
            ...
            -G.degree(x["id"]) if x["id"] in G else 0,
        )
    )
    neighbors_in.sort(
        key=lambda x: (
...
def impact_analysis(
    ...
    for k in impacted:
        impacted[k] = sorted(
            impacted[k],
            key=lambda x: (x["depth"], 0 if x["confidence"] == "EXTRACTED" else 1),
        )[:limit]
    ...
    return {
        ...
        "downstream_files": [
            {"file": f, "count": c} for f, c in downstream_files.most_common(15)
        ],
        "upstream_files": [
            {"file": f, "count": c} for f, c in upstream_files.most_common(15)
        ],
...
def task_compiler(
    G: nx.MultiDiGraph, task_description: str, semantic: bool = False
) -> dict:
    ...
    if not ranked:
        return {
            "error": f"No relevant nodes for task '{task_description}'",
            "suggestion": "Try broader terms: Stripe, Turnover Shield, Family Brain, MTNN, Ava, Scout",
        }
    ...
    files_ranked = [
        {"file": f, "relevance": round(c, 1)} for f, c in files_counter.most_common(15)
    ]
    ...
    if any(w in task_lower for w in ["add", "create", "implement"]):
        plan.append(
            f"1. Read GRAPH_REPORT.md god nodes + {len(files_ranked)} relevant files first"
        )
        plan.append(f'2. Query specific: pgraphify query "{task_description[:60]}"')
        plan.append(
            "3. Edit files in order of relevance (lowest dependency first): check upstream graph"
        )
        ...
        plan.append(
            f'1. Explain root cause: pgraphify explain "{ranked[0].get("label")}"'
        )
        plan.append(
            f'2. Impact check: pgraphify impact "{ranked[0].get("label")}" --direction both'
        )
        ...
        plan.append(
            f'1. Path analysis: pgraphify path "{ranked[0].get("label")}" "{ranked[-1].get("label")}" if 2 concepts'
        )
    ...
    scoped_tokens = payload_tokens(
        {"top_matches": top_matches_payload, "files": files_ranked, "plan": plan}
    )
...
def onboard_report(G: nx.MultiDiGraph, top_n_god: int = 10) -> dict:
    ...
    god = [
        ...
        for nid, deg, d in degs[:top_n_god]
    ]
    ...
    for _, data in G.nodes(data=True):
        ...
        if f:
            file_deg[f] += data.get(
                "degree", G.degree(data.get("id", "")) if "id" in data else 0
            )
    ...
    for nid, data in G.nodes(data=True):
        if data.get("type") == "file":
            ...
            if out_d > 5 and out_d > in_d * 2:
                entry_points.append(
                    {"file": data.get("label"), "out": out_d, "in": in_d}
                )
    ...
    product_nodes = [
        ...
        if d.get("type") in ("product", "ml_concept", "integration")
    ]
...
def format_query_answer(
    ...
    semantic_hint = (
        " [semantic rerank ON]"
        if semantic
        else " [lexical] — try --semantic for Ollama mxbai-embed-large rerank"
    )

    lines = []
    lines.append("Top matches:")
    for m in search_nodes(
        G, query, limit=12, semantic=semantic, embed_model=embed_model
    ):
        extra = f" sem={m.get('semantic_score')}" if "semantic_score" in m else ""
        lines.append(
            f"- {m.get('label')} ({m.get('type')}) in {m.get('file', '')} [comm {m.get('community', 0)}] deg {m.get('degree', 0)} score {m.get('score')}{extra}"
        )
    ...
    for u, v, d in list(sub.edges(data=True))[:20]:
        ...
        lines.append(
            f"  {src_label} --{d.get('type')} [{d.get('confidence')}]--> {tgt_label}"
        )

    rationale_nodes = [
        nid for nid, d in sub.nodes(data=True) if d.get("type") == "rationale"
    ]
    if rationale_nodes:
        lines.append("")
        lines.append(
            f"Rationale found: {len(rationale_nodes)} WHY/NOTE nodes — check explain"
        )
    ...
    if graph_path:
        try:
            log_query_cost(
                graph_path, query, naive, scoped, red, mode=mode_str, basis=basis
            )
        except Exception:
            pass

    header = (
        f"Query: '{query}' — {sub.number_of_nodes()} nodes, {sub.number_of_edges()} edges in scoped subgraph "
        f"(token-est ~{scoped} vs naive ~{naive} = {red}x reduction; {basis}){semantic_hint}"
    )
    return header + "\n\n" + body


def format_path_answer(
    G: nx.MultiDiGraph, src_q: str, tgt_q: str, semantic: bool = False
) -> str:
    ...
    for i in range(len(path) - 1):
        ...
        lines.append(
            f"  {i + 1}. {u_label} --{edata.get('type', '?')} [{edata.get('confidence', '?')}]--> {v_label} (file {G.nodes[v].get('file', '')})"
        )
    return "\n".join(lines)
...
def format_impact_answer(impact: dict) -> str:
    if "error" in impact:
        return impact["error"]
    lines = [
        f"Impact analysis for '{impact['root'].get('label')}' ({impact['root'].get('type')})",
        impact["summary"],
        "",
    ]
    if impact["downstream"]:
        lines.append(
            f"Downstream ({len(impact['downstream'])} nodes, {len(impact['downstream_files'])} files):"
        )
        for d in impact["downstream"][:15]:
            lines.append(
                f"  depth {d['depth']}: {d['label']} ({d['type']}) in {d['file']} --{d['edge_type']} [{d['confidence']}]"
            )
        lines.append("")
    ...
    if impact["upstream"]:
        ...
        for u in impact["upstream"][:15]:
            lines.append(
                f"  depth {u['depth']}: {u['label']} ({u['type']}) in {u['file']}"
            )
    return "\n".join(lines)
...
def format_task_answer(task_result: dict) -> str:
    ...
    lines = [
        f"Task: {task_result['task']}",
        f"Subgraph: {task_result['subgraph']['nodes']} nodes {task_result['subgraph']['edges']} edges | Token est {task_result['token_estimate']['scoped']} vs naive {task_result['token_estimate']['naive']} = {task_result['token_estimate']['reduction_x']}x",
        "",
    ]
    lines.append("Top matches:")
...
def format_onboard_answer(onboard: dict) -> str:
    lines = [
        f"Onboarding — {onboard['meta']['nodes']} nodes {onboard['meta']['edges']} edges {onboard['meta']['communities']} communities",
        "",
    ]
