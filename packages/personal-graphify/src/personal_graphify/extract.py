...
def extract_python(file_path: Path) -> tuple[list[dict], list[dict]]:
    ...
    rationale_re = re.compile(
        r"#\s*(NOTE|WHY|HACK|TODO|FIXME|BUG|OPTIMIZE|PERF):?\s*(.+)", re.I
    )
                ...
                if child.type == "identifier":
                    ...
                    for sub in child.children:
                        if sub.type == "identifier":
                            name = code[sub.start_byte : sub.end_byte].decode(
                                errors="ignore"
                            )
                            results.append(("function", name, node.start_point[0] + 1))
...
JS_FUNC_RE = re.compile(
    r"(?:function\s+(\w+)|const\s+(\w+)\s*=\s*\(.*?\)\s*=>|class\s+(\w+)|export\s+(?:default\s+)?(?:function\s+(\w+)|class\s+(\w+)))"
)
JS_IMPORT_RE = re.compile(
    r"import\s+.*?\s+from\s+['\"]([^'\"]+)['\"]|import\(['\"]([^'\"]+)['\"]\)|require\(['\"]([^'\"]+)['\"]\)"
)
...
def extract_js_generic(file_path: Path) -> tuple[list[dict], list[dict]]:
    ...
    nodes.append(
        {
            ...
            "type": "file",
            "file": str(file_path),
            "language": file_path.suffix.strip("."),
            "full_path": str(file_path),
        }
    )
    ...
    rationale_re = re.compile(
        r"(?:\/\/|#|\/\*+)\s*(NOTE|WHY|HACK|TODO|FIXME|OPTIMIZE):?\s*(.+)", re.I
    )
...
PERSONAL_PATTERNS = {
    ...
    "stripe": (
        ...
        "Stripe",
        "integration",
        "Billing for Turnover Shield $79-149/mo",
    ),
    "plaid": (
        "integration:plaid",
        "Plaid",
        "integration",
        "Bank linking for Davis Family Brain burn $11k EF $66k",
    ),
    "betterment": (
        "integration:betterment",
        "Betterment",
        "integration",
        "Family joint cash + investing buckets",
    ),
    "supabase": (
        "integration:supabase",
        "Supabase",
        "integration",
        "DB for Turnover Shield free-tier",
    ),
    "cloudflare": (
        "integration:cloudflare",
        "Cloudflare Workers",
        "integration",
        "Workers + R2 free hosting",
    ),
    "workers": (
        "integration:cloudflare",
        "Cloudflare Workers",
    ...
    "schwab": (
        ...
        "Schwab",
        "integration",
        "Schwab brokerage holdings (META/VOO)",
    ),
    "usaa": (
        "integration:usaa",
        "USAA",
        "integration",
        "USAA cash hub Classic + savings",
    ),
    # goal linking SOTA — First $1k/mo passive
    "first $1k": (
        "concept:first-1k-mrr",
        "First $1k/mo passive goal",
        "business_metric",
        "$1k MRR sustained, 7-13 customers @ $79-149/mo",
    ),
    "1k/mo passive": (
        "concept:first-1k-mrr",
        "First $1k/mo passive goal",
    ...
    "revenue tracker": (
        "concept:revenue-tracker",
        "Turnover Shield Revenue Tracker",
        "business_metric",
        "MRR tracker via crons weekly Friday trials/paid/MRR/churn%",
    ),
    "mrr tracker": (
        "concept:revenue-tracker",
        "Turnover Shield Revenue Tracker",
        "business_metric",
        "Weekly MRR check Friday 9am",
    ),
    "passive income": (
        "concept:passive-income",
        "Passive Income Web App",
        "product",
        "Self-sustaining web app for passive income — Turnover Shield Pick",
    ),
    "trade crew": (
        "concept:trade-crew",
        "Trade Crew (Plumbing/Electrical/HVAC)",
        "business_metric",
        "Boring B2B vertical for Turnover Shield",
    ),
    "goal_76da7701": (
        "concept:first-1k-mrr",
        "First $1k/mo passive goal",
        "business_metric",
        "Goal ID goal_76da7701a682 — pricing $79-149/mo",
    ),
    # products
    "turnover shield": (
        "concept:turnover-shield",
        "Turnover Shield",
        "product",
        "Boring B2B SaaS $79-149/mo churn prediction for trade crews, aiming $1k MRR = 7-13 customers",
    ),
    "turnover": (
        "concept:turnover-shield",
        "Turnover Shield",
        "product",
        "Churn prediction for trade crews",
    ),
    "retention playbook": (
        "concept:retention-playbook",
        "Retention Playbook",
        "product_feature",
        "Actionable steps to keep tech when churn risk high",
    ),
    "churn prediction": (
        "concept:churn-prediction",
        "Churn Prediction",
        "ml_feature",
        "Predict which plumber/electrician tech will leave",
    ),
    "family brain": (
        "concept:family-brain",
        "Davis Family Brain",
        "product",
        "V9 Household OS client-only localStorage + Plaid $11k burn tracking",
    ),
    "davis family brain": (
        "concept:family-brain",
        "Davis Family Brain",
    ...
    "mrr": (
        ...
        "MRR / Paid Users",
        "business_metric",
        "Target $1k MRR, pricing $79-149, ROI saving 1 tech = $5k hiring cost",
    ),
    ...
    "burn rate": (
        ...
        "Burn Rate $11k/mo",
        "business_metric",
        "$11k joint burn, 21mo runway, EF 206% $136k",
    ),
    # ml concepts
    "mtNN": (
        "concept:mtnn",
        "MTNN v5_concat_b2_h160_t32_d48_mlp128",
        "ml_concept",
        "Multi-Task Neural Net 120 feats 17 families cat([x·m,m]) masking 544+12→128→48 L2",
    ),
    "mtnn": (
        "concept:mtnn",
        "MTNN v5_concat_b2_h160_t32_d48_mlp128",
        "ml_concept",
        "MTNN architecture 12,966 seasons vector-hoops",
    ),
    "mtNN v4": (
        "concept:mtnn",
        "MTNN v5_concat_b2_h160_t32_d48_mlp128",
    ...
    "ava": (
        ...
        "ml_concept",
        "Real-mode Jacobian 4 J-Spaces S1 Fast hl8 S2 Slow hl300 Critic hl30 Planner hl150 Router/veto local Docker CUDA",
    ),
    "j-space": (
        "concept:jspace",
        "Ava J-Space Multi (S1 hl8 S2 hl300 Critic hl30 Planner hl150)",
        "ml_concept",
        "4 workspaces multi-space Jacobian",
    ),
    ...
    "vector hoops": (
        ...
        "product",
        "Daily NBA chimera 12,966 player-seasons PCA 3 8 archetypes MTNN CQS 85.87",
    ),
    "vector pitch": (
        "concept:vector-pitch",
        "Vector Pitch 633 WC tournaments",
        "product",
        "World Cup chimera 633 player-tournaments StatsBomb per-90",
    ),
    "vector gridiron": (
        "concept:vector-gridiron",
        "Vector Gridiron Fantasy",
        "product",
        "Fantasy cockpit MAE 4.268 R²0.39",
    ),
    "dumbmodel": (
        "concept:dumbmodel",
        "dumbmodel.com ecosystem",
        "product",
        "hoops/pitch/gridiron + jcamd.com hub Vercel static",
    ),
    "dinov3": (
        "concept:tennis-dinov3",
        "Tennis DINOv3 ExecuTorch",
        "product",
        "Serve coach DINOv3 + ConvNeXt-Tiny distilled XNNPACK",
    ),
    "tennis": (
        "concept:tennis-dinov3",
        "Tennis DINOv3",
        "product",
        "Digital line judge baseline",
    ),
    "graphify": (
        "concept:graphify",
        "Personal Graphify",
        "tool",
        "Ollama-first local graphify fork with task/impact/onboard",
    ),
    "ollama": (
        "concept:ollama",
        "Ollama qwen3:32b local",
        "tool",
        "Local LLM judge for frontier rubric 11-cat",
    ),
    "workforce embedding": (
        "concept:workforce-embedding",
        "Workforce Embedding 120d + 4 heads",
        "ml_feature",
        "120 days of turnover signals + 4 tower heads from Passive Lab research",
    ),
    "120d + 4 heads": (
        "concept:workforce-embedding",
        "Workforce Embedding 120d + 4 heads",
        "ml_feature",
        "Trade crew embedding",
    ),
    ...
    "scout cli": (
        ...
        "Scout CLI",
        "tool",
        "Personal control plane (ex-BigBang): Ava brain + authentic writing + lab MRR + RTX offload",
    ),
    "scout-cli": (
        "concept:scout",
        "Scout CLI",
        "tool",
        "Personal control plane agent-native security-first",
    ),
    "scout": (
        "concept:scout",
        "Scout CLI",
        "tool",
        "HOME-only personal control plane wiring Ava + lab + RTX",
    ),
    "bigbang": (
        "concept:scout",
        "Scout CLI (ex-BigBang)",
        "tool",
        "Renamed personal control plane",
    ),
    "rtx offload": (
        "concept:rtx-offload",
        "RTX Offload",
        "tool",
        "Local GPU offload path used by Scout/Ava",
    ),
    "jcamd.com": (
        "concept:jcamd",
        "jcamd.com hub",
        "product",
        "Workforce intelligence consulting + Lab + public Personal Graphify",
    ),
}
...
def extract_personal_patterns(file_path: Path) -> tuple[list[dict], list[dict]]:
    ...
    file_id = (
        f"doc:{file_path}"
        if file_path.suffix.lower() in _DOC_NODE_EXTS
    ...
    if "01_Finance" in parts:
        ...
        if not any(n["id"] == nid for n in nodes):
            nodes.append(
                {
                    "id": nid,
                    "label": "01_Finance - Davis Family Brain & FIRE $50",
                    "type": "ecosystem_domain",
                    "desc": "Family finance $2.25M vested + $11k burn FIRE 50",
                }
            )
        edges.append(
            {
    ...
    if "02_Passive_Lab" in parts:
        ...
        if not any(n["id"] == nid for n in nodes):
            nodes.append(
                {
                    "id": nid,
                    "label": "02_Passive_Lab - Turnover Shield + 10 boring B2B",
                    "type": "ecosystem_domain",
                    "desc": "Top 10 boring SaaS $79-149/mo, aiming first $1k/mo",
                }
            )
        edges.append(
            {
    ...
    if "04_Tennis_DINOv3" in parts or "tennis" in fp_str.lower():
        ...
        nodes.append(
            {
                "id": nid,
                "label": "04_Tennis_DINOv3 ExecuTorch",
                "type": "ecosystem_domain",
            }
        )
        edges.append(
    ...
    if (
        "first-1k-mo-passive" in fp_lower
        or "first_1k" in fp_lower
        or "goal_76da7701" in fp_lower
    ):
        ...
        if not any(n["id"] == nid for n in nodes):
            nodes.append(
                {
                    "id": nid,
                    "label": "Goal: First $1k/mo passive — Turnover Shield",
                    "type": "ecosystem_domain",
                    "desc": "Outcome $1k MRR sustained, 7-13 customers @ $79-149/mo, pricing vs competitors, weekly tracking trials/paid/MRR/churn",
                }
            )
        edges.append(
            {
        ...
        edges.append(
            {
                "source": file_id,
                "target": "concept:turnover-shield",
                "type": "tracks",
                "confidence": INFERRED,
            }
        )
        ...
        edges.append(
            {
                "source": file_id,
                "target": "concept:revenue-tracker",
                "type": "implements",
                "confidence": INFERRED,
            }
        )
    if (
        "build-a-self-sustaining-web-app-for-passive-income" in fp_lower
        or "passive-income" in fp_lower
    ):
        ...
        if not any(n["id"] == nid2 for n in nodes):
            nodes.append(
                {
                    "id": nid2,
                    "label": "Goal: Build self-sustaining web app for passive income",
                    "type": "ecosystem_domain",
                    "desc": "Start small grow long-term, deep market research niche, no ongoing input",
                }
            )
        edges.append(
            {
    ...
    try:
        ...
        if "plaid" in low and ("burn" in low or "family brain" in low):
            edges.append(
                {
                    "source": "integration:plaid",
                    "target": "concept:family-brain",
                    "type": "feeds",
                    "confidence": INFERRED,
                }
            )
            ...
            edges.append(
                {
                    ...
                    "target": "concept:churn-prediction",
                    "type": "contains",
                    "confidence": INFERRED,
                }
            )
        if "churn" in low and "retention" in low:
            edges.append(
                {
                    "source": "concept:churn-prediction",
                    "target": "concept:retention-playbook",
                    "type": "triggers",
                    "confidence": INFERRED,
                }
            )
        if (
            ...
            or "first 1k" in low
            or "1k/mo" in low
            or "revenue tracker" in low
            or "goal_76da7701" in low
        ):
            edges.append(
                {
                    "source": "concept:turnover-shield",
            ...
            edges.append(
                {
                    "source": "concept:revenue-tracker",
                    "target": "concept:first-1k-mrr",
                    "type": "tracks",
                    "confidence": INFERRED,
                }
        ...
        if "48→64" in src or "48->64" in src or "hl=" in low or "mtNN" in src:
            nodes.append(
                {
                    "id": "concept:mtnn-head",
                    "label": "MTNN head 48→64→k tower arch",
                    "type": "ml_concept",
                    "desc": "48→64→k from Vector Hoops truthful MTNN",
                }
            )
            edges.append(
                {
                    "source": file_id,
                    "target": "concept:mtnn-head",
                    "type": "uses",
                    "confidence": INFERRED,
                }
            )
        ...
        if (
            "ava" in low
            and ("j-space" in low or "jspace" in low or "planner" in low)
            and "critic" in low
        ):
            edges.append(
                {
                    "source": "concept:planner",
                    "target": "concept:critic",
                    "type": "interacts_with",
                    "confidence": INFERRED,
                }
            )
...
def extract_with_cache(
    files: list[Path], cache_path: Path, update: bool = False
