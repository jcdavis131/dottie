...
def build_graph(nodes: List[Dict], edges: List[Dict]) -> nx.MultiDiGraph:
    ...
    for e in edges:
        ...
        filtered = {k:v for k,v in e.items() if k not in ("source","target","type","confidence")}
        G.add_edge(src, tgt, type=e.get("type","references"), confidence=e.get("confidence","INFERRED"), **filtered)
    return G
