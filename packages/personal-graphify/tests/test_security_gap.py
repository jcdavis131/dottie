...
try:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("security_gap", "/home/hatch/workspace/dottie/packages/personal-graphify/src/personal_graphify/security.py")
...
def test_security_parametrize_case(case, expected, sample_harness_data, tmp_path):
    ...
    assert False, f"TODO: fill assert for packages/personal-graphify/src/personal_graphify/security.py case={case} expected={expected} dottie coverage <80% - module personal_graphify.security"
...
def test_security_mode_coverage(mode, sample_harness_data):
    ...
    assert False, f"TODO: packages/personal-graphify/src/personal_graphify/security.py mode={mode} not covered - dottie coverage <80%"
...
def test_security_tmp_path_integration(tmp_path, sample_harness_data):
    ...
    assert False, f"TODO: assert packages/personal-graphify/src/personal_graphify/security.py writes correct output to {out} - main PR repo dottie gap"
...
def test_security_state_store_or_registry_contract(sample_harness_data):
    ...
    assert False, f"TODO: contract for packages/personal-graphify/src/personal_graphify/security.py - verify HOME isolation, no work leak, free-tier - main PR repo"
