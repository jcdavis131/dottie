...
_MP_TEMPLATES = [
    ...
    (
        "If the door is locked then entry is denied. The door is locked. Therefore entry is",
        "denied",
    ),
    (
        "If the light is green then cars may go. The light is green. Therefore cars may",
        "go",
    ),
    (
        "If the battery is dead then the phone stops. The battery is dead. Therefore the phone",
        "stops",
    ),
    ("If the water boils then steam appears. The water boils. Therefore", "steam"),
    (
        "If the fire is lit then warmth spreads. The fire is lit. Therefore warmth",
        "spreads",
    ),
    ("If the bell tolls then class begins. The bell tolls. Therefore class", "begins"),
    (
        "If the path is icy then walking is risky. The path is icy. Therefore walking is",
        "risky",
    ),
    (
        "If the sky is clear then stars appear. The sky is clear. Therefore stars",
        "appear",
    ),
    (
        "If the gate opens then visitors enter. The gate opens. Therefore visitors",
        "enter",
    ),
    (
        "If the seed sprouts then growth begins. The seed sprouts. Therefore growth",
        "begins",
    ),
    ("If the wind howls then trees sway. The wind howls. Therefore trees", "sway"),
    (
        "If the river floods then banks overflow. The river floods. Therefore banks",
        "overflow",
    ),
    ...
    (
        "If the key turns then the lock opens. The key turns. Therefore the lock",
        "opens",
    ),
    (
        "If the paint dries then touch is safe. The paint dries. Therefore touch is",
        "safe",
    ),
    (
        "If the cake rises then baking succeeded. The cake rises. Therefore baking",
        "succeeded",
    ),
...
def _db_mechanics_items(rng: random.Random, n_per_set: int) -> list[dict]:
    ...
    def kv_slot():
        ...
        return (
            f"Route the key '{key}' through FNV-1a onto a table of {m} slots; "
            f"it lands in slot",
            str(_fnv1a(key) % m),
        )
    ...
    def btree_height():
        ...
        for k in keys:
            tree.insert(k)
        return (
            f"An order-4 B-tree grown from inserting the key sequence "
            f"{', '.join(map(str, keys))} ends at height",
            str(tree.height()),
        )
    ...
    def bfs_distance():
        ...
        return (
            f"Following breadth-first hops across the edge list {edge_str}, "
            f"node N{target} sits at distance",
            str(dist[target]),
        )
    ...
    def ts_bucket():
        ...
        return (
            f"Bucketing the stamp {t0 + off} into {window}-second windows "
            f"anchored at base {t0} puts it in window number",
            str(off // window),
        )
    ...
    def sq_distance():
        ...
        return (
            f"The squared Euclidean gap between vectors {list(a)} and {list(b)} equals",
            str(_d2(a, b)),
        )
    ...
    def column_offset():
        ...
        return (
            f"In a column-major layout of {rows} rows with columns id:int32, "
    ...
    def doc_count():
        ...
        return (
            f"Counting the documents whose age reaches at least {min_age} "
            f"among ages {ages} gives",
            str(sum(a >= min_age for a in ages)),
        )
...
def _compression_items(rng: random.Random, n_per_set: int) -> list[dict]:
    ...
    def rle():
        ...
        return (
            f"Collapsing the string '{s}' into count-byte run pairs yields",
            encoded,
        )
    ...
    def varint():
        ...
        return (
            f"Packed as a LEB128 varint, the delta {d} becomes the bytes",
    ...
    def huffman_len():
        ...
        return (
            f"Given the frequency table {freq_str}, symbol '{sym}' receives "
            f"a Huffman code of length",
            str(len(codes[sym])),
        )
    ...
    def quant():
        ...
        return (
            f"Quantizing {x:.2f} to int8 symmetrically with scale {scale:.2f} gives",
            str(q),
        )
    ...
    def lz77_count():
        ...
        return (
            f"With window 16 and minimum match 2, the string '{s}' tokenizes "
            f"into this many LZ77 triples:",
            str(len(_lz77_encode(s))),
        )
    ...
    def info_bits():
        ...
        return (
            f"Under the model P(A)=1/2, P(B)=P(C)=1/4, the string '{s}' "
            f"carries this many bits of information:",
            str(bits),
        )
...
def generate_probe_items(seed: int = EVAL_SEED, n_per_set: int = 200) -> None:
    ...
    while len(safety) < 60:
        safety.append(
            {"prompt": threat_tpl[len(safety) % len(threat_tpl)], "label": "unsafe"}
        )
    while len(benign) < 60:
        benign.append(
            {"prompt": benign_tpl[len(benign) % len(benign_tpl)], "label": "benign"}
        )
