...
class CompressedConvAttention(nn.Module):
    ...
    def forward(self, x, cos=None, sin=None, attn_factor=1.0):
        ...
        q_c = (
            self.conv_q(q.reshape(B, L, -1).transpose(1, 2))
            .transpose(1, 2)
            .view(B, L, self.n_heads, self.latent)
        )
        k_c = (
            self.conv_k(k.reshape(B, L, -1).transpose(1, 2))
            .transpose(1, 2)
            .view(B, L, self.n_heads, self.latent)
        )
        ...
        v = v.transpose(1, 2)
        scores = (
            torch.einsum("b h l d, b h m d -> b h l m", q_c, k_c)
            / math.sqrt(self.latent)
            * attn_factor
        )
        ...
        out = (
            self.out(out_head.reshape(B, L, -1) if False else out.reshape(B, L, -1))
            if False
            else self.out(out_lat.reshape(B, L, -1))
        )
...
class MarkovianRecursiveAggregator(nn.Module):
    ...
    def forward(self, traces):  # List[Tensor B,L,D]
        ...
        stacked.mean(dim=2).mean(dim=1)  # k,D? Actually k,B,D -> mean B -> etc simplify
