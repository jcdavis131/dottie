...
# --- Mock Router/Critic heads (match ava-skills/memory-router and safety-scanner) ---
...
def export_to_pte(
    model: Any, example_inputs: Any, out_path: pathlib.Path, backend: str = "xnnpack"
) -> dict[str, Any]:
    torch = _lazy_torch()
    executorch_mod, _exir_mod = _lazy_executorch()
    ...
    if executorch_mod is None:
        ...
        try:
            ...
            return {
                ...
                "note": "executorch not installed, saved pt2",
            }
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Export Router/Critic to ExecuTorch .pte XNNPACK"
    )
    ap.add_argument("--out-dir", default="ava/mobile/exports")
    ap.add_argument(
        "--backend", default="xnnpack", choices=["xnnpack", "coreml", "vulkan", "auto"]
    )
    ap.add_argument("--input-dim", type=int, default=128)
    ap.add_argument(
        "--quantize", action="store_true", help="8-bit dynamic quantization"
    )
    args = ap.parse_args()
    ...
    manifest = {
        ...
        "xnnpack": {
            ...
            "quantization": "8-bit dynamic" if args.quantize else "fp32",
        },
