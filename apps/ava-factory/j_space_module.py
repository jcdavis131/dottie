...
class JacobianLens(nn.Module):
    def __init__(self, d_model=2048, vocab_size=128000):
        super().__init__()
        self.verbalizer = nn.Linear(
            d_model, vocab_size, bias=False
        )  # W [V][D] approximates d(logit)/d(hidden)
...
class GlobalWorkspace(nn.Module):
    ...
    def forward(self, fused):
        ...
        return fused + broadcast * 0.2, {
            "workspace": ws,
            "broadcast": broadcast,
            "broadcast_strength": b_strength,
            "verbalizable_mass": v_mass,
            "top_concepts_idx": None,
