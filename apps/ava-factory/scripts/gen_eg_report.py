...
for cat in WEIGHTS:
    ...
    eg_flops = (
        baseline_cost_needed / candidate_point[cat]["cost"]
        if candidate_point[cat]["cost"] > 0
        else 0
    )
...
print(
    f"Wrote {out_path} weighted EG FLOPs {weighted_eg_flops:.3f} Time {weighted_eg_time:.3f}"
)
