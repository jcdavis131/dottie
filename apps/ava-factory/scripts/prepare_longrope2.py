...
def main():
    ...
    parser.add_argument(
        "--scales", type=str, default="1,2,4,8,16,32,100", help="comma scales"
    )
    ...
    print(
        f"Generating LongRoPE2 factors dim={args.dim} base={args.base} shift={args.critical_shift}"
    )
    ...
    for scale in scales:
        inv, lam, crit, crit_t = longrope2_factors(
            args.dim, args.base, scale, args.critical_shift, args.sharpness
        )
        ...
        print(
            f"scale={scale:>6} crit={crit:5.2f} crit_t={crit_t:.3f} lam min {lam.min():.2f} max {lam.max():.2f} mean {lam.mean():.2f} attn {attn_factor:.3f} mscale {mscale:.3f}"
        )
    ...
    torch.save(
        {k: torch.tensor(v["inv_freq"]) for k, v in out["scales"].items()}, pt_out
    )
