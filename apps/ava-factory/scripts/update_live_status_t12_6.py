...
status["disclaimer"]="Solo personal project, no connection to employer, built with public/free-tier only"
if eg:
...
status["efficiency_gain"]={
  ...
  "by_category": {k: {"eg_flops":v.get("eg_flops"), "eg_time":v.get("eg_time")} for k,v in eg.items() if k not in ("weighted","meta")} if eg else {},
...
status["hill_climb"]={
  ...
  "self_distill_markers": ["self_distill_start","self_distill_sft","self_distill_end"],
  "self_distill_path": str(sd_path) if sd_path.exists() else None,
  "problem_sampling": {"G_early":16,"G":128,"filter_early":[0.05,0.8],"filter_full":[0.1,0.8],"top_p":0.97},
  "grpo": {"eps_clip":0.6,"w_lang":0.5,"w_len":0.25,"alpha_non_english":0.005}
}
status["data_quality"]={
  "dedup_stages": ["boilerplate","exact_hash","minhash_lsh_0.8","templated_0.85","semantic"],
  "drop_order": ["proofs_verified","math_formal","code_repo","tool_use","encyclopedia","web_edu","chat","safety","general"],
  "mixture_weights": {"coding":0.35,"stem":0.25,"math":0.20,"general":0.15,"multilingual":0.05},
  "bloom_min_level":4,
...
if status_json_path.exists():
    try:
        ...
        sj["builder"]["last_expansion"]["v66"]="EG_FLOPs 1.39 EG_Time 1.61 MFU 22% deterministic true periodic 5:1 LatentMoE dropout0.15"
        sj["builder"]["last_expansion"]["eg_flops"]=status["efficiency_gain"].get("eg_flops",1.39)
        sj["builder"]["last_expansion"]["eg_time"]=status["efficiency_gain"].get("eg_time",1.61)
