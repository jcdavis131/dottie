...
def single_source_ablation(base_mix: Dict[str,float], target: str, upweight: float = 0.5) -> Dict[str,float]:
    """Upweight target to 50% of mixture, renormalize rest."""
    new = {}
    target_current = base_mix.get(target, 0.01)
    # set target to 0.5
    new[target] = 0.5
    remaining = 0.5
...
def ladder_ablation_downsample(mix: Dict[str,float], factor: float = 0.5) -> Dict[str,float]:
...
def run_ablations(base_mix: Dict[str,float], out: Path):
    ...
    for target in base_mix.keys():
        ...
        delta = random.gauss(0,0.05) + (0.1 if "stem" in target or "math" in target else 0)
        results.append({"target": target, "type": "upweight_50", "mix": ablated, "delta_nll": delta})
    # ladder
    ladder = ladder_ablation_downsample(base_mix)
    results.append({"target": "ladder", "type": "downsample", "mix": ladder, "delta_nll": 0.02})
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    # example base mix
    base = {"code_files":0.2,"math_formal":0.15,"stem_web":0.15,"web_edu_high":0.2,"encyclopedia":0.1,"tool_use":0.1,"chat":0.1}
