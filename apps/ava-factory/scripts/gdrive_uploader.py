    ...
    for attempt in range(retries):
        try:
            ...
            if result.returncode == 0:
                ...
                print(
                    f"[GDrive] CLI failed attempt {attempt + 1}/{retries}: {result.stderr[:500]}"
                )
...
def check_work_drive_guard():
    ...
    if not out:
        ...
        return (
            False,
            "Could not list files to determine drive type — proceed with caution",
        )
    ...
    try:
        ...
        for f in files:
            ...
            owners = f.get("owners", [])
            # work indicators
            if "health.json" in name and (
                "lockedunn" in name or "marcelo" in name or "health" in name
            ):
                work_indicators.append(
                    f"{name} suggests work drive (health.json pattern)"
                )
            for owner in owners:
                email = owner.get("emailAddress", "")
                if email.endswith("@meta.com") or email.endswith("@fb.com"):
                    work_indicators.append(
                        f"Owner {email} is Meta — work drive detected (file {name})"
                    )

        if work_indicators:
            msg = (
                "WORK DRIVE DETECTED — Home/Work separation violation risk. Indicators:\n"
            ...
            return (
                False,
                f"No work indicators found in {len(files)} files — appears to be personal Drive or empty, safe to proceed",
            )
    except Exception as e:
...
def get_or_create_folder(folder_name):
            ...
            if files:
                ...
                for f in files:
                    if f["id"] == canonical_preferred:
                        print(
                            f"[GDrive] Found preferred canonical folder {folder_name} id {canonical_preferred} (43 files) among {len(files)} duplicates - using this"
                        )
                ...
                print(
                    f"[GDrive] Found existing folder {folder_name} id {fid} (oldest of {len(files)} duplicates, newest would be {files_sorted[-1]['id']}) - reusing oldest to stop duplication"
                )
                if len(files) > 3:
                    print(
                        f"[GDrive] WARNING: {len(files)} duplicate folders named {folder_name} exist from previous bug - should cleanup empty ones, keeping {fid} as canonical. Best known canonical with files is {canonical_preferred}"
                    )
                return fid
        except Exception as e:
    ...
    # Fallback: use root
    print("[GDrive] Could not get/create folder, using root")
    return "root"
...
def upload_file_with_dedup(local_path: Path, folder_id, dry_run=False):
    ...
    if out:
        try:
            ...
            if data.get("files"):
                print(
                    f"[GDrive] Skip {local_path.name} — already exists with sha {sha12}: {data['files'][0]['name']}"
                )
                return {
                    "skipped": True,
                    "sha12": sha12,
                    "existing_id": data["files"][0]["id"],
                }
        except:
            pass
    ...
    if dry_run:
        print(
            f"[Dry-run] Would upload {local_path} (sha12 {sha12}, {local_path.stat().st_size / 1e6:.1f}MB) to folder {folder_id}"
        )
    ...
    json_payload = json.dumps(
        {
            "name": f"{local_path.stem}_{sha12}{local_path.suffix}",
            "parents": [folder_id],
        }
    )
    ...
    if out:
        try:
            ...
            print(
                f"[GDrive] Upload output (non-JSON) for {local_path.name}: {out[:500]}"
            )
            if "id" in out.lower():
...
def upload_folder(
    local_folder_or_pattern, remote_folder_name, dry_run=False, workers=2
):
    ...
    if isinstance(local_folder_or_pattern, str) and "*" in local_folder_or_pattern:
        ...
        if local_path.is_dir():
            files = (
                ...
                + list(local_path.rglob("*.gz"))
                + list(local_path.rglob("*.json"))
            )
            # deduplicate
    ...
    if is_work:
        ...
        fallback = (
            repo_root
            / "data/for_upload"
            / f"BLOCKED_WORK_DRIVE_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        print(
            f"[GDrive] Saved fallback marker {fallback} — DO NOT upload to work Drive, connect personal Drive jcdavis131@gmail.com or use R2"
        )
    ...
    if dry_run:
        ...
        print(
            f"[GDrive Dry-run] Would upload {len(files)} files to {remote_folder_name} ({folder_id})"
        )
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(upload_file_with_dedup, f, folder_id, False): f
            for f in files
        }
    ...
    print(
        f"[GDrive] Batch done: uploaded {uploaded}, skipped (dedup) {skipped}, failed {failed} / total {len(files)} to folder {remote_folder_name} id {folder_id}"
    )
    return {
        "uploaded": uploaded,
        "skipped": skipped,
...
def main():
    ...
    ap.add_argument(
        "--upload",
        type=str,
        help="local file or folder or glob, e.g., data/daily_expanded/",
    )
    ap.add_argument(
        "--folder", default="Ava-Datasets-Expansion", help="remote folder name"
    )
    ap.add_argument(
        "--dry-run", action="store_true", help="don't actually upload, just check dedup"
    )
    ...
    res = upload_folder(
        args.upload, args.folder, dry_run=args.dry_run, workers=args.workers
    )
