...
def test_nano_v66():
    ...
    pattern_ok = (
        cfg.model.use_periodic_attention and cfg.model.periodic_pattern == "5:1"
    )
    print(
        f"periodic attention: {cfg.model.use_periodic_attention} pattern {cfg.model.periodic_pattern} ok={pattern_ok}"
    )
    ...
    matches = sum(1 for a, b in zip(globals_list, expected, strict=False) if a == b)
    print(
        f"periodic pattern matches {matches}/{len(globals_list)} globals {globals_list[:12]}"
    )
    ...
    print(
        f"Final max violation {max_violation_overall:.3f} should be <2.0 for healthy dropless"
    )
    ...
    result = {
        ...
        and zero_init_ok
        and dropout_ok
        and max_violation_overall < 2.5,
    }
    Path(REPO / "reports").mkdir(exist_ok=True)
    Path(REPO / "reports" / "t12_2_nano_quick.json").write_text(
