        ...
        shards.append(
            {
                "bin": str(bpath),
                "idx": str(ipath),
                "tokens": int(idx["tokens"]),
                "docs": len(chunk),
                "bytes": bpath.stat().st_size,
            }
        )
    return {
        "tokenizer_sha": lt.sha256,
        "eod_id": lt.eod_id,
...
def stage_register(pack_info: dict, vocab: int, needed_tokens: int) -> dict:
    ...
    return {
        ...
        "shard_copies_registered": copies,
        "entries_registered": registered,
        "entries_packed": completed,
        "tokens_ready_phase0": ready,
...
def run_train(
    *,
    ...
    if not final_ckpt.exists():
        raise StageError(
            f"training finished but final checkpoint missing: {final_ckpt}"
        )
    return {
        "cmd": cmd,
        ...
        "final_ckpt_bytes": final_ckpt.stat().st_size,
        **metrics,
    }
...
def parse_metrics(path: Path) -> dict:
    ...
    for line in path.read_text().splitlines():
        ...
        if ev == "step":
            ...
            branch_forked = {
                k: rec.get(k) for k in ("branch", "init", "step", "frozen", "trainable")
            }
    if not steps:
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--branch-steps", type=int, default=25, help="branch fine-tune steps"
    )
    ap.add_argument(
        "--corpus-mb",
        type=float,
    ...
    manifest: dict = {
        ...
        "note": (
            "nano (~14M param) CPU smoke run proving every pipeline stage "
    ...
    try:
        ...
        print(
            f"[register] {reg['entries_packed']} PACKED entries, "
        ...
        print(
            f"[pretrain] {args.steps} steps in {base['wall_seconds']}s; "
        ...
        print(
            f"[branch] {args.branch_steps} steps in {branch['wall_seconds']}s; "
