...
# ~10 high-level bins
HIGH_LEVEL_BINS = ["coding", "stem", "math", "pdfs", "general_web", "encyclopedia", "tool_use", "chat", "safety", "multilingual"]
...
def random_mix() -> Dict[str, float]:
    ...
    s = sum(weights)
    return {k: v/s for k, v in zip(HIGH_LEVEL_BINS, weights)}
...
def local_search(fixed_high: Dict[str, float], target_bin: str, n_variants: int = 5) -> List[Dict[str, float]]:
    ...
    for _ in range(n_variants):
        ...
        sub_weights = {sub: (rv/s)*bin_total for sub, rv in zip(subs, r)}
        # build full mix
        full = {}
...
def global_search(fixed_relative: Dict[str, Dict[str, float]], n_variants: int = 8) -> List[Dict[str, float]]:
...
def cap_epochs(mix: Dict[str, float], dataset_sizes: Dict[str, int], token_budget: int, max_repeat: int = 8) -> Dict[str, float]:
...
def nll_for_mix_placeholder(mix: Dict[str, float], eval_target: str = "physics") -> float:
    ...
    stem_w = sum(mix.get(k,0) for k in ["stem_web","stem_pdfs","stem_wiki","pdf_academic","pdf_technical"])
...
def search(n_rounds: int = 3, eval_target: str = "physics") -> Dict:
    ...
    fixed_relative: Dict[str, Dict[str, float]] = {bin_name: {sub:1/len(SUB_SOURCES.get(bin_name,[bin_name])) for sub in SUB_SOURCES.get(bin_name,[bin_name])} for bin_name in HIGH_LEVEL_BINS}
    best = None
    best_nll = 999
    history = []
    for rnd in range(n_rounds):
        ...
        for lv in l_variants:
            ...
            history.append({"round": rnd, "type": "local", "bin": target_bin, "mix": lv, "nll": nll})
            if nll < best_nll:
                best_nll = nll
                best = lv
        ...
        if best:
            ...
            for bin_name in HIGH_LEVEL_BINS:
                ...
                if total>0:
                    fixed_relative[bin_name] = {sub: best.get(sub,0)/total for sub in subs}
            high_mix = {bin_name: sum(best.get(sub,0) for sub in SUB_SOURCES.get(bin_name,[bin_name])) for bin_name in HIGH_LEVEL_BINS}

    return {"best_mix": best, "best_nll": best_nll, "history": history, "eval_target": eval_target}
