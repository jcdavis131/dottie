...
def main():
    ...
    if entries:
        try:
            total_tokens = sum(
                e.get("tokens", 0) if isinstance(e, dict) else 0 for e in entries
            )
        except:
            total_tokens = 0
    print(
        f"[HF Uploader] {DISCLAIMER} Repo: {args.repo} entries={len(entries)} tokens={total_tokens}"
    )

    hf_token = os.environ.get("HF_TOKEN") or os.environ.get("HUGGINGFACE_HUB_TOKEN")
    if not hf_token:
        print(
            "WARNING: HF_TOKEN not set — saving locally for Alienware push (expected in Hatch VM)"
        )
        Path("data/for_upload").mkdir(parents=True, exist_ok=True)
        out = Path(
            f"data/for_upload/hf_ready_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.json"
        )
        out.write_text(
            json.dumps(
                {
    ...
    if hf_token and args.push:
        try:
            ...
            if entries:
                ...
                if len(ds_val) > 0:
                    ds_val.push_to_hub(
                        args.repo, private=args.private, split="validation"
                    )
                if len(ds_test) > 0:
                    ds_test.push_to_hub(args.repo, private=args.private, split="test")
                print("Push complete")
            else:
