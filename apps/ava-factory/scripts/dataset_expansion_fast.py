...
def gen_example_fast(topic, phase, nonce):
    ...
    reasoning = "\n".join(
        [
            f"Reasoning step {i}: analysis of {topic} under {phase}, consider formal semantics and proof obligations, check invariants, cross-reference with known results."
...
def main():
    ...
    ap.add_argument(
        "--phases",
        nargs="+",
        default=["p0_logic", "p1_math", "p2_foundation", "p3_code"],
    )
    ...
    print(
        f"[{DISCLAIMER}] Fast expansion target {args.tokens}={target} phases {args.phases}"
    )
    ...
    def new_shard():
        ...
        path = (
            out_root
            / f"packed_{timestamp_str}_{shard_idx:05d}_{random.randint(1000, 9999)}.jsonl.gz"
        )
    ...
    while total_tokens < target:
        ...
        if attempts > target * 3:  # safety, shouldn't happen
            print("[Fast] Too many attempts, breaking")
            break
        for phase in args.phases:
            ...
            ok, _reason = quality_filter_fast(ex["text"])
            if not ok:
                filtered_qual += 1
                continue
            ...
            if cur_bytes > args.shard_mb * 1024 * 1024:
                ...
                print(
                    f"[Fast] Shard full {cur_path.name} {cur_bytes / 1e6:.1f}MB tokens {total_tokens}/{target} docs {total_docs} dup {filtered_dup} qual {filtered_qual} elapsed {time.time() - start_t:.1f}s"
                )
                batch_entries = []
                fh, cur_path, cur_bytes = new_shard()
        ...
        if total_docs % 500 == 0:
            print(
                f"[Fast] Progress {total_tokens}/{target} ({100 * total_tokens / target:.1f}%) docs {total_docs} dup {filtered_dup} qual {filtered_qual} elapsed {time.time() - start_t:.1f}s"
            )
    ...
    print(
        f"[Fast] Done: {total_tokens} tokens, {total_docs} docs, {len(new_shards)} shards, dup {filtered_dup}, qual {filtered_qual}, elapsed {elapsed:.1f}s"
    )
    ...
    upload_manifest = {
        ...
        "split_counts": {
            "train": train_docs,
            "validation": val_docs,
            "test": total_docs - train_docs - val_docs,
        },
        "phases": args.phases,
