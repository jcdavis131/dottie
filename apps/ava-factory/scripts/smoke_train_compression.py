...
def main():
    ...
    print(
        "[solo disclaimer] Solo personal project, no connection to employer, built with public/free-tier only"
    )
    cfg = DottieConfig.load(args.preset)
    print(
        f"[cfg] preset={cfg.preset} d_model={cfg.model.d_model} layers={cfg.model.n_layers} vocab orig={cfg.model.vocab_size}"
    )
    ...
    print(
        f"[flags] moe={new_mcfg.use_moe} rel={new_mcfg.use_relative} short_conv={new_mcfg.use_short_conv} effort={new_mcfg.use_effort} rope_type={new_mcfg.rope_type}"
    )
    ...
    print(
        f"[model] built {n_params / 1e6:.2f}M params, inkling flags gated, byte-identical default when flags False"
    )
    model.train()
    ...
    if tokenizer is None:
        print(
            "[warn] no tokenizer, using char-level fallback with vocab 256 (will still verify forward)"
        )
        vocab_size = 8192
    ...
    print(
        f"[data] compression={len(comp_docs)} docs avg_len={sum(len(d['text']) for d in comp_docs) // max(1, len(comp_docs))} "
    ...
    print(
        f"[compression families] {fam_counts} (expected shannon 25%, huffman 20%, lz77 20%, arithmetic 15%, bwt 10%, z_token 10%)"
    )

    # Verify byte-determinism
    comp_docs2 = gen_texts(CompressionGenerator, target_bytes // 2, seed=1234)
    sha1 = hashlib.sha256(
        "".join(d["text"][:100] for d in comp_docs).encode()
    ).hexdigest()[:12]
    sha2 = hashlib.sha256(
        "".join(d["text"][:100] for d in comp_docs2).encode()
    ).hexdigest()[:12]
    print(
        f"[determinism] sha {sha1} vs {sha2} match={sha1 == sha2} (byte-deterministic)"
    )

    # Verify verifiers inside compression (entropy, kraft, lz)
    # sample checks
    from dottie.datagen.compression import (
        entropy_bits,
        kraft_sum,
        lz77_compress,
        lz77_decompress,
    ...
    print(
        f"[verify] entropy_bits([0.5,0.5])={entropy_bits([0.5, 0.5]):.3f} expected 1.0"
    )
    print(
        f"[verify] kraft_sum([1,2,3])={kraft_sum([1, 2, 3]):.3f} <=1? {kraft_sum([1, 2, 3]) <= 1}"
    )
    samp = "ABABABAB"
    tuples = lz77_compress(samp)
    decomp = lz77_decompress(tuples)
    print(
        f"[verify] LZ77 {samp!r} -> {tuples[:2]}... -> {decomp!r} ok={decomp == samp}"
    )
    ...
    for step in range(args.steps):
        ...
        input_ids = (
            torch.tensor(batch_ids[:-1], dtype=torch.long).unsqueeze(0).to(device)
        )  # [1, T]
        target_ids = (
            torch.tensor(batch_ids[1:], dtype=torch.long).unsqueeze(0).to(device)
        )
        ...
        if step % 5 == 0:
            print(
                f"[train] step {step}/{args.steps} loss={loss.item():.4f} incl compression {len([d for d in all_docs if 'compression' in d['source']])} samples"
            )

    print(
        f"[done] final loss after {args.steps} steps: {losses[-1]:.4f} avg last 5={sum(losses[-5:]) / 5:.4f}"
    )
    ...
    try:
        ...
        for ph in range(6):
            ...
            print(
                f"[collector phase {ph}] total_weight={total:.2f} compression={comp_w:.2f} ({comp_w / total * 100:.1f}% if total>0) sources={len(phases_weights[ph])}"
            )
    except Exception as e:
