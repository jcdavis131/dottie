...
def gen_textbook_example(topic, phase):
    ...
    text += "\n\n" + " ".join(
        [
            f"Reasoning step {i}: analysis of {topic} with formal logic."
...
def main():
    ...
    ap.add_argument(
        "--tokens", default="10M", help="tokens to generate this run, e.g., 10M, 100M"
    )
    ap.add_argument(
        "--phases", nargs="+", default=["p0_logic", "p1_math"], help="phases to expand"
    )
    ap.add_argument("--out", default="data/daily_expanded", help="output root")
    ap.add_argument(
        "--upload-mode",
        default="local",
        choices=["local", "gdrive", "r2"],
        help="upload mode",
    )
    ap.add_argument(
        "--dry-run", action="store_true", help="don't write large files, just simulate"
    )
    ap.add_argument(
        "--dedup-threshold", type=int, default=3, help="simhash hamming threshold"
    )
    ...
    print(f"[{DISCLAIMER}]")
    print(
        f"[Expansion] Target {args.tokens} = {target_tokens} tokens, phases {args.phases}, out {args.out}, mode {args.upload_mode}, dry-run={args.dry_run}"
    )
    ...
    daily_manifest = (
        out_root / f"manifest_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.jsonl"
    )
    new_shards = []
    ...
    if args.dry_run:
        print(
            f"[Dry-run] Would generate ~{target_tokens // 400} docs (avg 400 tokens/doc)"
        )
        ...
        print(
            f"[Dry-run] Sample: generated {len(sample_docs)} docs, filtered dup {filtered_dup} qual {filtered_qual}"
        )
        ...
        print(
            f"[Dry-run] Wrote sample manifest {daily_manifest} ({len(sample_docs)} entries)"
        )
        # also write for_upload marker
        (
            for_upload_root / f"sample_{datetime.now().strftime('%Y%m%d')}.json"
        ).write_text(
            json.dumps(
                {
                    "mode": "dry-run",
                    ...
                    "tokens": sum(len(d["text"]) // 4 for d in sample_docs),
                },
                indent=2,
            )
        )
        print("[Dry-run] HOME/Work separation check:")
        print("  - Would NOT upload to work Drive camd@meta.com")
    ...
    def new_shard():
        ...
        path = (
            out_root
            / f"packed_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}_{current_shard_idx:05d}_{random.randint(1000, 9999)}.jsonl.gz"
        )
    ...
    try:
        while total_tokens < target_tokens and attempts < max_attempts + 1000:
            ...
            for phase in args.phases:
                ...
                # quality filter
                ok, _reason = quality_filter(ex["text"])
                if not ok:
                    filtered_qual += 1
                    continue
                ...
                if dedup.is_duplicate(ex["text"]):
                    ...
                    if filtered_dup > 100 and filtered_dup > total_docs * 3:
                        print("[Expansion] Dedup too strict, resetting filter window")
                ...
                if cur_bytes > args.shard_mb * 1024 * 1024:
                            ...
                            try:
                                with open(manifest_path, "a") as gmf:
                                    gmf.write(
                                        json.dumps(
                                            {
                                                "path": str(out_root / entry["file"]),
                                                "sha256": entry["sha256"],
                                                "sha256_full": entry["sha256_full"],
                    ...
                    print(
                        f"[Expansion] Shard full {cur_path} {cur_bytes / 1e6:.1f}MB, total tokens {total_tokens}/{target_tokens}, docs {total_docs}"
                    )
                    batch_buffer = []
                    current_shard_idx += 1
                    fh, cur_path, cur_bytes = new_shard()
            ...
            if total_tokens % 50000 < 1000:
                print(
                    f"[Expansion] Progress {total_tokens}/{target_tokens} tokens, docs {total_docs}, dup filtered {filtered_dup}, qual filtered {filtered_qual}"
                )
    ...
    print(
        f"[Expansion] Done: {total_tokens} tokens, {total_docs} docs, shards {new_shards}, dup filtered {filtered_dup}, qual {filtered_qual}"
    )
    ...
    if args.upload_mode == "local":
        ...
        upload_manifest = (
            for_upload_root
            / f"upload_manifest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        print(
            f"[Expansion] Saved locally to {for_upload_root}, manifest {upload_manifest}"
        )
        print(
            f"  -> For Alienware: rsync -avz {out_root}/ your-alienware:~/ava-agi-factory-v6-4/data/daily_expanded/"
        )
        ...
        if is_work:
            print(f"[BLOCKED] {msg}")
            print(
                f"[Expansion] Aborting GDrive upload, saving to {for_upload_root} instead."
            )
            # fallback to local
            upload_manifest = (
                for_upload_root
                / f"BLOCKED_WORK_DRIVE_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            upload_manifest.write_text(
                json.dumps(
                    {
        ...
        endpoint = os.environ.get("CLOUDFLARE_R2_ENDPOINT") or os.environ.get(
            "R2_ENDPOINT"
        )
        bucket = os.environ.get("CLOUDFLARE_R2_BUCKET", "ava-datasets")
        if not ak or not sk:
            print(
                f"[Expansion] R2 credentials missing, saving to {for_upload_root} instead. Set CLOUDFLARE_R2_ACCESS_KEY, SECRET_KEY, ENDPOINT"
            )
            upload_manifest = (
                for_upload_root
                / f"R2_MISSING_CREDS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            upload_manifest.write_text(
                json.dumps(
                    {
        ...
        try:
            ...
            # upload manifest
            s3.upload_file(
                str(daily_manifest), bucket, f"expansion/{daily_manifest.name}"
            )
            print(
                f"[R2] Done uploaded {len(new_shards)} shards + manifest to s3://{bucket}/expansion/"
            )
        except Exception as e:
