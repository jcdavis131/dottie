...
print(
    f"[cfg] {cfg.preset} d_model={cfg.model.d_model} n_layers={cfg.model.n_layers} vocab={cfg.model.vocab_size}"
)
...
cfg_moe = dataclasses.replace(
    cfg,
    model=dataclasses.replace(
        cfg.model, use_moe=True, moe_n_routed_experts=32, moe_top_k=2
    ),
)
...
try:
    ...
    def encode(s):
        return [ord(c) % 8192 for c in s][:256]
...
for doc in gen.generate(200_000):
    ...
    if len(docs) >= 50:
        break
print(
    f"[compression] generated {len(docs)} docs avg_len {sum(len(d['text']) for d in docs) // len(docs)} byte-det deterministic"
)
# Check sha
docs2 = []
...
print(
    f"[verify] lz {s} -> {tups} -> {lz77_decompress(tups)} ok={lz77_decompress(tups) == s}"
)
...
for ph in range(6):
    ...
    print(
        f"[collector phase {ph}] total {total:.2f} comp {comp} {comp / total * 100:.1f}%"
    )
...
encoded = [
    torch.tensor(encode(d["text"]), dtype=torch.long)
    for d in docs
    if len(encode(d["text"])) > 10
]
losses = []
for step in range(20):
    ...
    logits = (
        out.get("lm_logits") if out.get("lm_logits") is not None else out.get("logits")
    )
    if logits is None:
...
print(
    f"[done] loss start {losses[0]:.4f} end {losses[-1]:.4f} avg last5 {sum(losses[-5:]) / 5:.4f}"
)
