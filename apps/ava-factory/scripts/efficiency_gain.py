...
def main():
    ...
    if not args.baseline or not args.candidate:
        ...
        data = {
            ...
            "cost": args.cost,
            "fit": {"A": 1.0, "alpha": 0.2, "E": 1.0},
            "eg": {"flops": 1.0, "time": 1.0},
        }
    ...
    for cat, points in baseline_per_cat.items():
        if len(points) < 3:
            continue
        costs, losses = zip(*sorted(points), strict=False)
        costs = np.array(costs)
        losses = np.array(losses)
        fitted = fit_law(costs, losses)
        # candidate best loss
        cand_points = (
            candidate_per_cat.get(cat) or candidate_per_cat.get("weighted") or []
        )
        if not cand_points:
            # take global last
        ...
        if not cand_points:
            continue
        _cand_cost, cand_loss = sorted(cand_points)[
            -1
        ]  # use highest cost entry as final
        ...
        report["per_category"][cat] = {
            "fit": fitted,
            "candidate_cost": cand_cost_at_min,
            "candidate_loss": cand_loss,
            "baseline_cost_for_same_loss": inv_cost,
            "eg": eg,
            "n_baseline_points": len(points),
        }
        if cat in WEIGHTS:
    ...
    if egs:
        ...
        all_base = [p for lst in baseline_per_cat.values() for p in lst] or [
            (1e12, 3.0),
            (2e12, 2.7),
            (4e12, 2.5),
        ]
        costs, losses = zip(*sorted(all_base), strict=False)
