...
def wsd_lr(step: int, total_steps: int, cfg: DottieConfig) -> float:
    ...
    return w.lr_min + 0.5 * (w.lr_max - w.lr_min) * (
        1 + math.cos(math.pi * min(1.0, frac))
    )
...
def gpu_stats() -> dict:
    ...
    try:
        ...
        stats = {
            "gpu_power_w": float(p),
            "gpu_mem_mb": int(float(mu)),
            "gpu_mem_total_mb": int(float(mt)),
            "gpu_sm_mhz": int(float(clk)),
            "gpu_temp_c": int(float(tc)),
...
def apply_phase(model, cfg: DottieConfig, phase: int, log) -> PhaseConfig:
    ...
    log(
        ...
        ntk=p.ntk,
        j_weight=cfg.jspace.j_weight_for_phase(phase),
    )
    return p
...
def main(argv=None) -> int:
    ...
    with (
        ...
        StreamingShardSampler(
            cfg, manifest, flow, seed=args.seed, packed_dir=args.packed
        ) as sampler,
    ):
        lm_hist: list[float] = []
        ...
        def heartbeat(step: int, phase: int, status: str = "running") -> None:
            ...
            try:
                manifest.upsert_run(
                    run_id, preset=args.preset, step=step, phase=phase, status=status
                )
            ...
            try:
                ...
                if step <= 1 or step % max(1, cfg.training.metrics_every_steps) == 0:
                    log(
                        ...
                        effort={
                            str(k): v for k, v in snap.effort_map().items() if v > 0
                        },
                    )
            except Exception as exc:
        ...
        if args.max_steps is not None:
            ...
            total_steps = max(
                1, int(branch_spec["tokens"]) // cfg.training.tokens_per_step
            )
        else:
        ...
        if step >= total_steps:
            ...
            log(
                ...
                final=str(ckpt_dir / f"{args.branch or 'base'}_final.pt"),
            )
            mfile.close()
            return 0
        phase = phase_for_step(cfg, tokens_done)
        ...
        while step < total_steps:
            ...
            try:
                for _ in range(accum):
                    ...
                    last_j = {
                        ...
                        "route_probs": [
                            round(x, 4) for x in mj["route_probs"].mean(0).tolist()
                        ],
                    }
            ...
            if step % cfg.training.metrics_every_steps == 0 or step == 1:
                ...
                log(
                    ...
                    tok_s=round(
                        step_tokens * cfg.training.metrics_every_steps / max(dt, 1e-6)
                    )
                    if step > 1
                    else None,
                    ...
                    **{k: float(f"{v:.4g}") for k, v in agg.items()},
                    hl_est={
                        s: round(getattr(model.multi_jspace, s).hl_est(), 2)
