...
class _Planet:
    def __init__(
        self, name: str, orbit_au: float, radius_e: float, moons: int, lum: float
    ):
        self.name = name
        ...
        self.radius_e = round(
            radius_e if self.kind == "rocky planet" else radius_e * 6.0, 1
        )
...
class WikiGenerator(Generator):
    ...
    def _system(self) -> tuple[str, str, float, list[_Planet]]:
        ...
        for i, a in enumerate(orbits):
            ...
            planets.append(
                _Planet(pname, a, rng.uniform(0.3, 1.8), rng.randint(0, 3), lum)
            )
    ...
    def _star_page(self, star, cls, lum, planets) -> str:
        rows = [
            f"# {star}",
            "",
            f"Class: {cls}. Luminosity: {lum} L_sun. Known planets: {len(planets)}.",
            "",
        ...
        rows += [f"- {_link(p.name)} — {p.kind}, {p.orbit_au} AU" for p in planets]
    ...
    def _concept_page(self, star, planets, kind) -> str | None:
        ...
        rows += [
            f"- {_link(p.name)} ({p.orbit_au} AU, {p.moons} moons)" for p in members
        ]
        return "\n".join(rows)
    ...
    def _pointing_doc(self, star, planets, atlas) -> str:
        ...
        for p in (planets[0], planets[-1]):
            page_idx, row_px, width_px, line_text = self._locate_orbit_line(pages, p)
            blocks.append(
    ...
    def generate(self, target_bytes: int) -> Iterator[dict]:
        ...
        while produced < target_bytes:
            ...
            pages.append(
                (
                    star,
                    f"class-{cls} star, {len(planets)} planets",
            ...
            for kind in ("rocky planet", "gas giant"):
                ...
                if cp:
                    pages.append(
                        (f"{kind.capitalize()}s of {star}", f"{kind}s overview", cp)
                    )
            ...
            if len(volume) == 3:
                ...
                d = self.doc(
                    text=vol,
                    task_type="automatic",
                    concept=f"{volume_star.lower()} volume",
                    phase=4,
                    source=self.name,
                )
                produced += len(vol)
