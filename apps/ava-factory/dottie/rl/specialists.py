...
SPECIALISTS = [
    SpecialistConfig(
        name="stem_reasoning",
        prompt_mix={"math": 0.585, "physics": 0.132, "chem": 0.109, "other": 0.174},
        reward_type="mcq_proof_open",
        steps=1000,
    ),
    SpecialistConfig(
        name="agentic_code",
        prompt_mix={"code": 0.6, "tool_use": 0.3, "temporal": 0.1},
        reward_type="tool_success",
        steps=1000,
    ),
    SpecialistConfig(
        name="helpfulness_safety",
        prompt_mix={"chat": 0.5, "safety": 0.3, "delegation": 0.2},
        reward_type="safety_score",
        steps=800,
    ),
]
...
def consolidate(specialist_ckpts: list[str], out: Path):
    ...
    data = {
        "specialists": specialist_ckpts,
        "method": "trace_distillation_sft",
        "timestamp": time.time(),
    }
