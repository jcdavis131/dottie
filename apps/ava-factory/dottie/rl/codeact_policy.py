...
`DottieTokenizer` satisfies it; its `decode(ids, skip_special=...)` keyword is used when available
...
class TorchModelPolicy:
    ...
    Args:
        ...
        eos_id: token id that hard-stops decoding (cut, not included). Default: sniffed from
            ...
            (`DottieTokenizer` exposes neither attribute — pass ``ava.tokenizer.EOS_ID`` explicitly.)
        ...
        *,
        ...
        if model is None:
            raise ValueError(
                ...
                "ava.model.build_model(DottieConfig.load(...)) and load real weights; do not stub "
        ...
        if tokenizer is None:
            raise ValueError(
                ...
                "encode(str)->list[int] and decode(list[int])->str (DottieTokenizer qualifies)."
            )
        if max_new_tokens < 1:
