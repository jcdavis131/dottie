...
class LLMVMKernel:
    ...
    async def exec_cell(self, code: str, timeout: float = 30.0) -> ExecutionResult:
        ...
        try:
            ...
            # compile main body
            compiled = compile(
                ast.Module(tree.body, []), f"<cell-{self._execution_count}>", "exec"
            )
            ...
            with (
                ...
                if last_expr:
                    ...
                    expr_code = compile(
                        ast.Expression(last_expr.value), "<expr>", "eval"
                    )
            ...
            output = (
                stdout_buf.getvalue() + stderr_buf.getvalue() + traceback.format_exc()
            )
            elapsed = int((time.time() - start) * 1000)
            exec_result = ExecutionResult(
                success=False,
    ...
    def register_tool(self, func: Callable, override: bool = False):
        ...
        if not override and name in self.globals:
            raise ValueError(
                f"Tool {name} already exists, set override=True to replace"
            )
