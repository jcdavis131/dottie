...
class SelfModifyManager:
    ...
    def override_tool(
        self, name: str, new_func: Callable, reason: str = ""
    ) -> AuditEntry:
        """Override existing runtime function — e.g., add caching layer"""
        if self.require_gate and not self.gate_enabled:
            raise PermissionError(
                "Self-modification requires ENABLE_LLMVM_WRITE=1 gate. Set env or call enable_write(). Audit mode only."
            )
        ...
        entry = AuditEntry(
            ...
            action="override",
            tool_name=name,
            code=new_func.__code__.co_code.hex()[:200]
            if hasattr(new_func, "__code__")
            else str(new_func)[:200],
            prev_code=prev_code,
            reason=reason,
        )
    ...
    def cached_wrap(
        self, func: Callable, cache_key_fn: Callable | None = None
    ) -> Callable:
        """
        ...
        self.override_tool(
            func.__name__,
            wrapper,
            reason="Add caching layer — self-modification emergent",
        )
        return wrapper
