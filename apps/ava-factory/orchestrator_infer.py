"""Orchestrator router inference — numpy + stdlib ONLY (no torch, ever).

This module is the deploy-side half of the orchestrator model pair:

  * ``orchestrator_model.py`` (same directory) trains the network with torch and
    exports ``champion_weights.json`` via ``save_weights``;
  * THIS file loads that JSON and runs the forward pass in pure numpy float64.

It is vendored into a serverless API package and file-path-imported by the
scout CLI's learned router, both of which run in images without torch. A test
(``tests/test_orchestrator_model.py``) enforces that no line here imports
torch. Keep it that way.

The featurize + forward math below is a FROZEN cross-lane contract (schema_version 1).
Any change here must be mirrored byte-for-byte in every consumer and in the
torch training side.
"""
...
def _gelu_tanh(u: np.ndarray) -> np.ndarray:
    """Tanh-approximation GELU — matches torch nn.GELU(approximate='tanh')."""
...
def predict(w: dict, goal_text: str, dense: dict | None = None) -> dict:
    """Convenience wrapper: derive default dense features from the goal itself.

    Caller-supplied ``dense`` entries override the derived defaults. The
    derived chain-signal expression is the exact expression of
    apps/scout-cli/bigbang/plugins/harness/cli.py:64.
    """
    ...
    out["model_version"] = w["model_version"]
    out["gate_passed"] = w["gate_passed"]
    return out
