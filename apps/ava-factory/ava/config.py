...
`AvaModel1B.freeze_spaces()` and the blueprint's BRANCH_CONFIGS.
"""
...
_CONFIG_DIR = Path(os.environ.get("AVA_CONFIG_DIR", _REPO_ROOT / "configs"))
...
class AvaConfig:
    ...
    @classmethod
    def load(cls, preset: str, config_dir: str | Path | None = None) -> "AvaConfig":
    ...
    @classmethod
    def from_dict(cls, raw: Mapping[str, Any]) -> "AvaConfig":
        try:
...
def m_slots(cfg: AvaConfig) -> Sequence[int]:
    return [cfg.jspace.slots[s] for s in SPACES]


def load(preset: str, config_dir: str | Path | None = None) -> AvaConfig:
    return AvaConfig.load(preset, config_dir)
