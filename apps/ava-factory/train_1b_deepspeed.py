    ...
    def merge(self):
        ...
        for key in first_sd.keys():
            ...
            try:
                tensors = [
                    sd[key].float() * w
                    for (_, sd), w in zip(self.buffer, weights, strict=False)
                ]
...
def _try_run_openwiki_and_harness(mode="mock", ckpt=None):
    ...
    try:
        ...
        for cand in [
            here.parent / "dottie-skills",
            here / ".." / "dottie-skills",
            Path.home() / "workspace" / "dottie-skills",
        ]:
        ...
        try:
            ...
            print(
                f"[openwiki-sync] Ingested {stats['n_files']} wiki files avg mass {stats['avg_mass']:.3f} — maps to S2 hl300"
            )
            if stats["has_france_for_generalization_test"]:
                print(
                    "[openwiki-sync] France→China probe ready (capital/language/continent/currency)"
                )
            # if model/ckpt given, attempt injection is handled by caller; here just log
        except Exception as e:
            print(
                f"[openwiki-sync] Adapter fallback (expected if no ~/.openwiki/wiki yet): {e}"
            )
            ...
            try:
                ...
                res = run_skill(
                    "openwiki-sync", mode=mode, ckpt=ckpt or "dottie_stable_736k.pt"
                )
        ...
        try:
            # add harness path
            for cand in [
                here.parent / "dottie-open-harness",
                here / ".." / "dottie-open-harness",
                Path.home() / "workspace" / "dottie-open-harness",
            ]:
            ...
            eval_names = (
                "jspace_all,frontier_rubric,openwiki_knowledge"
                if mode == "mock"
                else "jspace_all,frontier_rubric"
            )
            # In real mode we still want openwiki_knowledge if wiki exists
            if mode != "mock":
                eval_names = "jspace_all,frontier_rubric,openwiki_knowledge"
            results = run_harness(
                eval_names=eval_names, mode=mode, ckpt=ckpt, preset="nano"
            )
            passed = results.get("meta", {}).get("passed", 0)
            total = results.get("meta", {}).get("total", 0)
            print(
                f"[HARNESS GATE] {passed}/{total} passed — wall {results.get('meta', {}).get('wall_s', 0):.1f}s"
            )
            ...
            if passed < 3 and mode != "mock":
                print(
                    f"[HARNESS GATE] WARNING: only {passed} passed, expected >=3 — branching may be blocked"
                )
            return results
        except Exception as e:
            print(
                f"[HARNESS GATE] runner not available or failed (ok for local mock without harness installed): {e}"
            )
            return None
    except Exception as e:
...
ROPE_SCHEDULE = [
    ...
    },
    {
        ...
        "ctx": 16384,
        "ntk": 1.2,
        "desc": "420k-480k: 100k (16k)+NTK1.2",
    },
    {
        "start": 480000,
        "end": 660000,
        ...
        "ctx": 32768,
        "ntk": 1.5,
        "desc": "480k-660k: 500k (32k)+NTK1.5",
    },
    {
        "start": 660000,
        "end": 800000,
        ...
        "yarn": True,
        "ntk": 2.0,
        "desc": "660k-800k: 1M (64k/128k)+YaRN 2.0-4.0",
    },
]
BRANCH_CONFIGS = {
    "base": {
    ...
    "code": {
        "freeze": ["system1"],
        "finetune": ["system2", "planner", "router", "arbitration"],
        "router_bias": [0.25, 0.45, 0.05, 0.25],
        "router_frozen": ["system1"],
        "target_hl": {"system1": 8, "system2": 350, "critic": 30, "planner": 200},
        "data": "code_repo 50% + code_long_32k 20% + jobbench_code 15% + general 15%",
        "lr": 1e-4,
    },
    "math": {
        "freeze": ["system1", "planner"],
        "finetune": ["system2", "critic", "router"],
        "router_bias": [0.10, 0.65, 0.20, 0.05],
        "target_hl": {"system1": 8, "system2": 400, "critic": 40, "planner": 150},
        "data": "math_formal_lean 35% + lean_mathlib 20% + proofpile2 20% + synthetic_math_r1 15% + general 10%",
        "lr": 8e-5,
    },
    "chat": {
        ...
        "target_hl": {"system1": 8, "system2": 300, "critic": 35, "planner": 180},
        "data": "chat_alignment 30% + safety_blackmail_leverage 20% + jobbench_delegation_human_will 25% + gaia2_temporal_deadlines 15% + counterfactual_reflection 10%",
        "lr": 5e-5,
    },
}
...
def compute_capacity_curve():
    ...
    combined = [0.6 * b + 0.4 * a for a, b in zip(s1, s2, strict=False)]
    return ks, s1, s2, combined


def main():
    parser = argparse.ArgumentParser(
        description="Dottie AGI Factory v6.4 — WSD 736k branching YaRN 10k->1M Multi-J-Space WSM+OroJaR"
    )
    parser.add_argument(
        "--branch", default="base", choices=["base", "code", "math", "chat", "all"]
    )
    ...
    parser.add_argument(
        ...
        default="wsd",
        choices=["wsd", "wsm"],
        help="LR schedule: wsd (original 736k 92%%) or wsm decay-free merging",
    )
    parser.add_argument(
        "--orojar",
        type=float,
        default=0.0,
        help="OroJaR weight lambda for Jacobian disentangle loss",
    )
    parser.add_argument(
        "--wsm_merge_every", type=int, default=10000, help="WSM merge frequency steps"
    )
    parser.add_argument(
        "--wsm_buffer", type=int, default=5, help="WSM buffer size k last checkpoints"
    )
    ...
    parser.add_argument(
        ...
        help="root of sharded jsonl streaming data",
    )
    parser.add_argument(
        "--streaming",
        action="store_true",
        default=True,
        help="use DottieStreamingDataset constant-memory streamer",
    )
    parser.add_argument(
        "--no-streaming",
        dest="streaming",
        action="store_false",
        help="disable streaming, use old mock",
    )
    parser.add_argument(
        "--shuffle_buffer",
        type=int,
        default=10000,
        help="fixed shuffle buffer size — memory cap",
    )
    parser.add_argument(
        "--seq_len",
        type=int,
        default=2048,
        help="sequence length per sample (2048 early, 131072 later per RoPE schedule)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="yarn",
        choices=["yarn", "longrope2"],
        help="RoPE type: yarn baseline 10k->1M vs longrope2 non-uniform per-dim 31->25 + resonance",
    )
    parser.add_argument(
        "--n_sinks", type=int, default=4, help="attention sinks count (4)"
    )
    parser.add_argument(
        "--use_peri_ln",
        action="store_true",
        default=True,
        help="enable Peri-LN output-LN",
    )
    parser.add_argument(
        "--no-peri-ln", dest="use_peri_ln", action="store_false", help="disable Peri-LN"
    )
    parser.add_argument(
        "--critical_shift",
        type=int,
        default=6,
        help="LongRoPE2 critical dim shift 31->25",
    )
    parser.add_argument(
        "--max_steps",
        type=int,
        default=10,
        help="demo steps when mock — real trainer loops infinite stream",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="MOJO SSL weight λ from 2607.14086v1 — 0.3 sweet spot",
    )
    parser.add_argument(
        "--mojo_mask_ratio",
        type=float,
    ...
    print(
        f"WSD warmup {WSD_CONFIG['warmup']} stable {WSD_CONFIG['stable_steps']} 92% total {WSD_CONFIG['total_steps']} lr {WSD_CONFIG['lr_max']}→{WSD_CONFIG['lr_min']}"
    )
    print(
        f"Dataflow: streaming={args.streaming} root={args.data_root} shuffle_buffer={args.shuffle_buffer} seq_len={args.seq_len} batch={args.batch_size} branch={args.branch}"
    )
    print(
        f"Memory guarantee: 1 file handle per source + shuffle_buffer {args.shuffle_buffer} examples + 1 batch — no full corpus in RAM"
    )

    try:
        import torch
        from model_1b import apply_rope_scaling, get_model
    ...
    try:
        from streaming_data import (
            DottieStreamingDataset,
            SyntheticShardGenerator,
        )

        HAS_STREAMING = True
        print(
            "[Streaming] DottieStreamingDataset loaded — constant-memory multi-source weighted stream ready"
        )
    except Exception as e:
        HAS_STREAMING = False
    ...
    branches = (
        ["base", "code", "math", "chat"] if args.branch == "all" else [args.branch]
    )
    for branch in branches:
        ...
        if HAS_STREAMING:
            ...
            if dummy_needed:
                print(
                    f"[Streaming] No shards found in {args.data_root}, seeding 3 dummy sources for local test (constant memory)"
                )
                    ...
                    if not dummy.exists():
                        ...
                        with open(dummy, "w") as f:
                            for i in range(200):
                                _json.dump(
                                    {
                                        "text": f"Dummy {src} example {i} streaming test phase {branch} "
                                        + ("reasoning step. " * 50),
                                        "source": src,
                                    },
                                    f,
        ...
        if args.mock or not HAS_TORCH:
            if HAS_STREAMING and args.streaming:
                print(
                    f"[MOCK STREAMING] Using DottieStreamingDataset branch={branch} shuffle_buffer={args.shuffle_buffer} — never loads full corpus"
                )
                ...
                steps = 0
                for batch in ds.batched(
                    seq_len=args.seq_len, batch_size=args.batch_size
                ):
                    print(
                        f"[MOCK STREAM BATCH] step={steps} phase={batch['phase']} task_types={batch['task_type']} sources={batch['source']} tokens_seen={ds.tokens_seen}"
                    )
                    # illustrate per-space routing
                    for tt in batch["task_type"]:
                        if tt == "automatic":
                            print(
                                "  → S1 auto: DCLM top15% copying sentiment Spanish fluent — broadcast 0.18 hl8 w0.6"
                            )
                        elif tt == "deliberate":
                            print(
                                "  → S2 deliberate: logic/math/JobBench/Karpathy — broadcast 0.22 vm 0.065 hl300 w0.8"
                            )
                        elif tt == "safety":
                            print(
                                "  → Critic safety: leverage/blackmail/threat/fake — vm 0.08 hl30 w1.0"
                            )
                        elif tt == "temporal":
                            print(
                                "  → Planner temporal: GAIA2 dynamic async — broadcast 0.20 hl150 w0.7"
                            )
                    steps += 1
                ...
                if branch == "base":
                    Path("dottie_stable_736k.pt").write_text(
                        "mock stable 736k 13.8T streaming"
                    )
                    Path("dottie_stable_736k_rope1000000_ctx131072.pt").write_text(
                        "mock stable rope 1M ctx131k streaming"
                    )
                    _try_run_openwiki_and_harness(
                        mode="mock", ckpt="dottie_stable_736k.pt"
                    )
                Path(f"dottie_{branch}_final_800k.pt").write_text(
                    f"mock final {branch} 800k streaming"
                )
                print(
                    f"[MOCK STREAM] Branch {branch} done tokens_seen={ds.tokens_seen} stats={ds.stats()}"
                )
                os.system(
                    f"python3 eval_branch_harness.py --branch {branch} --mode mock"
                )
                continue
            else:
                print(
                    "[MOCK] S1 on automatic DCLM top15% copying sentiment Spanish fluent — broadcast target 0.18 hl8 weight 0.6"
                )
                print(
                    "[MOCK] S2 on deliberate logic/math/reasoning JobBench messy Karpathy AutoResearch — broadcast 0.22 vm 0.065 hl300 weight 0.8"
                )
                print(
                    "[MOCK] Critic on safety leverage/blackmail/scandal threat/survival/shutdown fake/fictional — vm 0.08 hl30 weight 1.0"
                )
                print(
                    "[MOCK] Planner on temporal GAIA2 dynamic async delegation_priority env_delta — broadcast 0.20 hl150 weight 0.7"
                )
                print(
                    "[MOCK] Inter-space: inter_mi MSE(cos(S1mean,S2mean),0.45) w0.3 routing KL w0.4 automatic [0.6,0.15,0.1,0.15] deliberate [0.15,0.55,0.1,0.2] safety [0.1,0.2,0.6,0.1] temporal [0.1,0.3,0.1,0.5]"
                )
                print(
                    "[MOCK] 4 base losses: lm + (report*1.0 + broadcast*0.5 + selectivity*0.3 + modulation*0.5)*j_weight 0.08 early 0.15 reasoning/long"
                )
                ks, s1, s2, comb = compute_capacity_curve()
                print(
                    f"[MOCK W&B] capacity_curve ks={ks} combined knee 9 — S1 knee 6 exp(-0.12*max(0,k-6)) S2 knee 10 exp(-0.08*max(0,k-10))"
                )
                print(
                    "[MOCK W&B] half_life curves: S1 hl=8 decay exp(-ln2*t/hl) S2 hl=300 etc every 50 steps log S1_hl_est vs target"
                )
                if branch == "base":
                    Path("dottie_stable_736k.pt").write_text("mock stable 736k 13.8T")
                    Path("dottie_stable_736k_rope1000000_ctx131072.pt").write_text(
                        "mock stable rope 1M ctx131k"
                    )
        ...
        try:
            ...
            model = get_model(
                rope_type=args.rope, n_sinks=args.n_sinks, use_peri_ln=args.use_peri_ln
            )
        print(
        ...
        if args.schedule == "wsm":
            wsm_merger = WSMCheckpointMerger(
                buffer_size=args.wsm_buffer,
                ema_decay=args.wsm_ema,
                merge_every=args.wsm_merge_every,
                save_dir=WSM_CONFIG["save_dir"],
            )
            print(
                f"[WSM] Decay-free mode: stable_lr {WSM_CONFIG['stable_lr']} merge_every {args.wsm_merge_every} buffer {args.wsm_buffer} ema {args.wsm_ema} — infinite continuation support"
            )

        if branch != "base" and Path("dottie_stable_736k.pt").exists():
            print(
                f"Loading stable checkpoint dottie_stable_736k.pt for {branch} — freeze {bcfg['freeze']}"
            )
        ...
        if HAS_STREAMING and args.streaming:
            print(
                f"[Real STREAMING] Building infinite low-memory stream for branch {branch} — seq_len auto from RoPE schedule schedule={args.schedule} orojar={args.orojar}"
            )
            ds = DottieStreamingDataset(
                data_root=args.data_root,
                branch=branch,
            ...
            for batch in ds.batched(seq_len=args.seq_len, batch_size=args.batch_size):
                ...
                apply_rope_scaling(
                    model,
                    rope["base"],
                    rope["base"] // 10000 if rope.get("yarn") else rope["base"] / 10000,
                )
                ...
                input_ids = (
                    ...
                    else torch.tensor(batch["input_ids"], dtype=torch.long)
                )
                task_types = batch["task_type"]
                from collections import Counter

                maj_task = (
                    Counter(task_types).most_common(1)[0][0]
                    if task_types
                    else "deliberate"
                )
                ...
                if args.orojar > 0 and fused is not None:
                    try:
                        ws_dict = (
                            jspace.get("workspaces")
                            if isinstance(jspace, dict)
                            else None
                        )
                        if ws_dict is not None:
                            orojar_loss_val, jacobian_metrics = (
                                jlosses.orojar_comprehensive_loss(
                                    ws_dict,
                                    fused,
                                    jspace.get("route_probs"),
                                    target_cos=0.45,
                                )
                            )
                ...
                if isinstance(ssl_loss_val, torch.Tensor):
                    ...
                    loss = (
                        loss + torch.tensor(ssl_loss_val, device=lm_loss.device)
                ...
                if wsm_merger is not None:
                    ...
                    if wsm_merger.should_merge(step):
                        ...
                        print(
                            f"[WSM] Merged checkpoint at step {step} buffer {len(wsm_merger.buffer)} ema {args.wsm_ema} -> {saved}"
                        )
                ...
                if step % 10 == 0:
                    orojar_str = (
                        f" orojar {orojar_loss_val.item():.4f}"
                        if isinstance(orojar_loss_val, torch.Tensor)
                        else f" orojar {orojar_loss_val:.4f}"
                        if orojar_loss_val
                        else ""
                    )
                    jac_str = (
                        f" fro {jacobian_metrics.get('fro', 0):.2f} cos {jacobian_metrics.get('cos', 0):.3f} orth {jacobian_metrics.get('orth', 0):.3f}"
                        if jacobian_metrics
                        else ""
                    )
                    mojo_str = (
                        f" mojo_ssl {ssl_loss_val.item():.4f}"
                        if isinstance(ssl_loss_val, torch.Tensor)
                        else f" mojo_ssl {ssl_loss_val:.4f}"
                        if isinstance(ssl_loss_val, (float, int)) and ssl_loss_val != 0
                        else ""
                    )
                    print(
                        f"[STREAM] step {step} lr {lr:.2e} sched {args.schedule} rope {rope['base']} ctx {rope['ctx']} phase={batch['phase']} task={maj_task} lm {lm_loss.item():.3f}{orojar_str}{jac_str}{mojo_str} delta {delta:.4f} tokens {ds.tokens_seen}"
                    )
                ...
                if step == 736000 and branch == "base" and args.schedule == "wsd":
                    ...
                    print(
                        "Saved dottie_stable_736k.pt at 736k — branching no memory spike"
                    )
                    _try_run_openwiki_and_harness(
                        mode="real", ckpt="dottie_stable_736k.pt"
                    )
                elif (
                    wsm_merger is not None
                    and step % args.wsm_merge_every == 0
                    and branch == "base"
                    and step > 0
                ):
                    torch.save(model.state_dict(), f"dottie_stable_wsm_{step}.pt")
                    print(
                        f"[WSM] Saved stable WSM ckpt at {step} (infinite continuation)"
                    )

                step += 1
                if step >= args.max_steps and args.max_steps > 0:
                    print(
                        f"Demo stop at {args.max_steps} steps — remove --max_steps for full 800k (WSM infinite if schedule=wsm)"
                    )
                    break
            ...
            print(
                f"Branch {branch} done streaming tokens_seen={ds.tokens_seen} schedule={args.schedule}"
            )
            os.system(f"python3 eval_branch_harness.py --branch {branch} --mode mock")
        else:
            wsm_merger_fallback = None
            if args.schedule == "wsm":
                wsm_merger_fallback = WSMCheckpointMerger(
                    buffer_size=args.wsm_buffer,
                    ema_decay=args.wsm_ema,
                    merge_every=args.wsm_merge_every,
                )
            for step in range(5):
                ...
                if step == 0:
                    ks, _s1, _s2, _comb = compute_capacity_curve()
                    print(f"W&B capacity law ks={ks} combined knee 9")
                if step % 2 == 0:
                    print(
                        f"step {step} lr {lr:.2e} sched {args.schedule} rope {rope['base']} ctx {rope['ctx']} orojar {args.orojar} — would log hl est etc"
                    )
            ...
            Path(f"dottie_{branch}_final_800k.pt").write_bytes(
                b"mock ckpt replace with torch.save"
            )
