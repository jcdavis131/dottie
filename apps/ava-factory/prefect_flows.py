...
def push_to_hf_task(
    manifest_path: str = "data/daily_expanded/manifest_*.jsonl",
    repo: str = "jcdavis131/dottie-textbook-v6",
    private: bool = True,
):
    ...
    if not hf_token:
        _log(
            "[hf] HF_TOKEN not set — saving manifest to data/for_upload/hf_ready_*.json for Alienware push (expected in Hatch VM)"
        )
...
def generate_phase(phase: str, tokens: int = 50_000_000):
    ...
    if script.exists():
        ...
        (out / f"{phase}.jsonl").write_text(
            json.dumps(
                {"phase": phase, "tokens": tokens, "ts": datetime.utcnow().isoformat()}
            )
            + "\n"
        )
        time.sleep(0.5)
...
def pack_shards(phase_dirs, seq_len: int = 1024):
    ...
    (packed / "manifest.json").write_text(
        json.dumps({"phases": phase_dirs, "seq_len": seq_len})
    )
    return str(packed)
...
def dottie_data_gen_flow(preset: str = "mini", tokens: int = 50_000_000):
    ...
    phases = (
        ["p0_logic", "p1_math", "p2_foundation", "p3_code"]
        if preset != "nano"
        else ["p0_logic", "p1_math"]
    )
...
def dottie_train_flow(
    preset: str = "mini", tokens_total: int = 2_500_000_000, resume: bool = True
):
...
def generate_teacher_rollouts(expert_name: str):
    ...
    return {
        "expert": expert_name,
        "logits_path": f"/tmp/{expert_name}_logits.pt",
        "specialization": expert_name,
    }
...
def reverse_kl_loss(student_rollout, teacher_rollouts):
    ...
    return {
        "loss": 0.42,
        "method": "MOPD reverse KL",
        "teachers": len(teacher_rollouts),
    }
...
def dottie_distill_flow(base_ckpt: str = "checkpoints/mini/dottie_stable_mini.pt"):
    ...
    _log(
        f"[distill] merged loss {loss} — ready for continual learning via earlier-teacher trick"
    )
    return loss
...
# ---------- EVAL FLOW ----------
@task(retries=2, log_prints=True)
def run_branch_eval(ckpt_path: str | None = None):
...
def run_frontier_eval_judge_ollama(domain: str = "all"):
    _log(
        f"[eval] frontier rubric domain={domain} judge={OLLAMA_MODEL} host={OLLAMA_HOST}"
    )
...
def dottie_eval_flow(ckpt_path: str | None = None):
    _log(f"Starting eval flow ckpt={ckpt_path}")
    branch = run_branch_eval(ckpt_path)
    if PREFECT_AVAILABLE:
        # Parallel domains
        frontier_list = run_frontier_eval_judge_ollama.map(
            ["finance", "bio", "code", "law", "macro"]
        )
    else:
        frontier_list = [
            run_frontier_eval_judge_ollama(d)
            for d in ["finance", "bio", "code", "law", "macro"]
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--run",
        choices=["data", "train", "distill", "eval", "all", "vector"],
        default="all",
    )
    ...
    print(
        f"Prefect available: {PREFECT_AVAILABLE} | OLLAMA_HOST={OLLAMA_HOST} | preset={args.preset}"
    )
    ...
    if args.run == "data":
        ...
        dottie_distill_flow(
            args.ckpt or f"checkpoints/{args.preset}/dottie_stable_{args.preset}.pt"
        )
    elif args.run == "eval":
