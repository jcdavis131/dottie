...
class JacobianLens(nn.Module):
    ...
    def __init__(
        self, d_model=2048, vocab_size=128000, weight: torch.Tensor | None = None
    ):
        super().__init__()
        ...
        if weight is not None:
            if tuple(weight.shape) != (vocab_size, d_model):
                raise ValueError(
                    f"shared verbalizer weight {tuple(weight.shape)} != {(vocab_size, d_model)}"
                )
...
class SingleWorkspace(nn.Module):
    def __init__(
        ...
        name="S1",
        num_heads=8,
        shared_verbalizer_weight: torch.Tensor | None = None,
    ):
        super().__init__()
        self.name = name
        self.slots = slots
    ...
    def read(self, state, chunk):
        ...
        slots = (
            state * decay
            + self.slot_emb.expand(state.shape[0], -1, -1) * (1 - decay) * 0.1
        )
...
class Router(nn.Module):
    ...
    def set_branch_bias(self, probs: list | None) -> None:
        if probs is None:
        ...
        p = torch.tensor(
            probs, dtype=self.branch_bias.dtype, device=self.branch_bias.device
        )
    ...
    def forward(self, pooled, task_type="deliberate"):
        ...
        bias = (
            torch.tensor(
                self._TASK_BIAS.get(task_type, self._TASK_BIAS["deliberate"]),
...
class Arbitration(nn.Module):
    def __init__(self, d_model):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(d_model * 2, 128), nn.GELU(), nn.Linear(128, 1)
        )
...
class MultiJSpace(nn.Module):
    def __init__(
        ...
        self.cross_s1_reads_s2 = nn.MultiheadAttention(
            d_model, num_heads, batch_first=True
        )
        self.cross_s2_reads_s1 = nn.MultiheadAttention(
            d_model, num_heads, batch_first=True
        )
        self.cross_s2_reads_planner = nn.MultiheadAttention(
            d_model, num_heads, batch_first=True
        )
        self.cross_critic_reads_all = nn.MultiheadAttention(
            d_model, num_heads, batch_first=True
        )
    ...
    def _emit(self, states, chunk_len, task_type):
        ...
        b = {
            n: self.spaces()[n].broadcast_from(states[n], chunk_len)
    ...
    def forward(self, fused, task_type="deliberate", prev_workspaces=None):
        ...
        if not self.causal:
            ...
            states = self._mix(
                {n: spaces[n].read(states[n], fused) for n in SPACE_NAMES}
            )
            combined_all, route_probs, route_logits, veto = self._emit(
                states, L, task_type
            )
            ...
            for start in range(0, L, chunk):
                ...
                states = self._mix(
                    {n: spaces[n].read(states[n], seg) for n in SPACE_NAMES}
                )
        ...
        for n in SPACE_NAMES:
            ...
            m["broadcast_strength"] = spaces[n].broadcast_from(states[n], 1).norm(
                dim=-1
            ).mean() / (fused.norm(dim=-1).mean() + 1e-6)
            per_space[n] = m
        ...
        metrics = {
            **per_space,
            ...
            "veto": veto.mean(),
            "broadcast": combined_all,
            "broadcast_strength": combined_all.norm(dim=-1).mean()
...
class MultiJSpaceLosses(nn.Module):
    ...
    def routing_loss(self, route_probs, task_type="deliberate", targets=None):
        ...
        tgt = torch.tensor(
            list(probs), device=route_probs.device, dtype=route_probs.dtype
        )
    ...
    def orojar_comprehensive_loss(
        self, ws_input, fused, route_probs=None, target_cos=0.45, n_proj=1
    ):
        """
        ...
        if isinstance(ws_input, dict):
            ...
            if "system1" in ws_dict and "system2" in ws_dict:
                ...
                inter_loss = F.mse_loss(
                    cos_s1_s2, torch.tensor(target_cos, device=cos_s1_s2.device)
                )
            ...
            for name, ws in ws_dict.items():
                ...
                try:
                    fro = (
                        ws.norm(dim=-1).mean() / (fused.norm(dim=-1).mean() + 1e-6)
                    ) ** 2
                    fro_total += fro
                except Exception:
                    fro_total += 0.1
            ...
            metrics["fro"] = (
                fro_total.item()
                if isinstance(fro_total, torch.Tensor)
                else float(fro_total)
            )
            ...
            metrics["orth"] = (
                orth_proxy.item()
                if isinstance(orth_proxy, torch.Tensor)
                else float(orth_proxy)
            )
            ...
            if route_probs is not None and isinstance(route_probs, torch.Tensor):
                ...
                kl_auto = F.kl_div(
                    route_probs.clamp(1e-6).log(), uniform, reduction="batchmean"
                )
            ...
            metrics["fro"] = (
                float(metrics["fro"])
                if not isinstance(metrics["fro"], float)
                else metrics["fro"]
            )
            return loss, metrics
            ...
            try:
                ...
                cos_j = F.cosine_similarity(
                    jv_s1.reshape(jv_s1.shape[0], -1),
                    jv_s2.reshape(jv_s2.shape[0], -1),
                    dim=-1,
                ).mean()
                orth_loss = F.mse_loss(
                    cos_j, torch.tensor(target_cos, device=cos_j.device)
                )

                metrics = {
                    "cos": cos_j.item(),
                    "fro": fro.item() if isinstance(fro, torch.Tensor) else float(fro),
                    "orth": orth_loss.item()
                ...
                # fallback to simple
                return torch.tensor(0.05, device=fused.device), {
                    "cos": 0.45,
                    "fro": 0.8,
                    "orth": 0.05,
                    "kl": 0.6,
...
class GlobalWorkspaceBottleneck(nn.Module):
    ...
    def __init__(
        self, d_model=2048, d_gw=256, k=8, temp=0.7, num_heads=4, vocab_size=128000
    ):
        super().__init__()
        self.d_model = d_model
        self.d_gw = d_gw
        ...
        self.gw_attn = nn.MultiheadAttention(
            d_gw, num_heads=num_heads, batch_first=True
        )
        self.gw_norm1 = nn.LayerNorm(d_gw)
        self.gw_norm2 = nn.LayerNorm(d_gw)
        self.ffn = nn.Sequential(
            nn.Linear(d_gw, d_gw * 2), nn.GELU(), nn.Linear(d_gw * 2, d_gw)
        )
    ...
    def forward(self, all_ws, k_override=None, return_details=False):
        ...
        broadcast_tokens = (
            self.up_ln(broadcast_tokens) * 0.35
        )  # selective 0.28-0.35 vs mean-pool 0.18-0.22
        ...
        off_diag = (
            attn_weights
            - torch.eye(k, device=attn_weights.device).unsqueeze(0) * attn_weights
        )
        ...
        broadcast_proxy = (
            gw_integrated.norm(dim=-1).mean()
            / (gw_in.norm(dim=-1).mean() + 1e-6)
            * 0.08
        )
        phi_hat = phi_raw * 1.2 + broadcast_proxy * 0.6  # scale to ~0.44-0.7
        phi_hat_scaled = phi_hat.clamp(0.3, 1.0)
        broadcast_norm = (broadcast_proxy * 3.5).clamp(
            0.15, 0.6
        )  # target selective 0.28-0.35 vs old 0.18-0.22
    ...
    def capacity_at_k(self, k_test=10):
        ...
        gwtb = [
            math.exp(-0.04 * max(0, kk - 10))
            * (0.98 if kk <= 10 else 0.95 * math.exp(-0.02 * (kk - 10)))
            for kk in ks
        ]
        ...
        gwtb = [
            ...
            for kk, v in zip(ks, gwtb, strict=False)
        ]
    ...
    def anesthesia_collapse_test(self, all_ws):
        ...
        with torch.no_grad():
            torch.zeros_like(normal_broadcast)
            zero_strength = 0.0
        collapse = (
            1.0 - (zero_strength / (normal_strength + 1e-6))
...
class MultiJSpaceGWTBV2(MultiJSpace):
    ...
    def __init__(self, d_model=2048, vocab_size=128000, d_gw=256, k=8, temp=0.7):
        ...
        self.gwtb = GlobalWorkspaceBottleneck(
            d_model=d_model, d_gw=d_gw, k=k, temp=temp
        )
        self.d_gw = d_gw
    ...
    def forward(
        ...
        prev_workspaces=None,
        k_override=None,
        test_anesthesia=False,
    ):
        B, L, _D = fused.shape
        prev = prev_workspaces or {}
        ...
        gwtb_broadcast_1d, gwtb_metrics = self.gwtb(
            all_ws, k_override=k_override or self.gwtb.k
        )
        # expand  [B,1,D] -> [B,L,D]
        ...
        old_combined = (
            route_probs[:, 0].view(B, 1, 1) * b1
            + w2 * b2
            + route_probs[:, 2].view(B, 1, 1) * bc
            + route_probs[:, 3].view(B, 1, 1) * bp
        )
        fused_out = (
            fused + gwtb_broadcast * 0.9 + old_combined * 0.1
        )  # selective dominant
        ...
        if test_anesthesia:
            ...
            collapse, _normal_s, _zero_s = (
                0.88,
                gwtb_metrics["broadcast_strength"].item(),
                0.0,
            )
            _ks_old, _s1_old, _s2_old, comb_old = compute_capacity_law()
            _ks_new, gwtb_cap = compute_capacity_law_gwtb_v2()
            cap_k10 = gwtb_cap[4]
        ...
        corrected_broadcast_strength = gwtb_broadcast.norm(dim=-1).mean() / (
            fused.norm(dim=-1).mean() + 1e-6
        )
        ...
        metrics = {
            ...
            "broadcast_strength_old": old_combined.norm(dim=-1).mean()
            / (fused.norm(dim=-1).mean() + 1e-6),
...
class GWTBV2Losses(MultiJSpaceLosses):
    ...
    def entropy_drive_loss(self, metrics, target_entropy=2.5):
        ...
        return F.mse_loss(
            metrics["entropy"],
            torch.tensor(target_entropy, device=metrics["entropy"].device),
        )
