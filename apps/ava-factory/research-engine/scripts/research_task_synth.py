...
"""
import os, sys, re, json, yaml, subprocess, time
...
target_list = tasks_cfg.get("target_list_id", "MDg4NTEzMTkzNjgwNzI5NDMyMDI6MDow")  # Lina's Morning
...
def list_existing_tasks(tasklist_id):
    try:
        cmd = ["hatch_gws_cli", "tasks", "tasks", "list", "--params", json.dumps({"tasklist": tasklist_id, "maxResults": 100})]
...
recent_scored = sorted(recent_sorted, key=lambda p: (topic_importance(p), p.get("updated","")), reverse=False)
...
"""
    ...
    try:
        cmd = ["hatch_gws_cli", "tasks", "tasks", "insert", "--params", json.dumps({"tasklist": target_list}), "--json", json.dumps(payload)]
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=20)
        if result.returncode == 0:
            ...
            cmd2 = ["hatch_gws_cli", "tasks", "tasks", "insert", "--params", json.dumps({"tasklist": "@default"}), "--json", json.dumps(payload)]
...
log_path.write_text(json.dumps({
    ...
    "tasks": [{"id": c["task"].get("id"), "title": c["title"], "arxiv_id": c["paper"]["arxiv_id"]} for c in created],
}, indent=2))
...
try:
    ...
    if result.returncode == 0:
        ...
        if also_create not in titles:
            ...
            cmd2 = ["hatch_gws_cli", "tasks", "tasklists", "insert", "--json", json.dumps({"title": also_create})]
