...
try:
    ...
    HAS_CHONKIE = False
    ChonkieTokenChunker = ChonkieRecursiveChunker = ChonkieSentenceChunker = (
        ChonkieCodeChunker
    ) = None
    print(
        f"[Chonkie] not installed, will use fallback SimpleTokenizer chunking — pip install chonkie for proper chunking: {_e}"
    )
...
BRANCH_MIX = {
    ...
    "math": {
        "math_formal_lean": 0.35,
        "lean_mathlib": 0.20,
        "proofpile2": 0.20,
        "synthetic_math_r1": 0.15,
        "general": 0.10,
    },
    "chat": {
        "chat_alignment": 0.30,
        "safety_blackmail_leverage": 0.20,
        "jobbench_delegation": 0.25,
        "gaia2_temporal": 0.15,
        "counterfactual": 0.10,
    },
}
...
SOURCE_TO_TASK = {
    "dclm": "automatic",
    "dclm_top15": "automatic",
    "web_edu_gte2": "automatic",
    "code_early": "automatic",
    "synthetic_logic_textbooks_phi_B": "deliberate",
    "math_textbooks_ordered": "deliberate",
    "math": "deliberate",
    "lean": "deliberate",
    "metamath": "deliberate",
    "fol": "deliberate",
    "long_docs_3x": "deliberate",
    "workflow_jobbench": "deliberate",
    "workflow_jobbench_messy": "deliberate",
    "jobbench_code": "deliberate",
    "workflow_karpathy": "deliberate",
    "safety_blackmail_leverage": "safety",
    "chat_alignment": "safety",
    "workflow_gaia2_simple": "temporal",
    "workflow_gaia2_dynamic": "temporal",
    "workflow_gaia2_hard": "temporal",
    "gaia2_temporal": "temporal",
    "jobbench_delegation": "temporal",
    "code_repo": "deliberate",
    "code_long_32k": "deliberate",
    "math_formal_lean": "deliberate",
    "lean_mathlib": "deliberate",
    "proofpile2": "deliberate",
    "synthetic_math_r1": "deliberate",
    "edu_gte4.5": "deliberate",
    "verified_proofs": "deliberate",
}
...
PHASE_CHONKIE_CONFIG = {
    "phase0_logic": {
        ...
        "overlap": 128,
        "recipe": "markdown",
        "desc": "logic textbooks — keep definitions/theorems intact",
    },
    "phase1_math": {
        "chunker": "recursive",
        "chunk_size": 4096,
        "overlap": 256,
        "recipe": "markdown",
        "desc": "math ordered curriculum arithmetic→probability",
    },
    "phase2_foundation": {
        "chunker": "token",
        "chunk_size": 4096,
        "overlap": 128,
        "desc": "web+code early — fixed tokens for speed",
    },
    "phase3_reasoning": {
        "chunker": "recursive",
        "chunk_size": 8192,
        "overlap": 512,
        "recipe": "default",
        "desc": "long docs upsampled 3x — semantic chunks",
    },
    "phase4_long": {
        "chunker": "recursive",
        "chunk_size": 32768,
        "overlap": 1024,
        "recipe": "default",
        "desc": "32k long — code_long + workflow messy",
    },
    "phase5_anneal": {
        "chunker": "token",
        "chunk_size": 65536,
        "overlap": 0,
        "desc": "anneal high quality edu>=4.5",
    },
}
...
class SimpleTokenizer:
    ...
    def encode(self, text: str) -> list[int]:
        ...
        return [
            (x % (self.vocab_size - 256)) + 256 for x in b[:8192]
        ]  # cap per example pre-chunk
...
def get_tokenizer():
    ...
    try:
        ...
        for p in [
            "dottie-tokenizer",
            "data/dottie-tokenizer",
            "data/streaming_shards/dottie-tokenizer",
        ]:
            if Path(p).exists():
...
class ChonkieChunkerWrapper:
        ...
        try:
            if self.chunker_type == "token":
                ...
                try:
                    ...
                    self._chunker = ChonkieCodeChunker(
                        tokenizer=self.tokenizer_name, chunk_size=self.chunk_size
                    )
            else:
                self._chunker = ChonkieRecursiveChunker(**kwargs)
            print(
                f"[Chonkie] Initialized {self.chunker_type} chunk_size={self.chunk_size} overlap={self.chunk_overlap} tokenizer={self.tokenizer_name} recipe={self.recipe}"
            )
    ...
    def chunk(self, text: str) -> list[dict]:
        ...
        if HAS_CHONKIE and self._chunker is not None:
            try:
                chonks = self._chunker.chunk(
                    text
                )  # returns list of Chunk objects with .text, .token_count【2997878068158776486†L91-L97】
        ...
        chunks = []
        step = max(
            1, self.chunk_size * 4 - self.chunk_overlap * 4
    ...
    def _open_next(self):
        ...
        try:
            if path.endswith(".gz"):
                ...
                self.fh = open(
                    path, encoding="utf-8", errors="ignore", buffering=8192 * 4
                )
            return True
    ...
    def __next__(self) -> dict:
        ...
        while attempts < 10:
            ...
            try:
                ...
                obj.setdefault(
                    "task_type", SOURCE_TO_TASK.get(self.source_name, "deliberate")
                )
                return obj
        ...
        time.sleep(0.2)
        return {
            "text": f"# Placeholder {self.source_name} synthetic chunk {self.line_no}",
            "source": self.source_name,
            "task_type": SOURCE_TO_TASK.get(self.source_name, "deliberate"),
            "_placeholder": True,
        }
    ...
    def __init__(self, mix: dict[str, float], data_root: Path, seed: int = 0):
        ...
        for src in self.sources:
            ...
            patterns = [
                ...
                str(
                    self.data_root / "synthetic" / f"*{src}*.md"
                ),  # for logic_textbook_pipeline legacy
            ]
...
class ShuffleBuffer:
    ...
    def __init__(
        self, source_iter: Iterator[dict], buffer_size: int = 10000, seed: int = 0
    ):
...
def get_phase_for_step(
    step: int, tokens_per_step: int = 2_097_152
) -> str:  # 2M tokens/step @ 1B bs 1M
...
class DottieStreamingDataset:
    ...
    def __init__(
        ...
        self._stream: MultiSourceWeightedStream | None = None
        self._shuffled: ShuffleBuffer | None = None
        ...
        self._chonkie_chunker: ChonkieChunkerWrapper | None = None
        if self.use_chonkie:
            chunker_type if chunker_type != "auto" else get_phase_chunker_config(
                self.current_phase, max_seq_len
            )["chunker"]
            self._chonkie_chunker = ChonkieChunkerWrapper.for_phase(
                self.current_phase, max_seq_len, branch
            )
            ...
            print(
                f"[Chonkie] Enabled for dataset — {self._chonkie_chunker.chunker_type} phase={self.current_phase} seq_len={max_seq_len}"
            )
        else:
            print(
                "[Chonkie] Disabled — using fallback tokenizer chunking (install with pip install chonkie)"
            )
    ...
    def _init_stream_for_phase(self, phase: str):
        ...
        print(
            f"[Streaming] Init phase={phase} branch={self.branch} mix={mix} root={self.data_root}"
        )
        weighted = MultiSourceWeightedStream(
            mix, self.data_root, seed=self.seed + self.steps_seen
        )
        self._stream = weighted
        self._shuffled = ShuffleBuffer(
            iter(weighted), buffer_size=self.shuffle_buffer, seed=self.seed
        )
        ...
        if self.use_chonkie:
            if self.chunker_type_arg == "auto" or self._chonkie_chunker is None:
                self._chonkie_chunker = ChonkieChunkerWrapper.for_phase(
                    phase, self.max_seq_len, self.branch
                )
            else:
                ...
                self._chonkie_chunker.chunk_overlap = cfg.get(
    ...
    def _maybe_switch_phase(self):
        if self.phase_mode != "auto":
            return
        new_phase = (
            get_phase_for_tokens(self.tokens_seen)
            if self.tokens_seen > 0
            else get_phase_for_step(self.steps_seen, self.tokens_per_step)
        )
        if new_phase != self.current_phase:
            print(
                f"[Streaming] Phase switch {self.current_phase} -> {new_phase} at tokens={self.tokens_seen} steps={self.steps_seen}"
            )
            self._init_stream_for_phase(new_phase)

    def __iter__(self) -> Iterator[dict]:
        yield from self._shuffled

    def batched(
        self, seq_len: int = 2048, batch_size: int = 4, use_chonkie: bool | None = None
    ) -> Iterator[dict]:
        """
        ...
        use_chonkie_eff = (
            self.use_chonkie if use_chonkie is None else (use_chonkie and HAS_CHONKIE)
        )
        buf_tokens: list[int] = []
        buf_task = []
        buf_source = []
        ...
        if (
            use_chonkie_eff
            and self._chonkie_chunker
            and seq_len != self._chonkie_chunker.chunk_size
        ):
            ...
            if use_chonkie_eff and self._chonkie_chunker:
                ...
                for ch in doc_chunks:
                    ch_text = (
                        ...
                        else getattr(ch, "text", str(ch))
                    )
                    # tokenize each Chonkie chunk — per-chunk, not per-doc, flat memory
                    ids = (
                        self.tokenizer.encode(ch_text)
                        if hasattr(self.tokenizer, "encode")
                        else self.tokenizer.encode(ch_text)
                    )
                    ...
                    while len(buf_tokens) >= seq_len:
                        ...
                        if len(batch_texts) >= batch_size:
                            ...
                            out = {
                                ...
                                "chonkie": {
                                    "enabled": True,
                                    "chunks_per_doc": len(doc_chunks),
                                    "chunker": self._chonkie_chunker.chunker_type,
                                },
                            }
                            batch_texts = []
                            buf_task = []
                            ...
                            if self.steps_seen % 20 == 0:
                                avg = chonkie_stats["chonkie_chunks"] / max(
                                    1, chonkie_stats["docs_chunked"]
                                )
                                print(
                                    f"[Chonkie] steps={self.steps_seen} tokens_seen={self.tokens_seen} docs={chonkie_stats['docs_chunked']} total_chunks={chonkie_stats['chonkie_chunks']} avg_chunks/doc={avg:.2f} shuffle_q={self._shuffled.qsize()} RAM=flat"
                                )
    ...
    def _save_checkpoint(self):
        try:
            ...
            out = (
                self.checkpoint_dir
                / f"stream_state_{self.current_phase}_{self.branch}.json"
            )
    ...
    def load_checkpoint(self, path: str):
        try:
            ...
            print(
                f"[Streaming] Loaded checkpoint {path} tokens={self.tokens_seen} steps={self.steps_seen}"
            )
        except Exception as e:
...
class SyntheticShardGenerator:
    ...
    def start(self):
        ...
        self.thread.start()
        print(
            f"[SyntheticGenerator] Started background Phi B generation into {self.data_root}/synthetic_logic_textbooks_phi_B"
        )
...
if __name__ == "__main__":
        ...
        if not dummy.exists():
            with open(dummy, "w") as f:
                for i in range(100):
                    json.dump(
                        {
                            "text": f"Dummy {src} example {i} for streaming test "
                            + "lorem ipsum " * 20,
                            "source": src,
                        },
                        f,
    ...
    try:
        for idx, batch in enumerate(
            ds.batched(seq_len=args.seq_len, batch_size=args.batch_size)
        ):
            ids = batch["input_ids"]
            try:
                import torch
                ...
                shape_str = "unknown"
            print(
                f"Step {idx} phase={batch['phase']} tasks={batch['task_type']} input_ids shape={shape_str} tokens_seen={ds.tokens_seen}"
            )
            if idx + 1 >= args.steps:
                break
    finally:
