...
# import our factory
try:
    from streaming_data import PHASE_TOKENS, AvaStreamingDataset
...
def wait_for_ready(
    data_root: Path, phase_name: str, poll_interval: int = 5, timeout: int = 3600
):
    start = time.time()
    while True:
        candidates = [
        ...
        if len(list(data_root.rglob("*.jsonl*"))) >= 2:
            print(
                f"[Trainer] Found {len(list(data_root.rglob('*.jsonl*')))} shards, treating as ready for {phase_name}"
            )
            return True
        for c in candidates:
            if c.exists():
        ...
        if time.time() - start > timeout:
            print(
                f"[Trainer] Timeout waiting for {phase_name} ready, proceeding anyway after {timeout}s"
            )
            return False
        print(
            f"[Trainer] Waiting for {phase_name} .ready or shards in {data_root}... {int(time.time() - start)}s"
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="Trainer Agent - watches lake and trains immediately"
    )
    ...
    ap.add_argument(
        "--max_steps", type=int, default=50, help="demo steps, set 736000 for full WSD"
    )
    ...
    phases = load_phases()
    print("[Trainer] Loaded phases curriculum:")
    for p in phases:
    ...
    print(
        f"[Trainer] Curriculum-first: waiting for {current_phase} to be collected by builder agent..."
    )
    ...
    if not HAS_STREAMING:
        ...
        for i in range(args.max_steps):
            print(
                f"[Trainer MOCK] step {i} phase={current_phase} tokens_seen={i * args.seq_len * args.batch_size}"
            )
            time.sleep(0.1)
        return
    ...
    try:
        ...
        from train_1b_deepspeed import BRANCH_CONFIGS, get_rope, wsd_lr

        HAS_MODEL = True
    except Exception as e:
        print(
            f"[Trainer] model import failed {e}, will run streaming dataset only demo"
        )
        HAS_MODEL = False
        BRANCH_CONFIGS = {}
    ...
    print(
        f"[Trainer] Building AvaStreamingDataset data_root={data_root} branch={args.branch} shuffle_buffer={args.shuffle_buffer} seq_len={args.seq_len} use_chonkie={args.use_chonkie}"
    )
    ...
    print(
        f"[Trainer] Dataset stats {ds.stats()} - memory guarantee: shuffle_buffer ({args.shuffle_buffer}) + batch + 1 doc chunks + 1 fh per source"
    )
    ...
    if trainer_state_path.exists():
        try:
            ...
            print(
                f"[Trainer] Resumed from {trainer_state_path} steps={steps} tokens={tokens_seen}"
            )
        except:
            pass
    ...
    if HAS_MODEL and HAS_TORCH:
        try:
            ...
            model.train()
            print(
                "[Trainer] Model loaded, starting real training loop with Chonkie chunks"
            )
        except Exception as e:
            print(
                f"[Trainer] model load failed {e}, fallback to dataset streaming demo"
            )
            model = None
    ...
    try:
        for batch in ds.batched(
            ...
            if HAS_MODEL:
                try:
                    ...
                    apply_rope_scaling(
                        ...
                        rope["base"],
                        rope["base"] // 10000
                        if not rope.get("yarn")
                        else rope["base"] / 10000,
                    )
                    lr = wsd_lr(steps)
            ...
            if model is not None and HAS_TORCH:
                try:
                    ...
                    task_type = (
                        batch["task_type"][0] if batch["task_type"] else "deliberate"
                    )
            ...
            if steps % 5 == 0:
                print(
                    f"[Trainer] step {steps} tokens_seen {tokens_seen} phase {batch.get('phase')} task_types {batch.get('task_type')} sources {batch.get('source')} loss {loss_val:.3f} chonkie={batch.get('chonkie')} RAM flat (shuffle_buffer {args.shuffle_buffer})"
                )

            # WSD checkpoint at 736k (mock at small)
            if steps == 20 and args.branch == "base":  # demo checkpoint, real is 736000
                try:
                    Path("ava_stable_736k.pt").write_text(
                        f"mock stable at step {steps} tokens {tokens_seen} with chonkie"
                    )
                    print(
                        "[Trainer] Saved ava_stable_736k.pt demo - other agents can start branch training code/math/chat"
                    )
                except:
                    pass
            ...
            if steps >= args.max_steps:
                print(
                    f"[Trainer] Reached max_steps {args.max_steps}, checkpoint and check next curriculum phase"
                )
                if args.loop:
            ...
            if batch.get("phase") != current_phase:
                print(
                    f"[Trainer] Curriculum advance: {current_phase} -> {batch.get('phase')} at tokens {tokens_seen}, builder should already be preparing next phase in background"
                )
        ...
        print(
            f"[Trainer] Done steps {steps} tokens_seen {tokens_seen} final stats {ds.stats() if HAS_STREAMING else {}}"
        )
