...
def auto_init_model(model, std_base=0.02):
    ...
    if hasattr(model, "lm_head"):
        with torch.no_grad():
            model.lm_head.weight.data *= 1.0 / math.sqrt(
                model.d_model if hasattr(model, "d_model") else 2048
            )
    print(
        "AutoInit done: scaled init std=0.02/sqrt(2*layer), RMSNorm ones, LM head scaled by 1/sqrt(d)"
    )
