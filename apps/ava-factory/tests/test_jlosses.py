...
def test_modulation_can_be_nonzero(setup):
    ...
    parts = _run(*setup)
    assert parts.modulation.abs().item() > 0.0, (
        "modulation cannot fire -- it is a constant"
    )
...
def test_selectivity_is_scale_invariant(setup):
    ...
    assert a.abs().item() > 0.1, (
        "on genuinely varied slots the term must be substantial"
    )
...
def test_selectivity_sign_flips_with_task_type(setup):
    ...
    jm = {
        "workspaces": {
            "system1": torch.randn(2, 8, 16),
            "system2": torch.randn(2, 8, 16),
        }
    }
...
def test_routing_target_depends_on_task_type(setup):
    cfg, model, obj = setup
    losses = {
        tt: _run(cfg, model, obj, task_type=tt).routing.item()
...
def test_total_is_the_weighted_sum(setup):
    ...
    aux = (
        p.report * w["report"]
        + p.broadcast * w["broadcast"]
        + p.selectivity * w["selectivity"]
        + p.modulation * w["modulation"]
    )
    expect = (
        p.lm
        + aux * jw
