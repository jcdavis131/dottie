...
def test_call_record_produces_full_react_cycle():
    ...
    assert "<functioncall>" not in text and "<|endoftext|>" not in text  # markup rewritten
...
def test_garbage_records_skip_honestly():
    ...
    assert adapt_record({"chat": "USER: hi ASSISTANT: <functioncall> not-json "
                                 "<|endoftext|>"}) is None               # unparseable call
...
def test_registered_in_adapters():
    ...
    assert apply_adapter("glaive_react", CALL_REC)["_concept"] == "glaive_get_stock_price"
