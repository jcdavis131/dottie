...
from ava.rl.grpo_torch import TorchGRPOStep
...
def _load_module():
    ...
    sys.modules["rl_smoke_update"] = (
        mod  # dataclasses resolves cls.__module__ via sys.modules
    )
...
def test_recording_policy_records_ids_and_plain_logprobs():
    ...
    expected = float(torch.log_softmax(torch.tensor([5.0] + [0.0] * 127), dim=-1)[0])
...
def _rollout(prompt_ids, gen_ids, old_logps, **kw):
    ...
    defaults.update(kw)
    return smoke.Rollout(
        prompt_ids=prompt_ids, gen_ids=gen_ids, old_logps=old_logps, **defaults
    )
...
def test_compute_r_task_containment_rules():
    ...
    assert (
        smoke.compute_r_task("42", "42", False) == 0.0
    )  # step-cap: no FINAL, no credit
...
def test_full_wiring_tiny_stub_through_real_sandbox_and_grpo_step():
    ...
    stepper = TorchGRPOStep(
        lm,
        torch.optim.SGD(lm.parameters(), lr=0.01),
        EntropyThermostat(kappa=0.5, h_target=0.3),
        r_outer=1.0,
    )
    stats = stepper.step(
