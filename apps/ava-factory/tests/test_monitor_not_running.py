...
def _run(monkeypatch, tmp_path, collect_return, status_json=None):
    ...
    monkeypatch.setattr(
        ps, "collect_status", lambda *a, **k: collect_return, raising=False
    )
...
def test_no_training_reports_not_running_never_builder_stale(monkeypatch, tmp_path):
    ...
    assert res["detail"].get("builder_age_s", 0) > 1800, (
        "builder age recorded for context"
    )
