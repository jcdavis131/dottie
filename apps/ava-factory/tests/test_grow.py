...
from dottie.grow import grow_state_dict, validate_growth  # noqa: E402

from test_model import _tiny  # noqa: E402


def _widened(cfg: AvaConfig, k: int) -> AvaConfig:
    """The same tiny preset, k× wider (d_model and n_heads; head_dim fixed)."""
    import dataclasses
    model = dataclasses.replace(cfg.model, d_model=cfg.model.d_model * k,
                                n_heads=cfg.model.n_heads * k)
...
def test_integer_width_growth_is_function_preserving():
    ...
    grown, manifest = grow_state_dict(src.state_dict(), dst,
    ...
    ids = torch.randint(0, src_cfg.model.vocab_size, (2, 16),
...
def test_depth_stretch_runs_and_warm_start_beats_fresh():
    ...
    v = validate_growth(src, dst, fresh, vocab=src_cfg.model.vocab_size, seq=32, batch=2)
    assert all(map(lambda x: x == x, v.values())) # no NaN sneaks through rounding
...
def test_vocab_mismatch_is_refused_without_allow_partial():
    ...
    grown, manifest = grow_state_dict(src.state_dict(), dst, src_cfg=src_cfg,
                                      dst_cfg=dst_cfg, allow_partial=True)
