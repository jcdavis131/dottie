...
def test_jspace_noncausal_mode_does_leak():
    ...
    assert not torch.allclose(a[:, 0], b[:, 0], atol=1e-5), (
        "non-causal mode should leak"
    )
...
def test_chunk0_broadcast_is_data_independent():
    ...
    mj = m.multi_jspace
    with torch.no_grad():
        _a, _ = mj(torch.randn(1, 8, 32))
        _b, _ = mj(torch.randn(1, 8, 32))
...
def test_deltanet_block_is_causal():
    ...
    torch.testing.assert_close(base[:, :t], after[:, :t], atol=1e-5, rtol=1e-5)
    assert not torch.allclose(base[:, t], after[:, t], atol=1e-5), (
        "perturbation must propagate forward"
    )
...
def test_top_concepts_returns_real_ids_and_input_dependent_mass(tiny_model):
    ...
    for out in (a, b):
        ...
        assert float(m) != pytest.approx(0.06, abs=1e-9), (
            "constant 0.06 = the old dead branch"
        )
    ...
    masses = {
        float(a["system2"]["verbalizable_mass"]),
        float(b["system2"]["verbalizable_mass"]),
    }
...
def test_longrope2_rope_scaling_updates_lambda_factors():
    ...
    assert m.rope.critical < 31.0, (
        "critical dim must shift down (31->25 direction) as scale grows"
    )
...
def test_longrope2_rotary_preserves_norm_and_relative_position():
    ...
    torch.testing.assert_close(
        q.norm(dim=-1) * rope.mscale, qr.norm(dim=-1), atol=1e-4, rtol=1e-4
    )
    torch.testing.assert_close(
        k.norm(dim=-1) * rope.mscale, kr.norm(dim=-1), atol=1e-4, rtol=1e-4
    )
...
def test_router_branch_bias_shifts_routing():
    ...
    ids = torch.randint(0, 64, (1, 8))
    with torch.no_grad():
        before = m(input_ids=ids, task_type="deliberate")["jspace"][
            "route_probs"
        ].clone()
...
def test_routing_responds_to_task_type(tiny_model):
    ...
    with torch.no_grad():
        probs = {
            tt: tiny_model(input_ids=ids, task_type=tt)["jspace"]["route_probs"][0]
            for tt in (
                "automatic",
                "deliberate",
                "safety",
...
def test_init_loss_matches_uniform_predictor():
    ...
    assert abs(float(loss) - math.log(V)) < 0.5, (
        f"init loss {float(loss):.2f} vs ln(V)={math.log(V):.2f}"
    )
