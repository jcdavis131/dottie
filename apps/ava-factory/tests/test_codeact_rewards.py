...
class TestBlend:
    def test_task_dominates_exec_hack(self):
        ...
        correct = codeact_return(
            1.0, [obs()], token_count=20, family_pass_rate=0.5, weights=w
        )
        hack = codeact_return(
            0.0,
            [obs(), obs(), obs(), obs()],
    ...
    def test_return_is_finite_number(self):
        v = codeact_return(
            1.0, [obs(), obs(ok=False)], token_count=120, family_pass_rate=0.7
        )
