...
def test_classify_html_scrapeable():
    c = reach.classify(
        ...
        body_text="<!doctype html><html>hi",
        url="https://x",
        json_body=None,
    )
    assert c["kind"] == "html"
...
def test_classify_empty_not_hopeful_json():
    ...
    c = reach.classify(
        status=204, content_type="", body_text="", url="https://x", json_body=None
    )
    assert c["kind"] == "empty"
...
def test_plan_html_generates_scraper():
    steps = reach.plan_unblock(
        {"url": "https://blog.x.dev", "kind": "html", "suggested_name": "x"}
    )
...
def test_plan_mcp_forges_proxy():
    steps = reach.plan_unblock(
        {"url": "https://x/mcp", "kind": "mcp", "suggested_name": "x"}
    )
...
def test_plan_openapi_unregistered_says_register():
    steps = reach.plan_unblock(
        {"url": "https://x/openapi.json", "kind": "openapi", "registered": False}
    )
...
def test_plan_openapi_registered_is_done():
    steps = reach.plan_unblock(
        {"url": "https://x/openapi.json", "kind": "openapi", "registered": True}
    )
    assert steps == []
...
def test_plan_error_suggests_diagnose_before_giving_up():
    steps = reach.plan_unblock(
        {"url": "https://x", "kind": "error", "tried_well_known": False}
    )
