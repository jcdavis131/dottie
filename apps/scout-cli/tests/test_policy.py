...
def _manifest(enabled=True, domains=None, fs_write=False):
    return {
        "name": "t",
        "capabilities": {
            "network": {"enabled": enabled, "domains": domains if domains is not None else []},
...
class TestManifestNetworkMatrix:
    def test_network_disabled_denies(self):
        ok, reason = policy.check_permission(_manifest(enabled=False), "network", "https://a.com")
        assert not ok
    ...
    def test_enabled_empty_domains_denies_everything(self):
        ...
        ok, reason = policy.check_permission(_manifest(domains=[]), "network", "https://a.com")
        assert not ok
        assert "default-deny" in reason or "empty" in reason

    def test_matching_domain_allows(self):
        ok, _ = policy.check_permission(_manifest(domains=["api.example.com"]),
                                        "network", "https://api.example.com/v1/x")
        assert ok

    def test_subdomain_allows(self):
        ok, _ = policy.check_permission(_manifest(domains=["example.com"]),
                                        "network", "https://api.example.com/v1")
        assert ok

    def test_url_mismatch_denies(self):
        ok, reason = policy.check_permission(_manifest(domains=["example.com"]),
                                             "network", "https://evil.org/x")
        assert not ok
    ...
    def test_fs_write_denied_by_default(self):
        ok, reason = policy.check_permission(_manifest(), "fs_write", "/tmp/x")
        assert not ok
    ...
    def test_enforce_or_raise_exits_on_deny(self):
        with pytest.raises(typer.Exit):
            policy.enforce_or_raise(_manifest(enabled=False), "network", "https://a.com")
...
class TestUserAllowlist:
    ...
    def test_missing_file_materializes_default_local_only(self):
        ...
        assert ok
        assert self.fp.exists(), "default policy file should be created for the user to edit"
        ok2, reason = policy.check_user_url("https://api.example.com/x")
        assert not ok2
