...
def test_json_contract():
    ...
    r = subprocess.run([sys.executable, "-m", "bigbang.cli", "--json", "tools", "list"], capture_output=True, text=True, encoding="utf-8", errors="replace")
...
def test_write_cli_json():
    import subprocess, json
    r = subprocess.run([sys.executable, "-m", "bigbang.cli", "--json", "write", "scan", "--text", "Hello world this is a simple note from Austin."], capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=8)
    assert r.returncode == 0
    data = json.loads(r.stdout)
