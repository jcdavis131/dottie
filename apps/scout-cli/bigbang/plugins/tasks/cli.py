...
def _run_gws(args: List[str], json_input: Optional[dict] = None) -> dict:
    ...
    try:
        if json_input is not None:
            ...
            proc = subprocess.run(cmd + ["--json", json.dumps(json_input)], capture_output=True, text=True, timeout=30)
        else:
        ...
        try:
            ...
            return {"ok": proc.returncode == 0, "raw": out, "parsed_error": "non-json output", "stderr": proc.stderr[:500]}
...
def lists_cmd():
    ...
    emit({"tasklists": items, "count": len(items), "raw": res, "hint": "bb tasks list --tasklist <id> or bb tasks list --tasklist @default"}, command="tasks lists")

@app.command("list")
def list_tasks(
    tasklist: str = typer.Option("@default", "--tasklist", help="tasklist id or @default"),
    show_completed: bool = typer.Option(False, "--show-completed", help="include completed"),
    max_results: int = typer.Option(50, "--max", help="max tasks"),
    due_min: Optional[str] = typer.Option(None, help="RFC3339 dueMin filter"),
    due_max: Optional[str] = typer.Option(None, help="RFC3339 dueMax filter"),
):
    params = {"tasklist": tasklist, "maxResults": max_results, "showCompleted": show_completed, "showDeleted": False}
    if due_min:
    ...
    items = res.get("items", []) if isinstance(res, dict) else []
    emit({"tasklist": tasklist, "tasks": items, "count": len(items), "raw": res}, command="tasks list")
...
def get_task(
    ...
    res = _run_gws(["tasks", "get", "--params", json.dumps({"tasklist": tasklist, "task": task_id})])
    emit({"task": res, "tasklist": tasklist}, command="tasks get")

@app.command("add")
def add_task(
    title: str = typer.Argument(..., help="task title"),
    notes: Optional[str] = typer.Option(None, help="notes/body"),
    due: Optional[str] = typer.Option(None, help="RFC3339 due e.g. 2026-07-16T00:00:00Z"),
    tasklist: str = typer.Option("@default", "--tasklist", help="tasklist id"),
):
    body = {"title": title}
    if notes:
    ...
    if due:
        body["due"] = due
    res = _run_gws(["tasks", "insert", "--params", json.dumps({"tasklist": tasklist})], json_input=body)
    emit({"created": res, "tasklist": tasklist, "input": body, "next": f"bb tasks list --tasklist {tasklist}"}, command="tasks add")
...
def update_task(
    ...
    due: Optional[str] = typer.Option(None, help="new due RFC3339"),
    tasklist: str = typer.Option("@default", "--tasklist", help="tasklist id"),
):
    body = {}
...
def complete_task(
    ...
    res = _run_gws(["tasks", "patch", "--params", json.dumps({"tasklist": tasklist, "task": task_id})], json_input=body)
    emit({"completed": res, "task_id": task_id, "tasklist": tasklist}, command="tasks complete")
...
def sync_bb(
    ...
    if from_audit:
        try:
            ...
            for ev in events[-5:]:
                ...
                body = {"title": title[:100], "notes": f"From audit.jsonl: {json.dumps(ev)[:800]}"}
                res = _run_gws(["tasks", "insert", "--params", json.dumps({"tasklist": tasklist})], json_input=body)
                if res.get("id"):
                    created.append(res)
        except Exception as e:
    ...
    starters = [
        {"title": "Wire Google Tasks into BigBang — bb tasks ↔ Hatch", "notes": "Done: tasks plugin with status/lists/list/add/complete. Next: add to ava route and agent bus."},
        {"title": "Generate LLM-wiki for BigBang CLI v0.4", "notes": "Create docs/llm-wiki/*.md for each plugin with cap-aware docs, then graphify build."},
        {"title": "Run graphify build over bigbang-cli → graphify-out", "notes": "Use personal-graphify to build graph.json + GRAPH_REPORT.md for token-efficient querying."},
    ]
    for s in starters:
        res = _run_gws(["tasks", "insert", "--params", json.dumps({"tasklist": tasklist})], json_input=s)
        if res.get("id"):
            created.append(res)
