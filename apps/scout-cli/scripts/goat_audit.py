...
COMMENTED_CODE_RE = re.compile(
    r"^\s*#\s*(?:def |class |return |import |if |for |while )"
)
...
def audit_plugin(name: str, run_tests: bool = False) -> dict:
    ...
    if undeclared:
        findings.append(
            f"D1: UNDECLARED third-party imports {undeclared} — the openswap premise "
    ...
    if home_hits:
        findings.append(
            f"D3: HOME-layout assumption at line(s) {home_hits[:4]} (portability)"
        )
    ...
    if tf.exists():
        ...
        if not imports_plugin:
            findings.append(
                f"D4: test_{name}.py never references the plugin — may pass with it deleted"
            )
        if asserts == 0:
            findings.append(f"D4: test_{name}.py has no assertions")
        d4 = _clamp(
            (10 if imports_plugin else 4)
            - (6 if asserts == 0 else 0)
        ...
        if run_tests:
            ...
            if not ok:
                findings.append(
                    f"D4: TESTS FAIL ({detail}) — scored 0; a passing "
        ...
        d4, tinfo = (
            0,
            {"exists": False, "test_fns": 0, "asserts": 0, "references_plugin": False},
        )
    ...
    if longest > 80:
        findings.append(
            f"D5: {longest_name}() is {longest} lines — candidate for a human read"
        )
    ...
    if not has_doc:
        findings.append(
            "D6: cli.py has no module docstring stating what it replaces / why"
        )
    ...
    if not cli.exists() or loc == 0:
        findings.insert(
            0,
            "INCOMPLETE: no cli.py implementation (scaffolding only) — "
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Automated Carmack/Bellard GOAT audit of scout-cli plugins."
    )
    ap.add_argument(
        "--plugin", action="append", help="audit only this plugin (repeatable)"
    )
    ap.add_argument("--json", action="store_true", help="machine-readable output")
    ap.add_argument(
        "--baseline",
        action="store_true",
        help="write the current state as the accepted baseline",
    )
    ap.add_argument(
        "--check",
        action="store_true",
        help="exit 1 only if a plugin REGRESSED vs the baseline",
    )
    ap.add_argument(
        "--run-tests",
        action="store_true",
    ...
    doc = {
        "tool": "goat_audit.py",
        "rubric": "Carmack/Bellard 1-10 (heuristic, mechanizable subset)",
        "count": len(reports),
        "plugins": reports,
    }
    ...
    if args.baseline:
        BASELINE.write_text(
            json.dumps({r["plugin"]: r["mean"] for r in reports}, indent=2) + "\n",
            encoding="utf-8",
        )
    ...
    if args.json:
        ...
        print(
            f"GOAT audit — {len(reports)} plugins (heuristic; regressions are the signal)\n"
        )
        for r in worst:
            s = r["scores"]
            print(
                f"  {r['mean']:5.2f}  {r['plugin']:<12} "
    ...
    if args.check:
        ...
        regressions = [
            (r["plugin"], base[r["plugin"]], r["mean"])
            for r in reports
            if r["plugin"] in base and r["mean"] < base[r["plugin"]] - 0.01
        ]
        new_bad = [
            r
            for r in reports
            if r["plugin"] not in base
            and args.min_mean is not None
            and r["mean"] < args.min_mean
        ]
        ...
        for r in new_bad:
            print(
                f"NEW PLUGIN BELOW BAR: {r['plugin']} mean {r['mean']} < {args.min_mean}",
                file=sys.stderr,
            )
            rc = 1
        if rc == 0:
