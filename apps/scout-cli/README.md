...
- **Bounded Recovery Ladder** — `dottie_loop.execution.recovery_ladder` (the one implementation; `pipeline/recovery_ladder.py` re-exports it) FailureTaxonomy 5 `INPUT_CORRUPTION/CONTEXT_STARVATION/TOOL_FAILURE/REASONING_COLLAPSE/OUTPUT_CORRUPTION` + SideEffect 4 `READ/WRITE_IDEMPOTENT/WRITE_DESTRUCTIVE/EXTERNAL_NOTIFY` + ladder `retry1→patch→replan→escalate`. Cannot skip levels. 28→315 features via enriched schedule binary gate rather than shipping coin-flip map.

- **Pacing Filter** — pacing constants in the route output (`tempo`, `max_concurrent_safe`) and `harness ops dashboard`'s `pacing_filter`: `max3 parallel / max4 concurrent safe`, `tempo :13 Never :00` timing over speed, sub-swarm 3-5 medium 13 only epic. CrewAI >5-6 noisy needs filtering. Agent-to-agent handoff is a message in jarvisd's one store (`jarvis.send` / `jarvis.inbox`, `docs/JARVISD_SPEC.md`) or `scout comms` (file bus). There is no `HandoffEnvelope` type in code; the `bundles/ultra/communication-pacing.js` this line used to cite never existed in this repo.
