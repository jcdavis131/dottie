...
def group_advantages(
    returns: Sequence[float], *, eps: float = _ADV_STD_EPS
) -> list[float]:
...
class EntropyThermostat:
    ...
    k_max: float = (
        4.0  # saturation on the integral term (bounds the widest trust region)
    )
    ...
    def update(self, h_policy: float) -> float:
        ...
        self.k = _clamp(
            self.k + self.kappa * (self.h_target - h_policy), 0.0, self.k_max
        )
        return self.k
...
def importance_weighted_entropy(
    logp_new: Sequence[float], logp_old: Sequence[float]
) -> float:
    ...
    if len(logp_new) != len(logp_old):
        raise ValueError(
            "logp_new and logp_old must be the same length (one per rollout token)"
        )
    weights = [math.exp(ln - lo) for ln, lo in zip(logp_new, logp_old, strict=False)]
    wsum = sum(weights)
    if wsum <= 0.0:
        return 0.0
    return sum(w * (-ln) for w, ln in zip(weights, logp_new, strict=False)) / wsum
...
def clipped_surrogate(
    ratio: float, advantage: float, *, lower: float, upper: float, r_outer: float
) -> SurrogateResult:
    ...
    inner_clipped = (
        clipped_obj < unclipped_obj
    )  # the clip branch was the binding (smaller) one
    return SurrogateResult(
        objective=objective, inner_clipped=inner_clipped, outer_clipped=outer_clipped
    )


def group_surrogate_objective(
    ...
    *,
    ...
    total = 0.0
    hits = 0
    for r, a in zip(ratios, advantages, strict=False):
...
class TraceBank:
    ...
    *diversity* matters more than per-prompt volume), (3) sample uniformly at random. No
    ...
    def _prompt_capped_pool(
        self, per_prompt_cap: int, rng: random.Random
    ) -> list[BankedTrace]:
    ...
    def recovery_sample(
        self, n: int, *, per_prompt_cap: int = 4, rng: random.Random | None = None
    ) -> list[BankedTrace]:
...
class EntropyRunTrace:
    ...
    collapsed_at: int | None  # first step H hit the floor, or None if it never did


def simulate_entropy_control(
    *,
    kappa: float,
    h_target: float = 0.3,
    h_start: float = 0.6,
    ...
    return EntropyRunTrace(
        entropy=entropy, k=ks, steps_in_band=steps_in_band, collapsed_at=collapsed_at
    )
