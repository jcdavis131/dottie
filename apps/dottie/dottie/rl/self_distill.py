...
def self_distill_filter(
    traces: list[dict], keep_success_only: bool = False, max_traces: int = 10000
) -> list[dict]:
...
def mix_with_mid_data(traces: list[dict], mid_data_frac: float = 0.15) -> list[dict]:
    ...
    mid_data = []
    n_mid = (
        int(len(traces) * mid_data_frac / (1 - mid_data_frac))
        if mid_data_frac < 1
        else 0
    )
    for i in range(n_mid):
        mid_data.append(
            {
                "prompt": f"Mid doc {i}",
                "response": f"Academic text {i} long context analysis...",
                "source": "mid_training",
                "length": 4000,
            }
        )
...
def run_self_distillation(n_traces: int = 10000, out_path: Path | None = None):
    ...
    print(
        f"Self-distill: collected {len(traces)} filtered {len(filtered)} mixed {len(mixed)} -> {out_path}"
    )
