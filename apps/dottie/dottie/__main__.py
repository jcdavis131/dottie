...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    cp = sub.add_parser("climb", help="run measured hill-climb iterations (gated, logged)")
    cp.add_argument("--families", default="mixed",
    ...
    crp = sub.add_parser("climb-report",
                         help="render climb_log.jsonl as a table + paired verdicts")
    ...
    if args.cmd == "climb":
        ...
        cfg = climb.ClimbConfig(
            families=args.families, n=args.n, seed_base=args.seed_base,
            backend=args.backend, max_steps=args.max_steps, use_skills=args.use_skills,
            evaluate=args.evaluate, train_step=args.train_step, compute=args.compute,
        ...
        try:
            for i in range(args.iterations):
                ...
                if prev is not None:
                    print(climb.render_verdict(
                        climb.compare_iterations(prev, record, tolerance=cfg.tolerance)))
                prev = record
        except DottiePolicyUnavailable as e:
            print(f"[dottie climb] backend unavailable (honest refusal, no fake data): {e}",
                  file=sys.stderr)
            return 2
