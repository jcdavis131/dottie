...
_PAST_VALIDATION_STATES = frozenset({
    "rejected", "sota", "failed_training", "ready_for_training",
    "evaluation_pending"})
...
_NODE_EVENTS = frozenset({
    "model_built", "phase_enter", "checkpoint", "done", "branch_forked",
    "resumed", "ckpt_rotated", "error", "crash"})
...
def ingest_trainer_metrics(store: GraphStore, path: str | Path,
                           run_key: Optional[str] = None) -> Dict[str, int]:
    ...
    n_events = 0
    n_steps = 0
    first_lm: Optional[float] = None
    last_lm: Optional[float] = None
    last_step: Optional[int] = None
    prev_event_id: Optional[str] = None
    current_phase: Optional[str] = None
    store.upsert_node(run_id, "run", f"training run {run_key}",
                      {"metrics_file": str(p)}, src, p.name)
    with p.open("r", encoding="utf-8", errors="replace") as fh:
        for lineno, line in enumerate(fh, 1):
            ...
            if name == "phase_enter":
                ...
                store.upsert_node(phase_id, "phase", pname,
                                  {"seq": ev.get("seq")}, src, ref)
                store.add_edge(eid, phase_id, "entered_phase", {}, src, ref)
                current_phase = phase_id
            elif name == "checkpoint":
                step = ev.get("step")
                ck_id = f"checkpoint:{run_key}:step_{step}"
                store.upsert_node(ck_id, "checkpoint", f"{run_key} step_{step}",
                                  {"step": step}, src, ref)
                ...
                if init:
                    ...
                    store.upsert_node(ck_id, "checkpoint", base, {"path": init},
                                      src, ref)
            ...
            prev_event_id = eid
    store.upsert_node(run_id, "run", "", {
        "n_step_events": n_steps, "last_step": last_step,
        "first_lm_loss": first_lm, "last_lm_loss": last_lm}, src, p.name)
    store.commit()
    ...
    * lm_loss > 8x the min of the previous 8 points (and above an absolute
    ...
    for i, (s, l) in enumerate(zip(steps, losses)):
        ...
        if prev_step is not None and s <= prev_step:
            out.append({"kind": "step_reemitted", "index": i, "step": s,
                        "lm_loss": l,
                        "note": "step value repeated/regressed -> restart or resume boundary"})
        if len(window) >= 4:
            floor = min(window[-8:])
            if l > max(0.5, 8.0 * floor):
                out.append({"kind": "loss_spike", "index": i, "step": s,
                            "lm_loss": l, "recent_min": floor})
...
def ingest_live_status(store: GraphStore, path: str | Path) -> Dict[str, int]:
    ...
    with p.open("r", encoding="utf-8", errors="replace") as fh:
        j = json.load(fh)
    counts = {"snapshots": 1, "sites": 0, "containers": 0, "events": 0,
              "anomalies": 0, "promotions": 0}
    ...
    store.upsert_node(snap_id, "snapshot", f"live status {snap_key}", {
        "source": j.get("source"), "hostname": j.get("hostname"),
        "run_id": j.get("run_id"), "preset": pipeline.get("preset"),
        "mode": pipeline.get("mode"), "disk_free_gb": pipeline.get("disk_free_gb"),
    }, src, p.name)
    ...
    if last:
        ...
        run_id = f"run:{run_key}"
        store.upsert_node(run_id, "run", f"live {preset} run", {
            "metrics_path": trainer.get("metrics_path"),
            "n_points": trainer.get("n_points"), "last_step": last.get("step"),
            "last_lm_loss": last.get("lm_loss"), "phase": last.get("phase"),
            "tok_s": last.get("tok_s"), "gpu_mem_mb": last.get("gpu_mem_mb"),
            "gpu_sm_mhz": last.get("gpu_sm_mhz"),
        }, src, f"{p.name}:pipeline.trainer.last")
        store.add_edge(snap_id, run_id, "observed", {}, src, p.name)
        phase_id = f"phase:p{last.get('phase')}"
        store.upsert_node(phase_id, "phase", f"p{last.get('phase')}", {}, src,
                          f"{p.name}:pipeline.trainer.last.phase")
        store.add_edge(run_id, phase_id, "in_phase", {}, src, p.name)
        series = trainer.get("series") or {}
        anomalies = _series_anomalies(series.get("step") or [],
                                      series.get("lm_loss") or [])
        prev_eid: Optional[str] = None
        for a in anomalies:
            eid = f"event:{run_key}:series_{a['index']}:{a['kind']}"
            ref = f"{p.name}:pipeline.trainer.series[{a['index']}]"
            store.upsert_node(eid, "event",
                              f"{a['kind']} @step {a['step']}", a, src, ref)
            store.add_edge(run_id, eid, "emitted", {}, src, ref)
            if prev_eid:
    ...
    for site in hub.get("sites") or []:
        ...
        mss = [h["ms"] for h in hist
               if isinstance(h, dict) and isinstance(h.get("ms"), (int, float))]
        store.upsert_node(sid, "site", str(site.get("url") or name), {
            "url": site.get("url"), "history_n": len(hist),
            "history_up_n": len(ups),
            "history_avg_ms": round(sum(mss) / len(mss), 1) if mss else None,
            "history_max_ms": max(mss) if mss else None,
        }, src, f"{p.name}:hub.sites")
        store.add_edge(snap_id, sid, "probed", {
            "http": site.get("http"), "ms": site.get("ms"), "up": site.get("up"),
        }, src, f"{p.name}:hub.sites")
        counts["sites"] += 1
    ...
    if isinstance(fleet, list):
        ...
        if isinstance(inner, list):
            ...
            entries = [dict(v, name=k) for k, v in fleet.items()
                       if isinstance(v, dict)]
    for e in entries:
        name = (e.get("name") or e.get("Name") or e.get("Names")
                or e.get("container"))
        if not name:
            continue
        ...
        store.upsert_node(cid, "container", str(name), props, src,
                          f"{p.name}:hub.fleet")
    ...
    for i, ev in enumerate(j.get("recent_events") or []):
        ...
        store.upsert_node(eid, "event", str(ev.get("message", ""))[:120], {
            "level": ev.get("level"), "source": ev.get("source"),
            "timestamp": ev.get("timestamp"),
        }, src, f"{p.name}:recent_events[{i}]")
    ...
    if baseline.get("metric_name"):
        ...
        store.upsert_node(bid, "baseline", baseline["metric_name"], {
            "value": baseline.get("metric_value"), "sem": baseline.get("metric_sem"),
            "provenance": baseline.get("provenance"),
            "notes": str(baseline.get("notes", ""))[:200],
        }, src, f"{p.name}:research.baseline")
        store.add_edge(snap_id, bid, "observed", {}, src, p.name)
    for s in research.get("sota_history") or []:
        if not isinstance(s, dict) or not s.get("id"):
            continue
        pid = f"promotion:{s['id']}"
        store.upsert_node(pid, "promotion", str(s.get("name", s["id"])), {
            "metric": s.get("metric"), "metric_name": s.get("metric_name"),
            "baseline_value": s.get("baseline_value"),
            "updated_ts": s.get("updated_ts"),
        }, src, f"{p.name}:research.sota_history")
        store.add_edge(pid, f"experiment:{s['id']}", "promoted", {}, src,
                       f"{p.name}:research.sota_history")
        if baseline.get("metric_name"):
            store.add_edge(pid, f"baseline:{baseline['metric_name']}",
                           "moved_baseline", {}, src, p.name)
        counts["promotions"] += 1
    store.commit()
    return counts
...
def ingest_ledger(store: GraphStore, ledger_path: str | Path) -> Dict[str, int]:
    ...
    for cid, pattern, hint_summary in taxonomy.FAILURE_CLASSES:
        ...
        h_id = f"hint:{cid}"
        store.upsert_node(fc_id, "failure_class", cid, {"pattern": pattern},
                          "taxonomy", "dottie/kg/taxonomy.py")
        store.upsert_node(h_id, "hint", hint_summary,
                          {"mirrors": "dottie.research.validate._HINTS"},
                          "taxonomy", "dottie/kg/taxonomy.py")
        store.add_edge(fc_id, h_id, "hinted_by", {}, "taxonomy",
                       "dottie/kg/taxonomy.py")
    store.upsert_node("failure_class:unclassified", "failure_class",
                      "unclassified", {"pattern": None}, "taxonomy",
                      "dottie/kg/taxonomy.py")
    for row in con.execute(
        ...
        store.upsert_node(exp_id, "experiment",
            ...
            "module_name": impl.get("module_name") if isinstance(impl, dict) else None,
        ...
        if isinstance(hist, list) and hist:
            final_ok = any(isinstance(h, dict) and h.get("ok") is True
                           for h in hist)
            ...
            for idx, cls, level, sig in traj:
                d = seen_classes.setdefault(cls, {
                    "attempt_indices": [], "level": level, "signature": sig})
                d["attempt_indices"].append(idx)
            for cls, d in seen_classes.items():
                last_idx = max(d["attempt_indices"])
                later_other = any(i > last_idx for i, c, _, _ in traj
                                  if c != cls)
                ...
                store.upsert_node(f"failure_class:{cls}", "failure_class", cls,
                                  {}, src, ref)
                store.add_edge(exp_id, f"failure_class:{cls}", "struggled_with", {
                    "attempt_indices": d["attempt_indices"],
                    "level": d["level"], "signature": d["signature"],
                    "cleared": cleared, "final_ok": final_ok}, src, ref)
        ...
        if failure:
            ...
            store.add_edge(exp_id, lvl_id, "died_at",
                           {"attempts": row["attempts"]}, src, ref)
            ...
            if isinstance(per_level, dict):
                texts += [str(v.get("detail", "")) for v in per_level.values()
                          if isinstance(v, dict)]
            corpus = "\n".join(texts)
            primary = taxonomy.primary_class(corpus)
            sig = taxonomy.normalize_signature(taxonomy.salient_line(corpus))
            store.add_edge(exp_id, f"failure_class:{primary}", "classified_as",
                           {"signature": sig, "primary": True}, src, ref)
            for extra in taxonomy.classify(corpus)[1:]:
                store.add_edge(exp_id, f"failure_class:{extra}", "classified_as",
                               {"primary": False}, src, ref)
            ...
            store.upsert_node(out_id, "outcome",
                              "validation resolved by self-correction "
                              "(failure class unlogged)", {}, src, ref)
            store.add_edge(exp_id, out_id, "resolved_by_correction",
                           {"attempts": row["attempts"]}, src, ref)
        ...
        if verdict_raw:
            ...
            if isinstance(v, dict) and "promote" in v:
                ...
                store.add_edge(exp_id, v_id, "evaluated", {
                    "metric": v.get("metric"), "delta": v.get("delta"),
                    "baseline_value": v.get("baseline_value"),
                    "new_value": v.get("new_value")}, src, ref)
    row = con.execute("SELECT * FROM baseline WHERE singleton=1").fetchone()
    if row is not None:
        bid = f"baseline:{row['metric_name']}"
        store.upsert_node(bid, "baseline", row["metric_name"], {
            "value": row["metric_value"], "sem": row["metric_sem"],
            "notes": str(row["notes"] or "")[:200]}, src, "ledger:baseline")
    con.close()
    store.commit()
    return counts
...
def ingest_steer(store: GraphStore, path: str | Path) -> Dict[str, int]:
    ...
    with p.open("r", encoding="utf-8", errors="replace") as fh:
        for lineno, line in enumerate(fh, 1):
            ...
            is_ack = (str(obj.get("type", "")).lower() == "ack"
                      or body.startswith("\U0001f916"))  # the 🤖 ack prefix
            ntype = "steer_ack" if is_ack else "steer_directive"
            nid = f"{ntype}:{key}"
            store.upsert_node(nid, ntype, body[:120], {
                "status": obj.get("status"), "ts": obj.get("ts"),
                "action": obj.get("action")}, src, ref)
            parent = obj.get("ack_of") or obj.get("in_reply_to")
            if is_ack and parent:
                store.add_edge(nid, f"steer_directive:{parent}", "acks", {},
                               src, ref)
...
def ingest_incidents(store: GraphStore, seed_path: str | Path,
                     docs_root: str | Path) -> Dict[str, int]:
    ...
    def _doc_text(rel: str) -> str:
        if rel not in doc_cache:
            fp = root / rel
            doc_cache[rel] = (fp.read_text(encoding="utf-8", errors="replace")
                              if fp.exists() else "")
    ...
    for inc in seed.get("incidents", []):
        ...
        store.upsert_node(iid, "incident", inc.get("title", key), {
            "class": inc.get("class"), "severity": inc.get("severity"),
            "date": inc.get("date"), "root_cause": inc.get("root_cause"),
            "quote": inc.get("anchor"), "anchor_verified": ok,
        }, src, ref)
        counts["incidents"] += 1
        ...
        if inc.get("resolution"):
            fid = f"fix:{key}"
            store.upsert_node(fid, "fix", inc["resolution"][:120], {
                "commit": inc.get("fix_commit"),
                "detail": inc.get("resolution")}, src, ref)
            store.add_edge(iid, fid, "resolved_by", {}, src, ref)
        if inc.get("failed_fix"):
            nfid = f"fix:{key}:failed"
            store.upsert_node(nfid, "fix", inc["failed_fix"][:120], {
                "outcome": "failed", "detail": inc["failed_fix"]}, src, ref)
        ...
        if inc.get("steer"):
            ...
            store.upsert_node(sid, "steer_directive", inc["steer"][:120], {
                "pending": True, "anchor_verified": steer_ok}, src, steer_ref)
    ...
    for pol in seed.get("policies", []):
        ...
        counts["verified" if ok else "unverified"] += 1
        store.upsert_node(pid, "policy", pol.get("title", key), {
            "quote": pol.get("anchor"), "anchor_verified": ok,
            "rule": pol.get("rule")}, src, ref)
        counts["policies"] += 1
        for target in pol.get("governs", []):
            store.upsert_node(target, target.split(":", 1)[0],
                              target.split(":", 1)[1], {}, src, ref)
