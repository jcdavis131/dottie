...
_DEFAULT_GRAPH = Path(__file__).resolve().parents[2] / "data" / "kg" / "graph.sqlite3"
...
def hint_efficacy(store: GraphStore, class_id: str) -> dict[str, Any]:
    ...
    hints = [
        store.node(e["dst"])
        for e in store.edges_from(fc_id)
        if e["type"] == "hinted_by"
    ]
    died = []
    for e in store.edges_to(fc_id):
        ...
        if exp:
            died.append(
                {
                    "id": exp["id"],
                    "state": exp["props"].get("state"),
                    "attempts": exp["props"].get("attempts"),
                    "signature": e["props"].get("signature"),
                    "source_ref": e["source_ref"],
                }
            )
    encounters = [e for e in store.edges_to(fc_id) if e["type"] == "struggled_with"]
    ...
    return {
        ...
        "clearance_rate": (
            round(len(cleared) / len(encounters), 3) if encounters else None
        ),
        "cleared_by": [e["src"] for e in cleared],
        "n_died_matching": len(died),
        "died": died,
        "caveat": (
            "per-attempt classes are re-derived from validation.history "
...
def cmd_hints(store: GraphStore, args: argparse.Namespace) -> None:
    ...
    print(
        f"encounters    : {out['n_encounters']} correction trajectories hit "
    ...
    for src in out["cleared_by"][: args.limit]:
        print(f"    cleared: {src}")
    print(
        f"died matching : {out['n_died_matching']} experiments "
        f"(final failure in this class)"
    )
    for d in out["died"][: args.limit]:
        print(
            f"    {d['id']}  state={d['state']} attempts={d['attempts']}  "
            f"[{d['source_ref']}]"
        )
        if d.get("signature"):
...
def cmd_refine(store: GraphStore, args: argparse.Namespace) -> None:
    ...
    print(
        f"candidate NEW hint classes (unclassified clusters, >= "
...
def cmd_incidents(store: GraphStore, args: argparse.Namespace) -> None:
    ...
    for inc in incidents:
        p = inc["props"]
        print(
            f"{inc['id']}  [{p.get('severity')}/{p.get('class')}]  "
...
def cmd_preceded(store: GraphStore, args: argparse.Namespace) -> None:
    ...
    if node is None:
        ...
        if len(matches) == 1:
            ...
            print(
                f"node {args.node_id!r} not found."
    ...
    for i, n in enumerate(chain, 1):
        print(
            f"  -{i} before: {n['id']}  {_fmt_props(n['props'])}  [{n['source_ref']}]"
        )
...
def cmd_sites(store: GraphStore, _args: argparse.Namespace) -> None:
    for s in store.nodes_by_type("site"):
        ...
        print(
            f"{s['id']:14s} up_history={p.get('history_up_n')}/{p.get('history_n')}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not graph_path.exists():
        print(
            f"graph not found at {graph_path} — run `python -m dottie.kg.build` first"
        )
        return 2
