...
class Experiment:
    ...
    eval_verdict: dict[str, Any] | None = None
...
class Baseline:
    ...
    metric_sem: float | None = None
    metric_sem_n: int | None = None
...
class Ledger:
        ...
        with self._conn() as c:
            c.execute(
                ...
                (
                    ...
                    float(baseline.metric_value),
                    1 if baseline.higher_is_better else 0,
                    ...
                    None
                    if baseline.metric_sem_n is None
                    else int(baseline.metric_sem_n),
                    None
                    if baseline.per_seed is None
    ...
    def get_baseline(self) -> Baseline | None:
        ...
        return Baseline(
            ...
            notes=row["notes"],
            metric_sem=row["metric_sem"],
            metric_sem_n=row["metric_sem_n"],
    ...
    def promote_baseline(
        ...
        *,
        ...
        metric_sem: float | None = None,
        metric_sem_n: int | None = None,
        per_seed: builtins.list[float] | None = None,
        ts: float | None = None,
    ) -> Baseline:
        ...
        new = Baseline(
            metric_name=cur.metric_name,
            metric_value=float(metric_value),
            higher_is_better=cur.higher_is_better,
            architecture=architecture or cur.architecture,
            experiment_id=experiment_id,
            updated_ts=ts if ts is not None else time.time(),
            notes=notes,
            metric_sem=metric_sem,
            metric_sem_n=metric_sem_n,
            per_seed=per_seed,
        )
    ...
    def create(
        self, hypothesis: dict[str, Any], *, ts: float | None = None
    ) -> Experiment:
    ...
    def next_in_state(self, state: str) -> Experiment | None:
    ...
    def set_fields(
        self, experiment_id: str, *, ts: float | None = None, **fields: Any
    ) -> Experiment:
    ...
    def _write(
        self, experiment_id: str, state: str, *, ts: float | None = None, **fields: Any
    ) -> Experiment:
    ...
    def _row(row: sqlite3.Row) -> Experiment:
        return Experiment(
            ...
            updated_ts=row["updated_ts"],
            hypothesis=_loads(row["hypothesis"]) or {},
            implementation=_loads(row["implementation"]),
            workspace=row["workspace"],
            train_metrics=_loads(row["train_metrics"]),
            eval_verdict=_loads(row["eval_verdict"]),
            writeup=row["writeup"],
            failure=row["failure"],
