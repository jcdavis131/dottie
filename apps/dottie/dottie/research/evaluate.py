...
def parse_ab_verdict(output: str) -> str | None:
...
def subprocess_ab_runner(script_path: str | Path) -> tuple[int | None, str]:
    ...
    try:
        timeout_s = float(
            os.environ.get("DOTTIE_AB_GATE_TIMEOUT_S", _AB_GATE_TIMEOUT_S_DEFAULT)
        )
    except ValueError:
...
def _multi_seed_gate(
    ledger: Ledger,
    exp: Experiment,
    *,
    ab_runner: Callable[..., tuple[int | None, str]] | None,
    promotions_root: str | Path | None = None,
) -> dict[str, Any]:
...
def _baseline_contamination(ledger: Ledger, baseline: Baseline) -> str | None:
    ...
    try:
        ...
        return (
            f"the experiment that set this baseline ({baseline.experiment_id}) is no "
    ...
    if not code:
        return (
            f"the experiment that set this baseline ({baseline.experiment_id}) stored "
    ...
    try:
        ...
        res = _validate.validate(
            code,
            class_name=dry.get("class_name"),
        ...
        return (
            f"could not re-validate the baseline's source experiment "
    ...
    if res.ok:
        ...
        skipped = [
            lvl
            for lvl, info in (res.per_level or {}).items()
            if info.get("status") == "skipped"
        ]
        if skipped:
            return (
                f"baseline validity UNVERIFIED — re-checking the experiment that set it "
        ...
        return None
    return (
        f"CONTAMINATED BASELINE — the experiment that set it ({baseline.experiment_id}, "
...
def _baseline_capacity_caveat(ledger: Ledger, baseline: Baseline) -> str | None:
    ...
    if (
        ...
        or not isinstance(replaced, int)
        or delta >= 0
        or replaced <= 0
    ):
        return None
    ...
    if fraction < _CAPACITY_REMOVAL_FRACTION:
        return None
    return (
        f"CAPACITY-CONFOUNDED BASELINE — the experiment that set it "
...
def _spread(metrics: dict[str, Any]) -> dict[str, float] | None:
...
def run_evaluation(
    ...
    *,
    ...
    ts: float | None = None,
    ab_runner: Callable[..., tuple[int | None, str]] | None = None,
    promotions_root: str | Path | None = None,
) -> dict[str, Any] | None:
    ...
    paired_diffs: list[float] | None = None
    cand_ps = metrics.get("per_seed")
    base_ps = getattr(baseline, "per_seed", None)
    if (
        isinstance(cand_ps, (list, tuple))
        and isinstance(base_ps, (list, tuple))
        and len(cand_ps) == len(base_ps)
        and len(cand_ps) >= 2
    ):
        try:
        ...
        if diffs is not None and all(math.isfinite(x) for x in diffs):
            paired_diffs = diffs
    if sp is None:
        significant, sig_note = (
            None,
            "no per-batch series recorded — significance unmeasurable",
        )
    else:
        ...
        if paired_diffs is not None:
            ...
            basis = (
                f"PAIRED per-seed SE {se_diff:.5g} over n={n_p} shared seeds "
            ...
            basis = (
                f"two-sample SE_diff {se_diff:.5g} "
                f"(candidate SEM {sp['sem']:.5g}, baseline SEM {float(base_sem):.5g})"
            )
        else:
            se_diff = sp["sem"]
            basis = (
                f"candidate-only SEM {sp['sem']:.5g} — the baseline records NO spread, "
        ...
        if sp["series"] in _WITHIN_RUN_SERIES:
            basis += (
                f" [spread from '{sp['series']}' — a SINGLE run's batch-to-batch "
        ...
        sig_note = (
            f"{verdict_word}: |delta| {abs(delta):.5g} vs {SIGNIFICANCE_SEM}× "
    ...
    capacity_gated = (
        isinstance(block_delta, int)
        and isinstance(replaced_params, int)
        and replaced_params > 0
        and block_delta < 0
    ...
    would_promote = (
        improved
        and (stable if require_stable else True)
    ...
    if would_promote and not multi_seed_evidence:
        seed_gate = _multi_seed_gate(
            ledger, exp, ab_runner=ab_runner, promotions_root=promotions_root
        )
    ...
    verdict = {
        ...
        "seed_gated": seed_gated,
        "seed_gate": seed_gate,
        "significant": significant,
        "significance": sig_note,
        "baseline_provenance": base_kind,
        "baseline_caveat": base_caveat,
        ...
        "block_param_delta": block_delta,
        "capacity_caveat": capacity_note,
        "metric": baseline.metric_name,
    ...
    if not improved:
        ...
        reason = (
            f"improvement within noise — held ({sig_note})"
            if significant is False
            else f"improvement unverifiable — held ({sig_note})"
        )
    elif capacity_gated:
        reason = (
            f"capacity-gated — the swap deleted {-block_delta:,} of the replaced "
        ...
        reason = (
            f"multi-seed gate — significance rests on within-run spread only "
