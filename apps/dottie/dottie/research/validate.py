...
def _extra_required_forward_args(
    ...
    extra += [
        k.arg for k, d in zip(a.kwonlyargs, a.kw_defaults, strict=False) if d is None
    ]
    return extra
...
def dry_run_module(
    ...
    *,
    ...
    shape = [
        ...
        for d, dflt in zip(raw, (4, 16, 64), strict=False)
    ]
    ...
    return ValidationResult(
        True, "dry_run", "pass", f"forward ok on input {shape} -> {tuple(out_t.shape)}"
    )
