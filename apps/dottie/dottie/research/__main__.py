...
def cmd_calibrate_baseline(args) -> int:
    ...
    b = Baseline(
        ...
        metric_value=measured[FACTORY_METRIC],
        higher_is_better=False,
        architecture=measured["preset"],
        experiment_id=None,
        updated_ts=__import__("time").time(),
    ...
    _emit(
        {
            ...
            "note": (
                "baseline updated from this real measurement"
                if args.overwrite or eff.metric_value == measured[FACTORY_METRIC]
            ...
            if action == "evaluate":
                ...
                if rec["result"] and rec["result"].get("state") == "sota":
                    ...
                    rec["promotion"] = promote.build_promotion(
                        ...
                        out_root=paths.workspace_root(args.data_dir).parent
                        / "promotions",
                    )
            elif action == "train":
                tcfg: dict[str, Any] = {"steps": args.steps}
