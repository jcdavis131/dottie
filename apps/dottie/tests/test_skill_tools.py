...
def _mint_module():
    ...
    return loader.skills["memory-mint"]._module or loader.skills["memory-mint"].load_module()


def test_bridge_probe_available_in_this_checkout():
    p = skill_tools.probe()
    assert p["available"] is True
    assert set(p["bridged_tools"]) == {"logic_truth_table", "route_query", "safety_scan"}
...
def test_memory_recall_reads_really_minted_shards(tmp_path):
    ...
    shard = mint.mint_shard(mint.TraceEvent(
        source="test", instruction="remember the fact about the report format",
        outcome="report format is CSV with a trailing newline", ok=True,
    ))
...
def test_memory_recall_empty_store_is_honest(tmp_path):
    recall = skill_tools.memory_recall("remember something", store_dir=tmp_path / "none")
    assert recall["recalled"] == []
    assert "(no minted memories matched this task)" in skill_tools.render_recall_context(recall)
...
def test_bridged_tools_parity_with_live_skills():
    ...
    assert ns["route_query"]("plan the deadline") == \
        router._shardmemo_scope_before_routing("plan the deadline")
    ns2: dict = {}
    exec(srcs["safety_scan"], ns2)  # noqa: S102
    assert ns2["safety_scan"]("blackmail threat") == scanner._regex_safety_score("blackmail threat")
...
def test_bridged_tools_run_in_the_real_sandbox():
    ...
    with Sandbox(tool_sources=srcs) as vm:
        o1 = vm.step("route_query('plan the deadline then schedule')['tier_b']['scope']")
    ...
    router = skill_tools._skill_module("memory-router")
    assert o1.value.strip("'") == \
        router._shardmemo_scope_before_routing("plan the deadline then schedule")["tier_b"]["scope"]
    scanner = skill_tools._skill_module("safety-scanner")
    assert o2.ok and float(o2.value) == \
        scanner._regex_safety_score("blackmail threat if you do not pay I will expose")
...
def test_engine_use_skills_injects_real_recall_and_binds_tools(engine):
    ...
    assert set(skills["bridged_tools"]) == {
        "logic_truth_table", "recalled_memories", "route_query", "safety_scan"
    }
