...
MUTANTS = [
    (
        ...
        "gate ignores the baseline's recorded SEM",
    ),
    ...
    (
        ...
        "contamination verdict suppressed",
    ),
    (
        "implementation.py",
        '    for entry in reversed(getattr(outcome, "history", []) or []):',
        "    for entry in []:",
        "corrector_failure_is_distinguished",
        "corrector error never found in history",
    ),
    (
        "evaluate.py",
        '                                metric_sem=None if sp is None else sp["sem"],',
        "                                metric_sem=None,",
        "records_the_baselines_spread",
        "promotion drops the winning run's spread",
    ),
    (
        "validate.py",
        "        if in_spread > 1e-6 and out_spread <= 1e-6:",
        "        if False:",
        "rank_collapsing_block",
        "rank-collapse gate disabled",
    ),
    (
        "validate.py",
        ...
        '                False, "dry_run", "fail",\n'
        '                "no learnable parameters:',
        "zero_parameter_block",
        "zero-parameter gate disabled",
    ),
    (
        "validate.py",
        "        if tuple(out.shape) != tuple(x.shape):",
        "        if False:",
        "shape_change_that_only",
        "residual-stream shape check disabled",
    ),
    (
        "validate.py",
        ...
        "                           requires_grad=False)",
        "reading_input_grad",
        "probe input no longer requires grad (the whole point)",
    ),
    ...
    (
        ...
        "sequence_sized_parameter",
        "integration probe stops using the real seq (R28)",
    ),
    (
        "train.py",
        ...
        "    pool = finals or pys",
        "    finals = []\n    pool = finals or pys",
        "loads_the_validated_module",
        "loader stops preferring the real module (R49)",
    ),
    (
        "__main__.py",
        "    if avail is None or avail >= floor:",
        "    if True:",
        "memory_guard_refuses",
        "memory guard never refuses (R52)",
    ),
    (
        "ideation.py",
        ...
        "failing_dump",
        "dump failure escapes and masks the parse error (R58)",
    ),
    (
        "train.py",
        ...
        '                                                 "detail": "candidate module not loadable"},',
        "unloadable_candidate",
        "load failure treated as retryable infra again",
    ),
]
