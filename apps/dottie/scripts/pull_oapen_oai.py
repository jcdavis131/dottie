...
def oai_records(max_pages: int):
    ...
    for _ in range(max_pages):
        url = (
            f"{OAI}?verb=ListRecords&resumptionToken={urllib.parse.quote(token)}"
            if token
            else f"{OAI}?verb=ListRecords&metadataPrefix=dim"
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="OAI-PMH harvest of openly-licensed OAPEN books."
    )
    ...
    ap.add_argument(
        "--max-pages", type=int, default=400, help="OAI pages scanned (88 records/page)"
    )
    args = ap.parse_args()

    out_dir = Path(args.out)
    out_dir.mkdir(parents=True, exist_ok=True)
    jf = (out_dir / "oapen_open_books.jsonl").open(
        "w", encoding="utf-8"
    )  # incremental — checkpoint-safe
    n_books = 0
    seen: set[str] = set()
    ...
    try:
        for handle, langs, rights in oai_records(args.max_pages):
            ...
            row = {
                ...
                "nc": bool(reason == "NonCommercial (allowed via --allow-nc)"),
        ...
        json.dumps(
            {
                ...
                "classification": "REAL",
                "policy": {
                    "included": ["CC-BY", "CC-BY-SA", "CC0"]
                    + (["CC-BY-NC*"] if args.allow_nc else []),
                    "excluded": "*-ND, non-English, no CC license, duplicate content",
                    "full_text": args.full,
                },
                "counts": {
                    "books": n_books,
