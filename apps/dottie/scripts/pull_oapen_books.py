...
def is_safe(code: str | None, allow_nc: bool) -> tuple[bool, str]:
    ...
    if code in NC:
        return (
            (True, "NonCommercial (allowed via --allow-nc)")
            if allow_nc
            else (False, "NonCommercial")
        )
...
def gate_rights(
    rights_values: list[str] | None, allow_nc: bool
...
def search_page(offset: int, limit: int) -> list[dict]:
    ...
    url = (
        f"{OAPEN}/rest/search?query={q}&limit={limit}&offset={offset}"
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Pull openly-licensed OA books from OAPEN."
    )
    ap.add_argument(
        "--target", type=int, default=10, help="training-safe books to collect"
    )
    ...
    ap.add_argument(
        "--allow-nc", action="store_true", help="also include NonCommercial CC books"
    )
    ap.add_argument("--page", type=int, default=50, help="OAPEN search page size")
    ap.add_argument(
        "--max-pages", type=int, default=40, help="safety cap on pages scanned"
    )
    args = ap.parse_args()
    ...
    seen_sha: set[str] = (
        set()
    )  # by content — OAPEN serves the same book under multiple handles
    ...
    for page in range(args.max_pages):
        ...
        for item in items:
            ...
            if not is_english(md):  # client-side language filter (see _SEARCH_QUERY)
                ...
                continue
            included, code, license_url, reason = gate_rights(
                md.get("dc.rights"), args.allow_nc
            )
            if not included:
            ...
            if not text or not text.strip():
                excluded["no .pdf.txt bitstream"] = (
                    excluded.get("no .pdf.txt bitstream", 0) + 1
                )
                continue
            text = text.strip()
            ...
            if (
                sha in seen_sha
            ):  # same book under a different handle — skip the duplicate
            ...
            rows.append(
                {
                    ...
                    "nc": code in NC,
                    "publisher": _first(md, "publisher.name")
                    or _first(md, "oapen.imprint"),
                    "subjects": md.get("dc.subject.other")
                    or md.get("dc.subject.classification")
                    or [],
            ...
            print(
                f"  + [{code}] {str(_first(md, 'dc.title'))[:60]} "
                f"({len(text):,} chars)",
                file=sys.stderr,
            )
    ...
    print(
        ...
        f"({', '.join(f'{k}={v}' for k, v in sorted(license_hist.items()))})",
        file=sys.stderr,
    )
    print(
        f"scanned {scanned} records; excluded: "
