...
export default function Home() {
  ...
  return (
    <div className="site">
      <style>{`
        .site{
          --bg:#fcfcf8; --surface:#fff; --line:#e7e5e0; --line2:#ece9e3;
          --ink:#141210; --ink2:#6b6a64; --ink3:#9b9a95;
          --max: 980px; --r:16px; --r-lg:20px;
          min-height:100vh; background:var(--bg); color:var(--ink);
          font-family: ui-sans-system,-apple-system,BlinkMacSystemFont,"SF Pro Text",Inter,sans-serif;
          -webkit-font-smoothing:antialiased;
        }
        @media(prefers-color-scheme:dark){
          .site{--bg:#0f0e0d; --surface:#1a1816; --line:#2a2825; --line2:#24211e; --ink:#f5f3f0; --ink2:#a8a5a0; --ink3:#7a7874}
        }
      ...
      <div className="top mono">
        <div className="top-in">
          ...
          <div style={{display:"flex", gap:8, alignItems:"center"}}>
            <span className="pill">solo · MIT · no employer</span>
      ...
      <div className="wrap">
        <div className="hero">
          <div>
            ...
            <p className="lede">
              One machine that gathers data, cleans it, learns from it, and talks back.
              No team, no big budget — just a box getting a little better each day. This site is the control plane.
            </p>
            <div className="btns">
              <a className="b primary" href="https://github.com/jcdavis131/dottie">GitHub — dottie</a>
              <a className="b" href="/starter">Starter v5.1 →</a>
              <a className="b" href="https://raw.githubusercontent.com/jcdavis131/dottie/main/apps/dottie/README.md">Readme</a>
            </div>
            <div className="mono" style={{marginTop:12, fontSize:"11px", color:"var(--ink3)"}}>
              checksum {String(stats.checksum||"a3f9c1e2").slice(0,8)} · {String(stats.timestamp||"").slice(0,10) || "2026-08-05"} · local-first
            </div>
          </div>
          ...
          <div className="stack">
            <div className="card">
              ...
              <div className="grid2 mono">
                <div className="fact"><b>{papers}</b><span>papers</span></div>
                <div className="fact"><b>{nodes}</b><span>nodes</span></div>
                <div className="fact"><b>{people}</b><span>people</span></div>
                <div className="fact"><b>{edges}</b><span>edges</span></div>
              </div>
              <div className="mono" style={{marginTop:10, fontSize:"11px", color:"var(--ink2)"}}>
                {stats.cache || "graph cache 84% hit"} · {stats.token_saving || "71.5× via Graphify"}
              </div>
            </div>

            <div className="card">
              <h3 className="mono">How it works — plain English</h3>
              <div style={{display:"grid", gap:"8px", fontSize:"13.5px", lineHeight:1.5, color:"var(--ink2)"}}>
                <div><b style={{color:"var(--ink)"}}>1. Gathers</b> — pulls text steady, not fast. Hatch VM 500k every 4h, Alienware 10M chunks when free.</div>
                <div><b style={{color:"var(--ink)"}}>2. Cleans</b> — dedupes, splits 92/6/2, grades locally. No vector DB, just files you can read.</div>
                <div><b style={{color:"var(--ink)"}}>3. Learns & serves</b> — trains from its own traces, keeps last good checkpoint, serves chat when box is on.</div>
              </div>
              <div className="code mono">
                <div style={{opacity:.6}}># the loop — no tricks</div>
                <div>tasks → traces.jsonl → rft + memories</div>
                <div>→ eval gate → train step → better ckpt</div>
                <div style={{marginTop:6, opacity:.8}}>ollama qwen3:32b thinks today — ava still training</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="section">
          <div className="tri">
            <div className="card">
              <h4>Why Dottie not just Prime Agent?</h4>
              <p>Prime says code is a variable you can refine. Dottie adds the messy bit — actually training a tiny model from its own traces so it improves while you sleep. Solo, free-tier, honest 503 if down.</p>
            </div>
            <div className="card">
              <h4>What you see is what runs</h4>
              <p>No fake numbers. If Ollama is down, Dottie says so. If checkpoint is noise, it says noise. Every metric is computed from real inputs, every habit needs evidence.</p>
            </div>
            <div className="card">
              <h4>Use it</h4>
              <p><span className="mono" style={{background:"var(--bg)", padding:"2px 6px", border:"1px solid var(--line2)", borderRadius:6, fontSize:"11px"}}>pip install -e apps/dottie</span> gives you CLI. <span className="mono" style={{background:"var(--bg)", padding:"2px 6px", border:"1px solid var(--line2)", borderRadius:6, fontSize:"11px"}}>dottie repl</span> stays warm, missions pause Monday resume Thursday with receipts. Starter kit at <a href="/starter">/starter</a> → 10 sec clone.</p>
            </div>
          </div>
        </div>
        ...
        <div className="foot mono">
          ...
          <div>Vercel + GitHub raw + local box when on</div>
        </div>
      </div>
    </div>
  );
