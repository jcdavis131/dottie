...
for r in rows:
    ...
    for h in hist:
        if h.get("ok") is False and "detail" in h:
            ...
            has_hint = kind == "existing" or (
                h.get("level") == "dry_run" and "Traceback" in det
            )
            if not has_hint:
                no_hint_attempts += 1
...
print(
    f"attempts that today receive NO hint at all: {no_hint_attempts} ({100 * no_hint_attempts / total:.1f}%)"
)
ex_total = sum(n for (k, _), n in attempt_c.items() if k == "existing")
print(
    f"attempts covered by a targeted existing hint: {ex_total} ({100 * ex_total / total:.1f}%)"
)
