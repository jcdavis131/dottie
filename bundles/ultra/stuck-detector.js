...
export function detectStuck(input) {
  ...
  if (nodeIds.length) {
    ...
    for(let i=sliceStart;i<nodeIds.length;i++){
      ...
      if(st) { if(['ok','completed','success','done','passed'].includes(st)) hm.oks++; else if(st) hm.fails++; }
      statusArr.push(st);
    }
    for (const [id,c] of Object.entries(counts)) {
      if(c>StuckThresholds.loopRepeats) {
        ...
        const healthyRepetition = distinctRuns>=Math.min(c,3) && fails===0 && (oks===0 ? true : oks>=c-1);
