...
def test_wsm_merger_add_and_merge():
    ...
    merger = mod.WSMCheckpointMerger(buffer_size=3, ema_decay=0.9, merge_every=2, save_dir="/tmp/wsm_test")
    # add dummy state dicts
    sd1 = {"a": __import__("torch").tensor([1.0,2.0]), "b": __import__("torch").tensor([3.0])}
    sd2 = {"a": __import__("torch").tensor([2.0,3.0]), "b": __import__("torch").tensor([4.0])}
...
def test_wsm_should_merge_logic():
    ...
    assert merger.should_merge(10) is False or True  # needs buffer >=2; empty -> False, we just test call
