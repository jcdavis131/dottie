...
try:
    ...
    # auto-generated test gap mapper for policy - coverage <80%
    # Original target: apps.ava-factory.policy
    target_module = import_module("apps.ava-factory.policy")
except Exception:
    try:
        from importlib import import_module
        target_module = import_module("policy")
    except Exception:
        try:
            target_module = import_module("pipeline.policy")
        except Exception:
            target_module = None
