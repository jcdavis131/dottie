...
if "dottie.datagen.chat_safety" not in sys.modules:
    ...
    class ChatSafetyGenerator:
        def __init__(self, seed=0): self.name="chat_safety"
        def generate(self, n):
            for i in range(2):
                yield {"doc_id": f"cs{i}", "text": "hello safety", "task_type":"deliberate","concept":"safety","phase":"p5","source":"chat_safety"}
    ...
    class ReactToolsGenerator:
        def __init__(self, seed=0): self.name="react_tools"
        def generate(self, n):
            for i in range(2):
                yield {"doc_id": f"rt{i}", "text": "react tool", "task_type":"automatic","concept":"tool","phase":"p3","source":"react"}
    ...
    for name in ["compress_trace","db_trace","trace_common"]:
        ...
        if name=="trace_common":
            ...
            class G:
                ...
                def generate(self, n):
                    yield {"doc_id":"etcot","text":"trace text","task_type":"deliberate","concept":"etcot","phase":"p5","source":"etcot"}
    ...
    class Manifest:
        ...
        def freeze_tokenizer(self,*a,**k): pass
        def add_shard(self,*a,**k): return "shard"
    ...
    pack_mod.pack_docs = lambda docs, lt: ([1,2,3], {"tokens": len(docs)*10, "docs": docs, "tokenizer_sha":"abc123"})
    pack_mod.write_shard = lambda arr, idx, bin_path: pathlib.Path(bin_path).write_text("bin") if isinstance(bin_path, pathlib.Path) else open(bin_path,"w").write("bin")
...
def test_load_distilled_docs_if_present(tmp_path):
    ...
    jf.write_text('{"task_id":"t1","category":"code","source":"src","text":"hello world"}\n')
