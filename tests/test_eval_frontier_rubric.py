...
def test_frontier_task_creation():
    ...
    task = FrontierTask(
        ...
        context_docs=[{"text": "doc"}],
        expected_workflow=["step1"],
        rubrics=[r],
...
def test_judge_prompt_contains_criterion():
    r = Rubric(
        id="r1",
        category="Accuracy",
        criterion="Must mention revenue 2023",
        weight=1.0,
        eval_instructions="inst",
        ground_truth_ref="doc p.3",
        required=True,
    )
...
def test_parse_score_extracts_float():
    ...
    assert (
        mod._parse_score("Score 0.5 something") == 0.5 or mod._parse_score("0.5") == 0.5
    )
