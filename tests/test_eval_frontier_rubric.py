...
try:
    ...
    # auto-generated test gap mapper for eval_frontier_rubric - coverage <80%
    # Original target: apps.ava-factory.eval_frontier_rubric
    target_module = import_module("apps.ava-factory.eval_frontier_rubric")
except Exception:
    try:
        from importlib import import_module
        target_module = import_module("eval_frontier_rubric")
    except Exception:
        try:
            target_module = import_module("pipeline.eval_frontier_rubric")
        except Exception:
            target_module = None
