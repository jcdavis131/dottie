...
def test_single_workspace_init_state():
    w = mod.SingleWorkspace(
        d_model=16, slots=4, target_hl=8, vocab_size=100, name="S1", num_heads=2
    )
...
def test_single_workspace_decay_factor_range():
    w = mod.SingleWorkspace(
        d_model=16, slots=2, target_hl=30, vocab_size=50, name="Critic"
    )
    df = w.decay_factor()
...
def test_single_workspace_forward_chunk():
    ...
    w = mod.SingleWorkspace(
        d_model=16, slots=2, target_hl=8, vocab_size=50, name="S1", num_heads=2
    )
    B, L, D = 2, 8, 16
