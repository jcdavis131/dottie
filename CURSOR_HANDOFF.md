...
| `bluehenre-checks` | hub_registry.json embedded `bytes`/`sha256` of WINDOWS CRLF bytes while HEAD stores LF. `--check` compared like-with-like locally and passed | ⏳ **c4b916c — NOT YET CONFIRMED IN CI.** ecfec65 claimed this fixed and was WRONG (scoped `-text` per-extension; `tasks/artifacts/ledger_retroflag.md` also gets hashed, 2380 vs 2344 bytes). c4b916c scopes by tree: `tasks/artifacts/** -text`. **Verify before trusting:** `gh run list --workflow CI --limit 3`. Local `--check` is NOT evidence here — it passed after the broken fix too, because it compares the worktree against an artifact generated FROM that worktree. The real assertion: enumerate every path the registry references and byte-compare each to `git show HEAD:` — must be 0 differing. |
