...
- `bb tasks` as MCP tool: expose `bb_tasks_list`, `bb_tasks_add`, `bb_tasks_complete` via `bb mcp serve --port 8787` SSE
- Bidirectional sync: `bb family brain` → creates Google Task when kid school event detected via Gmail
- Vector Hoops MTNN drift detection → auto task `Retrain MTNN v5_concat_b2_h160_t32_d48_mlp128` via `bb tasks add`
- Turnover Shield SaaS churn event → `bb tasks add "Follow up $CLIENT - churn risk"` via Stripe webhook simulation
