...
def generate_report(G: nx.MultiDiGraph, output_path: Path, god_nodes_list, surprises, comm_summaries, token_stats_dict):
    lines = []
    ...
    for nid, data in god_nodes_list:
        ...
        lines.append(f"- **{data.get('label', nid)}** ({data.get('type','')}) — degree {deg} — file `{data.get('file','')}` — community {data.get('community',0)}")
    lines.append("")
    lines.append("## Communities")
    lines.append("")
    for comm in comm_summaries[:15]:
        lines.append(f"- **Community {comm['id']}** — {comm['size']} nodes — types {comm['types']} — sample: {', '.join(comm['sample_labels'][:5])}")
    lines.append("")
    ...
    for s in surprises[:12]:
        ...
        lines.append(f"- `{src_data.get('label', s['source'])}` [{s['data'].get('type')}] → `{tgt_data.get('label', s['target'])}` — [{s['data'].get('confidence')}] — files differ? {s['file_diff']} — communities {s['cross']}")
    lines.append("")
    ...
    lines.append("")
    rationale_nodes = [(nid,d) for nid,d in G.nodes(data=True) if d.get("type")=="rationale"]
    for nid, data in rationale_nodes[:15]:
        lines.append(f"- `{data.get('label')}` @ {data.get('file')}:{data.get('line')} — explains nearby code")
    if not rationale_nodes:
        lines.append("- No # NOTE / # WHY comments found. Consider adding them — they become first-class graph nodes linked to code.")
    lines.append("")
    lines.append("## Personal Ecosystem Overlay")
    lines.append("")
    lines.append("- **Family Brain**: Joint accounts, Betterment buckets, Plaid 5 institutions, Emergency $136.5k")
    lines.append("- **Passive Lab**: Turnover Shield $79-$149/mo, 7-13 customers → $1k MRR, Stripe → Supabase → Workers free-tier")
    lines.append("- **Ava AGI**: multi_jspace_module.py 4 workspaces S1 hl=8 S2 hl=300 Critic hl=30 Planner hl=150, Router/veto")
    lines.append("- **Vector Hoops**: 12,966 player-seasons, MTNN v5_concat_b2_h160_t32_d48_mlp128, CQS 85.87, leakfree 0.7937 composite")
    lines.append("")
